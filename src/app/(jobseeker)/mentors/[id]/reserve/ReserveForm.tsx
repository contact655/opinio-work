...
export default function ReserveForm({
  ...
  const canSubmit = selectedThemes.size > 0 && situation.trim() && questions.trim() && userEmail.trim() && !isSubmitting;
  ...
  async function handleSubmit() {
    ...
    setSubmitError("");
    try {
      const res = await fetch("/api/mentor-reservations", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          mentor_id: mentor.id,
          themes: Array.from(selectedThemes),
          current_situation: situation.trim(),
          questions: questions.trim(),
          background: background.trim() || null,
          preferred_days: Array.from(selectedDays),
          preferred_times: Array.from(selectedTimes),
          contact_email: userEmail,
          preferred_platform: tool === "どちらでも構わない" ? null : tool,
        }),
      });
      if (!res.ok) {
        const j = await res.json().catch(() => ({}));
        setSubmitError((j as { error?: string }).error ?? "送信に失敗しました");
        return;
      }
      setShowModal(true);
    } catch {
      ...
      <main style={{ maxWidth: 760, margin: "0 auto", padding: "48px 32px 80px" }}>
        ...
        <section style={{
          ...
          <div style={{ marginBottom: 4 }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              {(mentor.themes.length > 0 ? mentor.themes : ["キャリア相談", "転職相談"]).map((theme) => {
        ...
        <div style={{
          ...
          <div style={{ fontSize: 10, opacity: 0.8, marginTop: 14, lineHeight: 1.7 }}>
            送信することで、利用規約およびプライバシーポリシーに同意したものとみなされます。
          </div>
        </div>
      </main>
