      ...
      <section className="lp-section" style={{ background: C.paper2 }}>
        <div style={{ maxWidth: 780, margin: "0 auto", padding: "0 28px" }}>
          ...
          <div style={{ borderTop: `1px solid ${C.line}` }}>
            {[
              ...
              /* ⚠️★ここにあった FAQ「登録すると、スカウトが届きますか？」は
                     **2026-09-20 に削除した**（柴さんの指示）。長い経緯の注記も一緒に畳んだ。
                     ⚠️ 書き戻すときに要る事実だけ残す:
                       ・受け取りの可否は `ow_profiles.career_stance`（本人が選ぶ・既定値なし）。
                         **「初期設定は受け取る」と書かないこと。** 未設定のあいだは届かない
                       ・同じ内容を言う場所は `/onboarding/stance` と利用規約 第8条。
                         **ただし「準備中」はどちらにも無い**（＝求職者側には現在どこにも無い）
                       ・「準備中」は `isScoutSendingEnabled()` に連動させてあった。
                         **手で書かないこと**（消し忘れと消しすぎの両方が起きる）
                       ・「始まりましたらお知らせします」と書かないこと。
                         **知らせる手段が実装されていない**（お知らせのテーブルが無く、
                         `ow_notifications` の種別は like / comment / scout / message の4つだけ）
                     ⚠️ 経緯の全文は git 履歴（89ec68a3 の1つ前）にある。 */
            ...
            /* ⚠️★**`open` を戻さないこと**（2026-09-20 / 柴さんの指摘）。
                  先頭の1問だけ `open: true` が付いており、**リロードのたびに開いた状態**で
                  出ていた。**全問を閉じて始める。**
               ⚠️ `details` は利用者が開いた状態を保持しないので、1問だけ開いていると
                  「なぜこれだけ？」という見え方になる。 */
            ].map((item, i) => (
              <details key={i} style={{ borderBottom: `1px solid ${C.line}`, padding: "18px 0" }}>
