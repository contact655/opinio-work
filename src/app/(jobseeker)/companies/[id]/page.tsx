        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
          ...
          <div style={{ padding: "20px 22px", background: "var(--royal-50)", border: "1px solid var(--royal-100)", borderRadius: 12 }}>
            ...
            {hasProducts ? (
              ...
              <EmptyPlaceholder label="製品・サービス情報" />
            )}
          </div>
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: "24px 28px 32px", display: "flex", flexDirection: "column", gap: 32 }}>
        {DIVISION_ORDER.filter(div => grouped.has(div)).map((div) => {
          const teams = grouped.get(div)!;
          const config = DIVISION_CONFIG[div] ?? { label: div, icon: null, color: "var(--royal)", bg: "var(--royal-50)", border: "var(--royal-100)", missionBg: "var(--royal-50)" };
          return (
            <div key={div}>
              {/* Division header */}
              <div style={{
                display: "flex", alignItems: "center", gap: 8, marginBottom: 14,
                paddingBottom: 10, borderBottom: `2px solid ${config.border}`,
              }}>
                <span style={{
                ...
                <div>
                  <span style={{ fontSize: 15, fontWeight: 800, color: config.color, fontFamily: "var(--font-noto-sans)" }}>
                    {config.label}
                  </span>
                  <span style={{ fontSize: 12, color: "var(--ink-soft)", marginLeft: 8, fontFamily: "Inter, sans-serif" }}>
                    {teams.length} チーム
                  </span>
                </div>
              </div>
              ...
              {/* Team cards grid */}
              <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 12 }} className="grid grid-cols-1 md:grid-cols-2">
                {teams.map((team) => (
                  <div
                    key={team.name}
                    style={{
                  ...
                  >
                    ...
                    <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 8 }}>
                      <h3 style={{ margin: 0, fontSize: 14, fontWeight: 800, color: "var(--ink)", lineHeight: 1.4, fontFamily: "var(--font-noto-sans)" }}>
                        {team.name}
                      </h3>
                      <span style={{
