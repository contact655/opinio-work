...
export const metadata = {
  title: { absolute: "掲載企業の審査基準 | OPINIO" },
  description:
    "OPINIOは編集部による独自の審査を経た企業のみを掲載しています。審査プロセス・3軸の評価基準・取材体制をご紹介します。",
  alternates: { canonical: "https://opinio.jp/about/selection-criteria" },
  openGraph: {
  ...
  const axes = [
    {
      ...
      icon: (
        <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="var(--royal)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    ...
    <div className="min-h-screen" style={{ background: "var(--bg-tint)" }}>
      ...
      <div className="max-w-[820px] mx-auto px-4 sm:px-6 lg:px-8" style={{ paddingTop: 40, paddingBottom: 64 }}>
        ...
        <section style={{ background: "#fff", borderRadius: 14, padding: 28, boxShadow: "0 1px 4px rgba(0,0,0,0.06)", marginBottom: 20 }}>
          <h2 style={{ fontSize: 18, fontWeight: 700, color: "var(--ink)", marginBottom: 6 }}>3軸の評価基準</h2>
          <p style={{ fontSize: 13, color: "var(--ink-mute)", marginBottom: 20 }}>
            それぞれを定量・定性の両面で評価し、いずれかが基準を満たさない企業は掲載を見送ります。
          </p>
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {axes.map((a) => (
              <div key={a.title} style={{ display: "flex", gap: 14, padding: 16, borderRadius: 12, background: "var(--bg-tint)", border: "1px solid var(--line)" }}>
                <div style={{ width: 40, height: 40, borderRadius: 10, background: "var(--royal-50)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                  {a.icon}
                </div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 15, fontWeight: 700, color: "var(--ink)", marginBottom: 4 }}>{a.title}</div>
                  <p style={{ fontSize: 13, color: "var(--ink-soft)", lineHeight: 1.7, margin: 0 }}>{a.desc}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* 審査プロセス */}
        <section style={{ background: "#fff", borderRadius: 14, padding: 28, boxShadow: "0 1px 4px rgba(0,0,0,0.06)", marginBottom: 20 }}>
          <h2 style={{ fontSize: 18, fontWeight: 700, color: "var(--ink)", marginBottom: 6 }}>編集部の審査プロセス</h2>
              ...
              return (
                <div key={s.n} style={{ display: "flex", gap: 16, position: "relative" }}>
                  <div style={{ display: "flex", flexDirection: "column", alignItems: "center", flexShrink: 0 }}>
                    ...
                    {!isLast && (
                      <div style={{ width: 2, flex: 1, background: "var(--line)", marginTop: 4, marginBottom: 4, minHeight: 30 }} />
                    )}
                  </div>
                  <div style={{ flex: 1, paddingBottom: isLast ? 0 : 22 }}>
                    <div style={{ fontSize: 15, fontWeight: 700, color: "var(--ink)", marginBottom: 4 }}>{s.title}</div>
        ...
        {/* 早期離職ゼロの根拠 */}
        <section style={{ background: "var(--royal-50)", borderRadius: 14, padding: 28, border: "1px solid var(--royal-100)", marginBottom: 20 }}>
          <h2 style={{ fontSize: 18, fontWeight: 700, color: "var(--ink)", marginBottom: 10 }}>早期離職ゼロ実績の根拠</h2>
          <p style={{ fontSize: 14, color: "var(--royal)", lineHeight: 1.8, margin: 0, marginBottom: 12 }}>
        ...
        <section style={{ background: "#fff", borderRadius: 14, padding: 28, boxShadow: "0 1px 4px rgba(0,0,0,0.06)", marginBottom: 24 }}>
          <h2 style={{ fontSize: 18, fontWeight: 700, color: "var(--ink)", marginBottom: 10 }}>取材体制</h2>
          <p style={{ fontSize: 14, color: "var(--ink-soft)", lineHeight: 1.8, margin: 0, marginBottom: 12 }}>
          ...
          <ul style={{ display: "flex", flexDirection: "column", gap: 8, listStyle: "none", margin: 0, padding: 0 }}>
            {[
              ...
              <li key={item} style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
                ...
                <span style={{ fontSize: 13, color: "var(--ink-soft)", lineHeight: 1.7 }}>{item}</span>
              </li>
            ))}
          </ul>
        </section>
