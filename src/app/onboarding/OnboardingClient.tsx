...
function OnboardingInner({
  ...
  /* ⚠️★**範囲外の大きい値は「最後の画面」に寄せる**（2026-09-15）。
        画面を減らすと、**それまで開いていた `?step=4` が一斉に範囲外になる。**
        1 に落とすと、保存済みの人まで最初からやり直しに見える。
     ⚠️ 不正な値（数字でない・0以下）は従来どおり 1。 */
  const step = !Number.isInteger(rawStep) || rawStep < 1
    ? 1
    : Math.min(rawStep, STEPS.length);
  ...
  */
  ...
  /* ★★1画面目の保存（2026-09-11 / (c) を採った）。
     ...
     ⚠️★**職歴だけは入口で取れないと、そのあと誰も入れない**
        （実測 2026-09-16: **実ユーザー9人中、職歴があるのは6人**。
         ⚠️ 母数は `is_test` を倒すたびに動く。**数字を書くときは実測日を添えること**）。
  ...
  /* ★CTA を押せる条件。**画面ごとに違う。**
     ⚠️★**「N画面目」で書かないこと**（2026-09-15 に直した。番号がずれる）。`STEP` で書く。
       YOU     … 必須5項目が埋まるまで押せない（`youReady`）
       COMPANY … 会社を入れるまで押せない。⚠️ 抜け道は**「後で設定する」**（完了して
                 `/onboarding/stance` へ行く）。押せないままにはならない
  ...
  const goNextFromStance = async () => {
    ...
    /* ★★**この画面が最後**（2026-09-15 に「あとは任意」を削除）。進むのではなく完了する。
       ⚠️★**`goStep(数字)` と書かないこと。** 2026-09-14 に画面を1つ挟んだとき
          `goStep(3)` が残り、**この画面自身に戻って**オンボーディングが詰まった
          （2026-09-15 に本番で発覚）。押しても何も起きないので tsc でも lint でも出ない。 */
    await finish();
  };
  ...
  const finish = async () => {
    ...
    if (user) {
      ...
      /* ⚠️★**これまでの職歴の一括登録は 2026-09-15 にやめた**（4画面目ごと削除）。
            足し戻すなら画面も一緒に。`/mypage` の「＋ 職歴を追加」が入口。 */
      ...
      /* ⚠️★**学歴の一括登録も 2026-09-15 にやめた**（同上）。`/mypage` の「＋ 学歴を追加」が入口。 */
  ...
  return (
    <div style={pageWrap}>
      <div style={{ width: "100%", maxWidth: 480 }}>
        ...
        <div style={{
          ...
          <h2 style={{
            ...
            {/* ★★見出しは画面ごとに変える（2026-09-11）。
                   ...
                   ⚠️★**「任意」という語をここに書かないこと**（2026-09-11 に4回消した）。
                      ⚠️ 「あとは任意」の画面は 2026-09-15 に削除したので、この見出しの
          ...
          {/* ══ ★★1画面目（`?step=1`）＝「あなたのこと」（2026-09-14 / 柴さんの指示）══
              **氏名・ふりがな・生年月日を冒頭で必須にする。**
              ...
              ⚠️ 氏名も登録フォームでは**任意・いちばん下**だった（2026-09-14 に必須化）。
                 ⚠️★**「実測で12人中1人がメールのローカル部だった」は誤りだった**（同日訂正）。
                    正しく数えると `name = split_part(email,'@',1)` は**全46行中1行**で、
                    それは検証用の `contact+01`。**実ユーザーは0人。**
                    判定に `^[a-zA-Z0-9._+-]+$`（ASCII だけの名前）を使い、
                    「メールと同じ」と読み替えたのが原因（CLAUDE.md「部分一致で結論を出さない」）。
                 ⚠️★**冒頭に移した理由は生年月日のほう**（3/12）と、
                    **7人中2人（29%）がオンボーディングを完了しない**こと。
          ...
          {/* ── ★3画面目（`?step=3`）───────────────────────────────────────
                 ⚠️★1画面目は「保存に必要な3点」だけ。ここから下は
                    **押さなくても登録が終わる**もの。 */}
          {/* ★★4画面目「あとは任意」は **2026-09-15 に削除した**（柴さんの判断）。
                 ⚠️★**戻さないこと。** 戻すなら、下の実測を覆す根拠を先に用意すること。

                 消した中身は 勤務地 / 勤務形態 / この会社に役割を追加 / これまでの職歴 / 学歴。
                 **どれも `/mypage` で入れられる**ので、失われた機能は無い（聞く場所を移しただけ）。

                 ── なぜ消したか（2026-09-15 実測 / 実ユーザー）────────────────────
                 ⚠️★**ここには「後から追記してもらうのが最も難しいデータなので入口で聞く。
                    編集画面に追いやると実際には誰も戻ってこない」と書いてあったが、
                    どちらも実測で支持されなかった。**
                      ・入口で聞いても **勤務地・勤務形態は12行中1行**しか埋まっていない
                        （2026-08-13 に追加して約1か月）
                      ・**あとから戻って足している人はいる**（学歴・職歴とも、登録の
                        1日以上あとに足された行が複数）
                 ⚠️ 加えて**画面が読みにくかった** ——「これまでの職歴」を開くと
                    「**この会社での前の役割**」が現職の会社名で prefill され、
                    過去の勤務先を足したい人が別のものを聞かれる形だった。 */}
          ...
          <div className="onb-cta-sticky" style={{ marginTop: 20 }}>
            {/* ⚠️★**公開範囲の一文（「選んだ会社の企業ページに、あなたの名前が実名で
                   表示されます（見えるのは OPINIO にログインしている人だけです）。」）は
                   2026-09-12 に柴さんの指示で削除した。戻すときは下を読むこと。**

                ── 消えたことで、いまこうなっている ────────────────────────────
                ⚠️★**「登録した会社の企業ページに実名で載る」と告げる場所が、製品のどこにも無い。**
                   利用規約・プライバシーポリシー・アプリ内の本人向け画面のいずれにも
                   書かれていない（2026-09-09 に全文検索して確認した結果）。
                   「見えるのはログイン中の人だけ」の側は利用規約 第7条とプライバシーポリシーが
                   カバーしているが、**実名掲載そのものは別。**
                ⚠️ 保存される値は `visibility_company = 'real'`（＝実名で出る）。
                   2026-08-14 に「会社名は伏せる」チェックを外して以来、ここが唯一の告知だった。

                ⚠️★**戻すなら1文に縮めること。3文には戻さない**（2026-08-14 に一度縮めている）。
                ⚠️★**規約側に書き足す案を採るなら、改定日の告知が要る。** */}
            ...
            <button
              type="button"
              /* ⚠️★最後の画面（STANCE）は `goNextFromStance` が**保存したうえで
                    `finish()` を呼ぶ**。ここで `finish` を直に繋がないこと
                    ——`career_stance` の保存が飛ぶ。 */
        ...
        {/* スキップ。
            ⚠️★★**画面番号でなく `STEP` の名前で書くこと**（2026-09-16 に書き直した）。
               以前は「2画面目には出さない」と書いてあったが、**それは STANCE がまだ
               2画面目だった頃の記述**で、いまの2画面目（COMPANY）とは逆の意味に読めた。
               CLAUDE.md の「**名前にも画面番号を入れないこと**」は、コメントにも当てはまる。

            いまの条件は `step !== STEP.STANCE && step !== STEP.YOU`
            ＝ ★**出るのは COMPANY（直近のお勤め先）だけ。**

            ⚠️★**STANCE には出さない**（2026-09-11）。あの画面は `career_stance` を
               選ぶまで進めない作りなので、ここに離脱口があると**その場で無効になる。**
            ⚠️★**YOU にも出さない**（2026-09-14）。あの画面の5項目は**必須**なので、
               ここに離脱口があると必須にならない（柴さんの指示で生年月日を必須にした）。

            ⚠️★★**したがって、経歴を落とす唯一の抜け道が、経歴を聞く画面にある。**
               押すと `finish()` が走り、完了扱いで**経歴は保存されない**。
               実測（2026-09-16 / 実ユーザー9人）: **完了済みなのに経歴0が2人。**
               ⚠️ 下書きも部分保存も無いので、**このボタン由来かは区別できない。**
               ⚠️ これは「任意項目に逃げ道を残す」判断の結果で**バグではない**。
          ...
          <div style={{
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              {/* ⚠️★**「OPINIOに未掲載（企業ページはありません）」を戻さないこと**
                     （2026-09-12 / 柴さんの指示で削除）。
                     ⚠️ 会社を選ぶ人にとって**掲載の有無は関係ない**。自分の勤務先を選ぶだけで、
                        掲載されているかどうかで選択が変わることはない。
                     ⚠️ 掲載の有無は運営側の状態であって、本人の経歴の正しさとは無関係。
                        出すと「選べないのでは」「間違いなのでは」と読ませるだけになる。
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  {/* ⚠️★**掲載の有無をここに出さないこと**（2026-09-12 / 柴さんの指示で削除）。
                      ...
                      ⚠️ 業種も出さない（`lookup` が返さない。未掲載企業の業種まで
                         出すことになるため）。
                      ⚠️ 未掲載でも**選べる**（`company_id` で繋がる）ことは変わらない。 */}
                </div>
