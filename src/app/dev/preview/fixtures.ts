...
/* ── 主な営業先（2026-09-03 追加）─────────────────────────────────────────
   ⚠️★**「主な顧客」とは別の項目。** あちらは**顧客そのもの**（企業名）で、
      こちらは**顧客企業の中のどの部署に売るか**。粒度が違うので混ぜないこと。
   ⚠️ `customer_cases` の有無で出し分け**ない**（顧客リストと違い独立した項目）。
   ⚠️ 実データは **Opinio 1社・3件のみ**（2026-09-03 実測）。
      上限（5件）・超過・長文はここでしか踏めない。
   ⚠️ 製品と同じ `products-grid` / `PRODUCTS_LIMIT`（5件）を使う。 */
export const SALES_TARGETS_1 = ["営業部"];

/** ⚠️ 本番の実データ（株式会社Opinio）と同じ3件 */
export const SALES_TARGETS_3 = ["営業部", "人事部", "情報システム部"];

/** ⚠️ 上限ちょうど。900px で1行に収まり「すべて見る」は出ない */
export const SALES_TARGETS_5 = [
  "営業部", "人事部", "情報システム部", "マーケティング部", "経営企画部",
];

/** ⚠️★境界。6件目から「すべて見る（残り N）」が挟まる */
export const SALES_TARGETS_7 = [
  ...SALES_TARGETS_5, "カスタマーサポート部", "経理・財務部",
];

/** ⚠️ 括弧つき・長い部署名。1行に収まらないときの省略と、カードが横に伸びないこと */
export const SALES_TARGETS_EDGE = [
  "情報システム部（社内インフラとセキュリティを所管する部門）",
  "人事本部 人材開発統括部 採用戦略グループ",
  "営業",
];
...
/* ── ★求人ペイン（分割ビューの右側）─────────────────────────────────────────
   ⚠️★**本番の公開求人2件は本文が 205〜219字しかない**（2026-09-17 実測）。
      しかも下書き13件は本文が**1文字も無い**。つまり実データでは
      **ペインが1pxもあふれない**（実測: scrollHeight 504 = clientHeight 504）ので、
      内部スクロールも、スクロールしたときに残る帯も**一度も描画されない。**
   ⚠️★**DB には入れないこと。** ローカルの dev も本番 Supabase に繋がる
      （CLAUDE.md「ローカルだから安全ではない」）。長い本文はここに置く。 */
export const JOB_PANE_LONG: Job = job({
  id: "pv-job-long",
  slug: "pv-job-long",
  role: "エンタープライズ営業本部 第一営業部 アカウントエグゼクティブ（金融・通信担当）",
  highlight: "業種を越えた大規模アカウントに、複数プロダクトを横断して提案する。",
  employment_type: "正社員",
  location: "東京都（丸の内）／大阪府／愛知県",
  work_style: "ハイブリッド（週2日出社）",
  salary_min: 1200, salary_max: 2500,
  salary_note: "※ 年俸制。上記は基本給と賞与を含む想定年収です。",
  work_hours: "所定労働時間8時間、フレックスタイム制（コアタイム 11:00-15:00）",
  holidays: "完全週休2日制（土日）、祝日、年末年始、有給休暇（初年度10日〜）",
  probation_period: "あり（3ヶ月／条件の変更なし）",
  roleLabel: "アカウントエグゼクティブ",
  /* ⚠️ markdown で描かれる。見出し・箇条書き・強調が解釈されることを確かめる。
     ⚠️★日本語の **強調** は約物と隣り合うと効かない（CLAUDE.md）。ここでも避けてある。 */
  overview: `## 募集の背景

国内エンタープライズ領域の引き合いが増え、**専任のアカウントエグゼクティブ**を増員します。

既存のお客様は金融・通信を中心に大規模なアカウントが多く、
1社あたりの検討期間が長く、関わる部署も多いのが特徴です。

## このポジションの位置づけ

- 商談の入口から、導入後の定着まで一貫して担当します
- プリセールス・カスタマーサクセスと**同じチーム**で動きます
- 四半期ごとに担当アカウントを見直します

## 大切にしていること

数字を追うだけの動き方はしていません。
お客様の課題が自社の製品で解けないときは、そう伝える前提で商談を進めています。`,
  main_tasks: [
    "金融・通信領域の大規模アカウントに対する新規開拓と既存深耕",
    "経営層・情報システム部門・事業部門それぞれへの提案と合意形成",
    "プリセールスと連携した提案書・見積の作成、社内稟議の設計支援",
    "導入後の定着状況のモニタリングと、追加提案の企画",
    "パートナー企業との共同提案および案件の共有",
  ],
  required_skills: [
    "法人向けソフトウェアまたはSaaSの営業経験（3年以上）",
    "エンタープライズ（従業員1,000名以上）への提案経験",
    "無形商材の提案経験",
    "日本語：ネイティブレベル",
    "社内外の関係者を巻き込んだプロジェクト推進の経験",
    "数値目標に対する達成責任を負った経験",
    "Salesforce などのCRMを用いた案件管理の経験",
  ],
  preferred_skills: [
    "金融または通信業界への提案経験",
    "英語：ビジネスレベル（海外本社との連携が発生します）",
    "パートナーセールスの経験",
    "MEDDIC / MEDDPICC などの商談管理手法の実践経験",
  ],
  selection_flow: [
    { step: "1", name: "書類選考", meta: "1週間程度" },
    { step: "2", name: "カジュアル面談", meta: "希望される方のみ。選考には含みません" },
    { step: "3", name: "一次面接（現場マネージャー）", meta: "60分／オンライン" },
    { step: "4", name: "二次面接（部門長）", meta: "60分／オンライン" },
    { step: "5", name: "最終面接（本部長）", meta: "60分／原則対面" },
    { step: "6", name: "内定", meta: "条件提示ののち2週間以内にご回答ください" },
  ],
  selection_note: "選考の順番や回数は、ご経験に応じて前後する場合があります。",
});

/** ⚠️★中身が薄い求人。**帯だけが残って本文が無い形**を見る（下書き13件がこれ） */
export const JOB_PANE_THIN: Job = job({
  id: "pv-job-thin",
  slug: "pv-job-thin",
  role: "カスタマーサクセス",
  overview: "", main_tasks: [], required_skills: [], preferred_skills: [],
  selection_flow: [], selection_note: "",
  salary_min: 0, salary_max: 0, location: "", work_style: "", employment_type: null,
  roleLabel: null,
});
