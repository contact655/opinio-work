 ...
 *
 * ── ✅ 決着: 案B のまま（2026-09-02 / 柴さん）────────────────────────────
 * 実物を並べたうえで「今のままで」。**色は変更しない。**
 *
 * ⚠️★**以前のオレンジ `#F59E0B` には戻せない。** 白文字で **2.15**（必要 4.5）で、
 *    基準の半分以下。**文字を 19px にしても届かない**（3.0 にも満たない）。
 *    「暗い／赤茶に寄っている」という感覚は正しいが、明るくすると読めなくなる。
 * ⚠️ 明るさを取り戻す唯一の道は案F（`#EA580C` ＋ 19px 太字で 3.0 基準）だったが、
 *    **固定バーが高くなる**ため採らなかった。
 *
 * ⚠️★**このページは消さない。** 「もっと明るい橙にできないか」は今後も出る問いで、
 *    そのたびに測り直すことになる。ここに実物と数字が並んでいる状態を保つ。
 *    （アバターの A/B 比較を消したのは、表を畳んだ結果**2枚が同じ見た目になった**ため。
 *      こちらは案ごとに実際に違って見えるので、比較として生きている。）
 *
...
const ROWS = [
  ...
  {
    key: "案B ── 橙を濃くして白文字（グラデを保つ）★採用（2026-09-02 決着）",
