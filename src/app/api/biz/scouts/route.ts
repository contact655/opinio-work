...
export async function POST(req: NextRequest) {
  /* ⚠️ **スカウト送信はまだ止めてある。** 再開は SCOUT_SENDING_ENABLED=true を
     ...
     ⚠️★**「転職について」に答えていない人には送れない**（2026-08-27 / フェーズ3）。
        `can_send_scout` が `career_stance` を見るようになり、**未設定は false 扱い**。
        止めるのは `no_contact`（いまは声をかけられたくない）と未設定の2つだけ。
        ⚠️ 実測（2026-08-26 / 本番・読み取りのみで突き合わせ）: 送れる実ユーザーは
           **1人 → 2人**。減るのではなく入れ替わる（`scout_enabled` が null のまま
           「積極的に検討中」と答えていた2人に、はじめて届くようになる）。
