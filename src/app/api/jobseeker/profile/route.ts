...
export async function PUT(req: Request) {
  ...
  const patch: {
    /* ★表示名。⚠️★**姓＋名からの派生としてしか入らない**（下の1箇所）。
          リクエストの `name` をそのまま入れる分岐を足さないこと。 */
    name?: string;
  ...
  try {
    /* ★★`name` を**単独で書く経路は廃止した**（2026-09-15）。
          ⚠️★`ow_users.name` は「姓＋名」の**派生値**。書くのは下の1箇所だけ。
          ⚠️ 2026-09-15 まではここで単独の `name` も受けており、`/mypage` の
             「名前」欄（1欄）とオンボーディング1画面目（姓・名）が
             **同じ列を別々に書いていた**。先に保存したほうが後勝ちで、
             `name` と `family_name`/`given_name` が食い違う形だった
             （実害0件だったのは、まだ誰も姓・名を持っていなかったから）。
          ⚠️★**この分岐を復活させないこと。** 表示名を変えたいなら姓・名を送る。
          ⚠️★**型では止まらない。** `body` は `Record<string, unknown>` なので、
             呼び出し側が `name` を送っても tsc は何も言わない。**歯止めは
             この `console.warn` と本注記だけ**——送り元を足さないこと。
          ⚠️ 新規作成時の `name` は `lib/auth/linkOwUser.ts` の INSERT が入れる
             （登録フォームの「お名前」1欄）。**あれは作成で、ここは更新。別の話。** */
    if ("name" in body) {
      console.warn("[jobseeker/profile] `name` は受け付けない（姓・名から派生させる）。送り元を直すこと。");
    }
