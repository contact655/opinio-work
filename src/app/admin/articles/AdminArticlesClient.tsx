...
/* ⚠️ **データはサーバー（page.tsx + createAdminClient）で取る。**
      ...
      `ow_company_admins` に運営ポリシー（auth_is_admin）が無く、
      実測で4件が運営に見えていなかった（企業↔ユーザーの紐づけ候補が欠ける）。 */
export default function AdminArticlesClient({
  initialArticles, initialUsers, initialCompanies, initialCompanyUserPairs,
}: {
  initialArticles: Article[];
  initialUsers: OWUser[];
  initialCompanies: OWCompany[];
  /** [company_id, user_id] の組。Map は Server → Client に渡せないので配列で受ける */
  initialCompanyUserPairs: [string, string][];
}) {
  const [articles, setArticles] = useState<Article[]>(initialArticles);
  const [users] = useState<OWUser[]>(initialUsers);
  ...
  const [companies] = useState<OWCompany[]>(initialCompanies);
  // company_id → Set<user_id> (ow_experiences + ow_company_admins から構築)
  const companyUserMap = useMemo(() => {
    const map = new Map<string, Set<string>>();
    for (const [companyId, userId] of initialCompanyUserPairs) {
      if (!map.has(companyId)) map.set(companyId, new Set());
      map.get(companyId)!.add(userId);
    }
    return map;
  }, [initialCompanyUserPairs]);
