            ...
            <>
              ...
              <form onSubmit={handleSignup}>
                ...
                {/* ★お名前は**必須**（2026-09-14 / 柴さんの判断）。
                    ⚠️★**「省略可」に戻さないこと。** 省略されると
                       ...
                       **メールアドレスのローカル部が表示名として `/people` に出る**
                       （`/people` は `ow_users` 起点なので職歴が無い人も出る）。
                    ⚠️★**ただし実際には一度も起きていない**（2026-09-14 に測り直した）。
                       `name = split_part(email,'@',1)` で数えると**全46行中1行だけ**で、
                       それは `contact+01`（`is_test = true`）。**実ユーザーは0人。**
                       ⚠️ 最初に「実ユーザー7人中1人」と報告したのは**判定式の誤り**で、
                          `^[a-zA-Z0-9._+-]+$`（ASCII だけの名前）を「メールと同じ」と
                          読み替えていた。当たっていたのは本人が入力した英字名。
                    ⚠️★**それでも必須にする理由は別にある。** 実測で
                       **7人中2人（29%）がオンボーディングを完了していない。**
                       1画面目で離脱する人は姓名を入れる前に離脱するので、
                       **この欄が唯一の受け皿。**
                ...
                */}
                ...
                {/*
                  ★★スカウトの受け取り方の告知は**この画面から外した**（2026-09-09 / 柴さんの判断）。
                     理由は**長さ**。登録ボタンの下に4文（規約の1文と合わせて5文）が並び、
                     **登録の直前でいちばん読まれない位置に、いちばん長い文があった。**

                  ⚠️★**2026-09-20 に①が消えた。** 残っているのは②と③の**2つ**:
                       ① ~~LP の FAQ「登録すると、スカウトが届きますか？」~~
                          **柴さんの指示で削除。** 書き戻すなら LandingPage.tsx の FAQ 配列へ。
                       ② **登録直後の `/onboarding/stance`** —— この画面は
                          「この答えで、企業の採用担当から声をかけられるかどうかが決まります」
                          「あとから何度でも変えられます」「在籍中の会社と職歴の会社からは届きません」
                          を**選ぶ前に**出している。**必要な場面で必ず読まれる。**
                       ③ 利用規約 第8条（受け取りの条件そのもの）
                     ⚠️ **②を薄くしないこと。** ここを外したぶん、
                        登録者が意思表示の意味を知る唯一の確実な経路になっている。

                  ⚠️★**「現在準備中」は求職者側のどこにも無くなった**（2026-09-20）。
                     ②に無く、唯一あった①の FAQ を削除したため。
                     **再開時（`SCOUT_SENDING_ENABLED=true`）に消す文言は、求職者側には無い。**
                     LandingPage.tsx 側のコメントが「ここと登録画面の2箇所」と書いていたのは
                     この変更で**1箇所になった**（あちらも直してある）。

                  ⚠️ 統計的利用の告知も 2026-08-21 に同じ理由で外している。
                     根拠は**利用規約 第13条の4 に必要事項が揃っている**こと
                     （対象情報・目的・僅少なら公表しない・停止の請求先）。
                     上の「利用規約およびプライバシーポリシーに同意したものとみなします」で
                     包括的に同意を取っているので、画面に再掲しなくても伝達経路は残る
