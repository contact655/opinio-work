          ...
          {mode === "signup" && (
            <>
              ...
              <form onSubmit={handleSignup}>
                ...
                {/*
                  スカウトの受け取り方の告知。
                  ⚠️★2026-08-27 に**既定でオンをやめた**（フェーズ3）。送信可否は
                     `ow_profiles.career_stance` が決め、**答えるまでは届かない**。
                     既定でオンではなくなったので「受け取る設定になります」と書かないこと。
                  ⚠️ 登録の直後に1問おたずねする導線（`/onboarding/stance`）とセット。
                     どちらかを外すと、登録した人が意思表示に一度も触れないまま終わる。
                  ⚠️ 利用規約 第8条と LP の FAQ が同じことを言っている。
                     **3箇所を同時に直すこと**（規約は改定日が要る）。
                  ★★「現在準備中」を足した（2026-09-01）。**ここと LP の FAQ の2箇所。**
                     ⚠️ **規約（③）には足していない。** ③が定めているのは
                        **受け取りの条件**（`career_stance` に答えるまで届かない）で、
                        その条件は**変わっていない**。準備中は**運用上の一時的な状態**なので
                        規約に書くものではない（書けば改定日が要る）。
                        **「3箇所を直し忘れた」ではない。**
                     ⚠️ ここを外さないこと。招待リンクから直接この画面に来る人は
                        LP の FAQ を読んでいない。
                     ⚠️ **再開するときは2箇所とも消す**（`SCOUT_SENDING_ENABLED=true` と同時）。
                ...
                */}
                <p style={{ fontSize: 12, color: "var(--ink-mute)", fontWeight: 500, textAlign: "center", lineHeight: 1.8, marginTop: 6 }}>
                  登録のあと、企業から声をかけられてもよいかを1問おたずねします。答えるまで、企業からあなたに連絡は届きません。答えはマイページの「意思表示」からいつでも変えられます。なお、企業からのスカウト送信は現在準備中です。始まりましたらお知らせします。
                </p>
                {/*
