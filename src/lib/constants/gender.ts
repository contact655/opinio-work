 ...
 */
/* ── ⚠️★この列の「使い道」はまだ決まっていない（2026-09-14）──────────
 * **柴さんの判断で、先に器だけ持つと決めた。**
 *
 * ⚠️★**「読み手が無い死列」と判断して DROP しないこと。**
 *    このリポジトリは「保存経路の無い入力UIを作らない」を禁じているが、
 *    これは**保存も表示もある**（本人：`/mypage/settings` ∕ 運営：`/admin/candidates`）。
 *    足りないのは**その先の業務**だけ。
 *
 * ⚠️★**当初書いていた「求職管理簿のため」は誤り**（同日訂正）。
 *    OPINIO は**ダイレクトリクルーティング（募集情報等提供）**で、
 *    職業紹介ではない。求職管理簿は**別サービス**の話。
 *
 * ⚠️ 使い道が決まったら、**出す先**（本人だけ／運営だけ／企業にも）を
 *    改めて決めること。⚠️★**企業に見せる∕絞らせるは別の判断**で、
 *    上の均等法5条の話が直接かかる。**絞り込みは入れない。** */
