...
export const CAREER_STANCES = [
   ...
   *  ⚠️ ラベルは 2026-08-27 に「いまは声をかけられたくない」→「今はいない」に変えた
   *     （柴さんの指示）。**値と DB の CHECK は変えていない。**
   *  ⚠️★上の注記「4つ目は本人の状態のラベルにしない」と、この文言は**ずれている。**
   *     「今はいない」は連絡の希望ではなく本人の状態として読める。
   *     ⚠️ この欄は**企業から声がかかるかどうかを決める**ので、
   *        「転職の意思が無い」と読んで選んだ人に連絡が届かなくなる。
   *        文言を戻すか、注記のほうを変えるかは、次に触る人が判断すること。 */
  { value: "no_contact",  label: "今はいない" },
] as const;
