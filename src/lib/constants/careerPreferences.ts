...
export const CAREER_STANCES = [
  ...
  /** ⚠️★**スカウトの送信可否はこの値で決まる。** 値（`no_contact`）を変えないこと。
   *  ⚠️ ラベルは 2026-08-27 に「いまは声をかけられたくない」→「今はいない」、
   *     2026-09-12 に「今はいない」→「**今は考えていない**」に変えた（どちらも柴さんの指示）。
   *     **値（`no_contact`）と DB の CHECK は一度も変えていない。**
   *  ⚠️★上の注記「4つ目は本人の状態のラベルにしない」と、この文言は**ずれたまま**。
   *     「今は考えていない」は連絡の希望ではなく本人の状態として読める。
   *     ⚠️ この欄は**企業から声がかかるかどうかを決める**ので、
   *        「転職の意思が無い」と読んで選んだ人に連絡が届かなくなる。
   *        文言を戻すか、注記のほうを変えるかは、次に触る人が判断すること。
   *  ⚠️★**ラベルはここ1箇所。** オンボーディングの `StanceQuestion` と
   *     `/mypage` の `IntentCard` が同じ定数を見る。画面側に書き写さないこと。 */
  { value: "no_contact",  label: "今は考えていない" },
] as const;
