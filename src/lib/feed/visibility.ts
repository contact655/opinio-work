 ...
 *                                      ⚠️ 2026-09-18 以降、public の投稿は作られない（下記）
...
export function isPostVisibleTo(
  ...
  /* 3. 投稿を全体公開にしている場合は、投稿者が login_only でも未ログインに出す。
     ...
     ⚠️★**2026-09-18 以降、この分岐は発火しない。** 投稿欄の「公開範囲」の選択肢を外し、
        `POST /api/jobseeker/posts` が `login_only` 固定で書くようにしたため、
        `public` の投稿が新しく作られない（既存も0件）。
        ⚠️ **消さないこと。** 開け直すときはここが効く側になる。
        ⚠️★**「発火しないから」と言って、判定の順序を入れ替えないこと。**
           2の「投稿者が private なら出さない」より後ろにあることに意味がある。 */
