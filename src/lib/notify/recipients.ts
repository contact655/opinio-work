 ...
 *    企業担当者が /biz に来ていない企業の応募・面談を運営が受けて取り次ぐ。
 *    企業開拓が進むと「求人はあるが担当者は未登録」の企業が増えるため、
 *    そのたびに掲載を下ろす設計にはできない。
 *
 *    ⚠️★**訂正（2026-09-15）。この直前に「まだ実装していない」と書いたのは誤り。**
 *       **③は 2026-08-23 に実装済みで、いま稼働している**（この関数の最後の
 *       `return { to: [ADMIN_EMAIL], viaOps: true }`）。上の見出しにもそう書いてある。
 *       **設計メモだけを読んで「未実装」と判断しないこと。関数の最後まで読む。**
 *
 *    ⚠️★★**それとは別に、前提の1つが誤っていた。**
 *       元は「Opinio は**有料職業紹介事業者なので**、運営が受けて取り次ぐのが本来の形」と
 *       書いてあったが、**OPINIO は募集情報等提供（職安法4条6項）で職業紹介ではない**
 *       （`content/legal/terms-of-service-listing.md` 第2条3項）。許可 13-ユ-316441 は
 *       **会社の別サービス**のもの。**根拠の文はここから外した。**
 *       ⚠️★**理由を書き換えるだけで済む話ではない。** 「**運営が間に入って取り次ぐ**」形
 *          そのものが、募集情報等提供の枠に収まるかの判断を要する。⚠️★**そしてこれは
 *          「これから足すか」ではなく、すでに動いている挙動の話。**
 *       ✅★**2026-09-15 に決着（柴さん）：運営は受け取るが、取り次がない。**
 *          企業には「担当者を登録してください」と案内するだけ。登録されれば企業は
 *          `/biz` で応募・面談を**自分で見られる**（行は DB に残っている）。
 *          ⚠️★**運営が候補者と企業の間に入ってあっせんをしない**、という取り決め。
 *          ⚠️★**この③を畳まないこと。** 畳むと宛先0件の企業でCTAが閉じ、
 *             **届かないまま黙って落ちる**形（2026-08-23 以前）に戻る。
 *       ⚠️★**論点はコードではなく運用。** このコードは**運営が受け取るまで**しかしない
 *          （企業へ転送する処理はどこにも無い）。「取り次ぐ」かどうかは運用の取り決め。
 *       → 実装・実測・選択肢は **[docs/ops-fallback-20260915.md](../../../docs/ops-fallback-20260915.md)**。
 *          **ここに書き写さない。**
 *
 *    ── 実測（2026-09-15）。**いま発火しているのは0社だが、余白は小さい** ──
 *       掲載中 **22社中20社が宛先0件**。ただしそのうち
 *       **公開求人を持つ企業 0社 / 面談可 0社**なので、**実際に運営へ回る経路は今日0件**。
 *       ⚠️★**20社のどれかが求人を1件公開するか、面談可をONにした日に効き始める。**
 *          「0件だから当面問題ない」ではなく、**トグル1つ分の余白しかない。**
 *
 ...
 *   **掲載中79社のうち77社が宛先0件**（2026-08-23 実測）で、応募・面談・面談対応者の申請が
 *   誰にも届かない状態だった。
 *
 ...
 *   ⚠️ 実装時点での実影響は**0社**（2026-08-23 実測。⚠️ **その時点の数字**であって、
 *      今後この2つの母数が増えれば影響も増える）:
 *        公開求人を持つ企業 1社（セールスフォース・ジャパン）→ 宛先あり（新しく開く求人 0件）
