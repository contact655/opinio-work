 ...
 *
 * ── なぜ 1.2秒なのか（2026-08-20 / Supabase のドキュメントで確認）───────────
 * Supabase Auth の IP 単位のレート制限は **トークンバケット**で、
 *   ・**バケットの容量は 30 で固定**（ダッシュボードの数値では変えられない）
 *   ・ダッシュボードの数値は**補充の速さ**だけを決める
 * 既定の「150 requests / 5 min」は **0.5 個/秒**、つまり **1個貯まるのに2秒**かかる。
 *
 * ⚠️ **250ms で引き直しても、補充が間に合っていないので同じ 429 を踏むだけだった。**
 *    バケットが空になった直後に意味のある間隔にする。
 *
 * ⚠️ 遅くしすぎない。本当に失敗しているときはこの時間だけログイン画面が遅れる。
 *    最悪でも 上限2.5秒 + 1.2秒 + 上限2.5秒 = 6.2秒（Vercel の 25秒に対して十分余裕がある）。
 */
