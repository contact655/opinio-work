...
export type CompanyLookupResult = {
   ...
   *
   * ⚠️★**2026-09-14 時点で、この値を描画している画面は1つも無い。**
   *    企業ピッカーの3箇所（オンボーディング／職歴エディタ／企業作成ダイアログ）に
   *    「OPINIOに未掲載（企業ページはありません）」を出していたが、
   *    2026-09-12 と 2026-09-14 に**全部削除した**（会社を選ぶ人に掲載の有無は関係ない）。
   *
   * ⚠️★**false ＝「企業ページが無い」ではない。** ディレクトリ非掲載でも
   *    `is_published` ならページは存在する（実際 2026-09-14 に61社がその状態になった）。
   *    **この値を「ページの有無」として読まないこと。**
   *
   * ⚠️ 使い道が戻るまで**新しい読み手を足さない**。足すなら上の2点を先に読むこと。
   */
  isListed: boolean;
};
