          ...
          {!isReachableByCompanies(saved.prefs.career_stance) && (
            saved.prefs.career_stance == null ? (
              ...
              <div style={{
                ...
                <br />
                {/* ⚠️★**「企業から声がかかるようになります」と書かないこと**（2026-09-10 に実測して削った）。
                       スカウトの送信は止まっている:
                         ・本番の `POST /api/biz/scouts` は **503**（`SCOUT_SENDING_ENABLED` が未設定）
                         ・`ow_scouts` は **0件**（一度も送られていない）
                       ⚠️★**「いずれ声がかかります」のような将来形にもしないこと**（柴さんの指示）。
                          動いていないことを言い換えただけになる。
