/* ⚠️ **2026-08-26 時点で参照0件。** 企業説明の入力欄がこれを使っていたが、
      求職者側の描画が plain text（改行で段落分け）で markdown を解釈しないため、
      素の textarea に置き換えた（`##` が記号のまま出ていた／出ることになるため）。
   ⚠️ **消していない。** 描画側を markdown 対応させる日に戻せるようにしてある。
      戻すときは「入力欄と描画をセットで変える」こと。 */
"use client";
