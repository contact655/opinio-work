...
#
# ── ★溜まったダンプの扱い（2026-08-30 に棚卸し）───────────────────────────────
#   ⚠️★**時刻での一括削除をしないこと。** 各ファイルは**別々の migration の直前**の
#      状態で、古い1本が特定の変更を戻す唯一の材料になりうる。
#      「新しい順に N 本残す」方式は**使えない**。
#   ⚠️★**消してはいけないものが3種ある**（2026-08-30 実測）:
#        ① **旧ATSテーブルのダンプ22本**（`agents` `candidates` `crm_*` `jobs` ほか）。
#           **それらの表は DROP 済み**で、`public` は `ow_*` 101表のみ。**唯一の写し。**
#        ② **migration が「戻し方」で名指ししているもの**（88本中47本）。消すと手順が切れる。
#        ③ **`logos/` と `20260828-storage-orphans*/`**。
#           **Storage は日次バックアップに含まれない**ので、実ファイルはここにしか無い。
#   ⚠️★**本番の個人情報が平文で入っている。** 実測でユニークなメールアドレス71件、
#      `storage-candidate-documents/` には候補者の書類。**外に出さない・共有しない。**
#      ⚠️ `.gitignore` 済みだが、**並行セッションが同じ作業ディレクトリを共有している**。
#   → 中身の索引は **`.dumps/README.md`**（棚卸しのたびに作り直す。手で足さない）。
#   ⚠️ 合計 22M。**容量は問題ではない。** 問題は探しにくさと個人情報。
#
