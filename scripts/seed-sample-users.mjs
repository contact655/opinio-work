...
const sampleUsers = [
  { email: "tanaka.shota@example.com", name: "田中 翔太" },
  { email: "sato.misaki@example.com", name: "佐藤 美咲" },
  { email: "suzuki.kenta@example.com", name: "鈴木 健太" },
  { email: "yamamoto.yuki@example.com", name: "山本 由紀" },
  { email: "ito.ryota@example.com", name: "伊藤 遼太" },
  { email: "watanabe.haruka@example.com", name: "渡辺 はるか" },
  { email: "nakamura.takuya@example.com", name: "中村 拓也" },
  { email: "kobayashi.megumi@example.com", name: "小林 恵" },
  { email: "kato.sho@example.com", name: "加藤 翔" },
  { email: "yoshida.ai@example.com", name: "吉田 愛" },
  { email: "yamada.kenji@example.com", name: "山田 賢二" },
  { email: "sasaki.nana@example.com", name: "佐々木 奈々" },
  { email: "inoue.daisuke@example.com", name: "井上 大輔" },
  { email: "kimura.yoko@example.com", name: "木村 陽子" },
  { email: "hayashi.takeshi@example.com", name: "林 武" },
  { email: "shimizu.rina@example.com", name: "清水 里奈" },
  { email: "yamaguchi.sota@example.com", name: "山口 颯太" },
  { email: "matsumoto.sakura@example.com", name: "松本 さくら" },
  { email: "ikeda.hiroshi@example.com", name: "池田 博" },
  { email: "ogawa.mai@example.com", name: "小川 舞" },
];

async function main() {
  // Step 1: 企業IDを確認
  console.log("=== Step 1: 企業IDの確認 ===");
  ...
  if (compErr) {
    console.error("企業取得エラー:", compErr.message);
    return;
  }
  ...
  for (const c of companies) {
    const n = c.name;
    if (n.includes("Salesforce") || n.includes("セールスフォース")) companyMap.SALESFORCE = c.id;
    if (n.includes("HubSpot") || n.includes("ハブスポット")) companyMap.HUBSPOT = c.id;
    if (n.includes("Sansan")) companyMap.SANSAN = c.id;
    if (n.includes("freee") || n.includes("フリー")) companyMap.FREEE = c.id;
    if (n.includes("Ubie") || n.includes("ユビー")) companyMap.UBIE = c.id;
  }

  console.log("\nマッピング結果:", companyMap);

  const requiredCompanies = ["SALESFORCE", "HUBSPOT", "SANSAN", "FREEE", "UBIE"];
  const missing = requiredCompanies.filter((k) => !companyMap[k]);
  if (missing.length > 0) {
    console.error(`\n以下の企業が見つかりません: ${missing.join(", ")}`);
    console.log("企業名を確認してマッピングを調整してください。");
    return;
  }
  ...
  for (const user of sampleUsers) {
    ...
    if (error) {
      if (error.message.includes("already been registered") || error.message.includes("already exists")) {
        ...
        if (existing) {
          console.log(`  既存: ${user.name} (${existing.id})`);
          createdUsers.push({ id: existing.id, name: user.name, email: user.email });
        }
      } else {
        console.error(`  失敗: ${user.email}: ${error.message}`);
      }
      continue;
    }

    console.log(`  作成: ${user.name} (${data.user.id})`);
    createdUsers.push({ id: data.user.id, name: user.name, email: user.email });
  }

  if (createdUsers.length < 20) {
    console.error(`\n${createdUsers.length}/20名のみ作成。続行します。`);
  }

  // Step 3: ow_profiles にname を登録
  console.log("\n=== Step 3: プロフィールにname を同期 ===");
  for (const u of createdUsers) {
    const { error } = await supabase
      .from("ow_profiles")
      .upsert({ user_id: u.id, name: u.name }, { onConflict: "user_id" });
    if (error) {
      console.log(`  プロフィール作成スキップ (${u.name}): ${error.message}`);
    }
  }
  console.log("  プロフィール同期完了");

  // Step 4: work_histories にサンプルデータを投入
  console.log("\n=== Step 4: work_histories 投入 ===");

  const U = (idx) => createdUsers[idx]?.id;
  ...
  // Filter out entries with missing user_id
  const validHistories = workHistories.filter((wh) => wh.user_id);
  console.log(`${validHistories.length}件のwork_historiesを投入します...`);

  const { data: inserted, error: whErr } = await supabase
    .from("work_histories")
    .insert(validHistories)
    .select("id");

  if (whErr) {
    console.error("work_histories投入エラー:", whErr.message);
    // Try one by one
    let successCount = 0;
    for (const wh of validHistories) {
      const { error: singleErr } = await supabase.from("work_histories").insert(wh);
      if (singleErr) {
        const userName = createdUsers.find((u) => u.id === wh.user_id)?.name || "?";
        console.error(`  失敗 (${userName}): ${singleErr.message}`);
      } else {
        successCount++;
      }
    }
    console.log(`  ${successCount}/${validHistories.length}件 投入成功`);
  } else {
    console.log(`  ${inserted.length}件 投入成功`);
  }
  ...
  if (check) {
    check.forEach((wh) => {
      console.log(`  ${wh.ow_companies?.name} | ${wh.role} | ${wh.status} | ${wh.joined_year}〜${wh.left_year || "現在"}`);
    });
  }
