...
# ⚠️ pgrep -f はコマンドライン全体に当たるので、**このスクリプトを起動した shell 自身**を
#    拾うことがある（`sh -c '... npm run dev ...'` のような形。下の ancestors で大半は
#    除けるが、別プロセス経由で起動すると系列から外れて残る）。
#    2026-09-18 に実際に踏んだ: dev が動いていないのに「動いている」と言われて起動できなかった。
#    ⚠️ 回避は `npx next dev`（pre フックを通らないので guard も通らない）。
#    ⚠️ 直していない。fail-open（迷ったら通す）の方針とは逆向きの誤検知だが、
#       誤って**止める**側なので、二重起動の事故よりは軽いと判断している。
