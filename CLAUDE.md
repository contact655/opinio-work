...
- **★★表示件数・人数の前後比較は、Next のキャッシュを先に潰してから測る**（2026-09-10 追記。**2回目**）。
  `unstable_cache` と fetch キャッシュが残っていると、**「適用後の結果」を
  「適用前の測定値」として読んでしまう**（`/people` は **1800秒**、社員一覧は **300秒**）。
  **dev を止めて `.next` を消し、前後それぞれ cold から測る。**
  ⚠️ 2026-08-12 にも同じことを踏んでいる。**数字が食い違ったら、まずキャッシュを疑う。**
