...
### ⚠️ 認証は安くない（2026-08-09 実測）

閲覧者依存の値をクライアント取得に移す設計を検討するとき、この数字を前提にすること。

| エンドポイント | 中央値 |
|---|---|
| `/api/stats`（認証なし） | **0.067秒** |
| `/api/bookmarks`（認証 + 1クエリ） | 0.298秒 |
| 認証 + 2クエリ | **0.585秒** |

API のオーバーヘッドではなく、**`auth.getUser()` と `ow_users` の解決**が重い。
...
**純粋な悪化**になった（revert 済み）。

⚠️ 移してよいのは、**そのページがキャッシュされる場合だけ**。
`/jobs/[id]` は同じ変更をしたが、キャッシュ化（0.069秒）で埋め合わされている。

⚠️ **効くかどうかで速度が桁違いになる。** 同日の実測で
キャッシュが効くページの TTFB は **0.06〜0.10秒**、
効いていなかった頃の `/companies/[id]` は **0.41〜3.11秒**だった。
（修正後は 0.076秒）

#### `/companies/[id]` の作り（2026-08-09 に ISR 化）

**社員一覧を `isAuthenticated` で出し分けている**ため、`generateStaticParams` を
足すだけでは効かなかった。閲覧者ごとに変わるものを全部サーバーから追い出して解消した。

```js
// これがページ本体にあった。今は API 側にある
isAuthenticated ? emps : emps.filter((e) => e.visibility === "public")
```

| どこに移したか | 中身 |
|---|---|
| `CompanyEmployeeSections.tsx`（"use client"） | 社員セクション一式の描画（859行） |
| `GET /api/jobseeker/companies/[id]/employees` | **絞り込み**と在籍者判定 |
| `GET /api/jobseeker/companies/[id]/viewer-state` | ブックマーク・フォロー状態 |

⚠️ **絞り込みは必ずサーバー（API）で行う。** 全件返してクライアントで絞る形に
しないこと。非ログインに公開範囲外の社員情報を配ることになる。

⚠️ **ページ本体に `createClient()` / `auth.getUser()` を足さないこと。**
1箇所でも戻すとルートが動的になり、ISR が黙って効かなくなる。

#### 移設で踏んだ3つ

| 事象 | 原因 | 対処 |
|---|---|---|
| ビルドが `cookies was called outside a request scope` で落ちる | `generateStaticParams` から `getCompanies()`（Cookie を読む）を呼んだ | admin を使う `getCompaniesForList()` に変更 |
| `Cannot dot into a client module` | 定数を `"use client"` のファイルに置き、サーバー側から `.length` を参照した | 指定なしの共有ファイル（`avatarGradients.tsx`）へ |
| サーバー側の他セクションが壊れる | `SecTitle` は社員セクション以外の5箇所でも使っていた | 共有ファイル（`SecTitle.tsx`）に切り出し |

#### 未ログインに社員が0人という論点（性能とは別）

⚠️ `ow_users.visibility` は **public が0人**（login_only 25 / private 1）。
つまり**SEO流入には社員が1人も表示されていない**。ISR 化しても変わらない。
これはプロダクトの判断なので、直すなら「誰に何を見せたいか」から決めること。

⚠️ 社員タブ（sticky nav）だけ表示条件を変えた。閲覧者ごとの可視件数は
サーバーで分からないため、**在籍者が1人でもいれば出す**。
未ログインには中身が空になりうる。
...
### ✅ 完了 2026-07-15 セッション26: JSON-LD修正・テストデータ削除・ソフト404修正・フィード自動集約
  ...
  **テストデータ削除（Migration 231）:**
  ...
  - 公開求人 180件 → 74件（2026-07-15 時点）
  - JSON-LD 出力率 41% → 100%

  **求人ライフサイクル（Migration 232）:**
  - `expires_at` を74件（2026-07-15 時点）に `NOW() + 90日`（2026-10-12）でバックフィル
  ...
  **フィード自動集約（Migration 233 / 234 / 235）:**
  ...
  - 遡及生成: 企業80 + 求人74（2026-07-15 時点）+ 記事16 = 170件（2件の user_post と合わせて計172件）
...
- **/salary の再設計（優先度：高）** — `ow_salary_reports`（ユーザー投稿）が0件でページ未機能。
  `ow_jobs` の74件（2026-07-15 時点、現在は20件）・15職種以上の給与レンジデータで「IT/SaaS職種別 年収相場」ページを作るとSEO集客源になる。
- **E2Eテスト（未実施）** — 次に本物の求人を `/biz/jobs` から登録するとき、フィード自動投稿を確認。
- **/biz/dashboard 初回オンボーディング** — 企業紐付け後・求人0件のチェックリスト型導線が必要。
- **cron expired 自動遷移（未有効化）** — ⚠️ Migration 257 で全件の `expires_at` を NULL に設定済み（2026-07-19）。一斉消滅のリスクは解消。Migration 232 が設定した 2026-10-12 の値は上書きされている。
...
### ✅ 完了 2026-06-14 セッション21: 企業検索フィルター改善
  ...
  **企業検索バー（`src/components/companies/CompanySearchBar.tsx`）:**
  - フェーズドロップダウンに「成長ステージ（シード〜シリーズCのスタートアップ）」を最上部オプションとして追加（インディゴ色）
  - 「ユニコーン」オプション追加（パープル色、DB `unicorn` + `ユニコーン` にマッチ）
  - 「外資系」はフェーズ軸でないため削除済み（別途ピルボタンで対応）

  **企業検索ロジック（`src/lib/search/companies.ts`）:**
  - `PHASE_FILTER_MAP` 追加: UI表示名 → DB実値（英語/日本語混在）を配列でマッピング
  - フェーズフィルターを `.eq("phase", ...)` → `.in("phase", PHASE_FILTER_MAP[...])` に変更
  - 修正効果: 「上場」フィルターが 5社 → **61社**（`listed` 56社 + `上場` 5社 を両方ヒット）
  - 「成長ステージ」= シード/seed/シリーズA/series_a/series-a/シリーズB/series_b/シリーズC/series_c にマッチ（2社）
  - `PHASE_ORDER` も英語/日本語両対応に更新（フェーズソート用）

  **サイドバーDMウィジェット（前セッション継続）:**
  - 企業詳細ページ position-2 の重複従業員プレビューを削除
  - サイドバーに `allEmployees` prop 渡し（現役 + OB/OG 全員）

  **企業検索 業種グループ化（前セッション継続）:**
  - `src/lib/search/industryGroups.ts` 新規作成: 38+の生業種タグ → 8カテゴリに集約
  - `resolveIndustryFilter()` で業種グループキー → DB値配列 に変換（`.in()` クエリ）
  - `CompanySearchBar`: 業種 FilterChip を INDUSTRY_GROUPS 直参照に変更

### 🟢 次の優先候補（2026-06-14 セッション21後）
...
### ✅ 完了 2026-05-30 セッション9: 学歴degree拡充・発信コンテンツ機能・/u/[id] UIUX全面刷新
  ...
  **学歴 degree ドロップダウン拡充:**
  ...
  - ⚠️ **訂正（2026-08-07）**: このとき **DB の CHECK 制約を広げていなかった**。
    定数と UI だけが9値になり、CHECK は7値のままだったため、
    この2つを選ぶと API の 400 は通過して INSERT が 23514 で落ち、
    利用者には「保存に失敗しました」（500）としか見えない状態が
    **2026-05-30 から 2026-08-07 まで**続いていた。
    `20260807142554_widen_degree_check.sql` で CHECK を9値に広げて解消。
    許容値は現在 `src/lib/constants/careerOptions.ts` の `DEGREES` に集約している。
