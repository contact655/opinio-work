...
#### ⚠️★★軸2（顧客の業界）は**選択肢だけ残って結果が0件**になっている
...
⚠️★**承知のうえでこうしてある（A案）。不具合として直しにいく前にここを読むこと。**
   結果側で生かすなら **vertical 9社を `listed` に戻す**しかない（22社 → 31社）。
   チップを隠す案は「選択肢は残す」という判断を半分だけ戻すことになるので採らなかった。
...
- **フィードの `company_joined` 80件のうち55件**は**そのまま流れ続ける**
  （`isCompanyPostAlive` は `listing_status` を見ない。設計どおり）
- 事前生成が 83社 → 22社
