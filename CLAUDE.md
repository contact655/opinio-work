...
## ⚠️ スカウト送信は停止中（2026-08-09 停止）

**受け取る手段が無いため、送信を止めてある。再開は `SCOUT_SENDING_ENABLED=true`。**

⚠️ **フラグを true にするだけで再開しないこと。** 受信側が無いままなので、
   届かない状態が復活する。下の「塞ぐなら」を先に済ませること。

### 止め方（2箇所。片方だけ変えない）

| 場所 | 内容 |
|---|---|
| `POST /api/biz/scouts` | フラグを見て **503** を返す（最初のガード。認証より前） |
| `/biz/candidates` | 送信ボタンを出さず「スカウト準備中」表示＋上部に案内バナー |

⚠️ **APIだけ止めるのは不十分。** ボタンが残っていると企業は押せてしまい、
   503 を「失敗した」と受け取る。ページ側も同じフラグで出し分けること。

⚠️ 候補者検索そのものは止めていない。使える機能まで巻き込まない。

### 止める前の状態（記録）

**企業は送信でき、行も作られるが、求職者がそれを知る方法が1つも無かった。**

| | 状態 |
|---|---|
| 送信UI | `/biz/candidates` の「スカウトを送る」 → `POST /api/biz/scouts` |
| 送信の実体 | `ow_scouts` に INSERT **のみ** |
| メール通知 | **無し**（`sendEmail` の呼び出しが0箇所） |
| アプリ内通知 | **無し**（`ow_notifications` への書き込みが0箇所） |
| 求職者側の閲覧UI | **無し**（`ow_scouts` を読む求職者側ファイル3件はすべてコメントのみ） |
| 返信API | `/api/jobseeker/scouts/[id]/reply` は**存在するが、呼ぶ画面が無い** |

### なぜ今は表面化していないか

| | 2026-08-09 時点 |
|---|---|
| 送信できる企業（`is_published`） | **76社** |
| 受信対象の求職者（`scout_enabled = true`） | 3人 |
| 実際に送られたスカウト | **0件** |

⚠️ **まだ誰も送っていないだけ。** ガードは `ctx.isPublished`（運営審査完了）だけで、
公開76社は**今すぐ送れる**。1件送られた時点で、送った企業には
「送信済み」と表示され、受け手には何も届かない状態になる。

### LP は「届く」と書いている

⚠️ LP の FAQ に **「登録すると、スカウトが届きますか？」→「初期設定は『受け取る』」**
という項目がある（`LandingPage.tsx`）。`(jobseeker)/layout.tsx` の
meta description からも「スカウトなし」の記述を
**「事実と異なる」という理由で削除済み**。

つまり**利用者には届くと説明しているのに、届ける経路が無い。**
CLAUDE.md の「入力させたのに保存しない UI を作らない」の裏返しにあたる。

### 塞ぐなら、どれか1つでは足りない

1. 求職者側の閲覧画面（`/mypage` にスカウト一覧など）
2. 届いたことを知らせる手段（メール or アプリ内通知）
3. 返信導線を既存の `/api/jobseeker/scouts/[id]/reply` に繋ぐ

⚠️ **1だけ作っても、見に行かない限り気づけない。** 2が要る。
⚠️ メールを足すときは `RESEND_API_KEY` 経由（`lib/notify/email.ts`）。
   週次メールとは別系統なので、あちらの停止フラグの影響は受けない。

⚠️ `/biz/scouts`（スカウト管理）の「返信率」は既に
`readOrMore === 0` のとき `null` になりタブごと出ない実装だったので、
そちらは触っていない。
