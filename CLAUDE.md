...
## オンボーディングの現状（2026-09-14 更新・これが正）

⚠️★**この節は 2026-09-14 まで「単一画面。ステップ分岐は無い」と書かれたままだった。**
   実装は 2026-09-11 に3ステップ、2026-09-14 に4ステップになっており、**記録が3日遅れていた。**
   **画面を足したら、同じコミットでここを直すこと。**

### 画面構成：**4ステップ**（`?step=` は URL に置く）

⚠️★**番号を直書きしないこと。** `OnboardingClient` の
   `STEP = { YOU: 1, COMPANY: 2, STANCE: 3, OPTIONAL: 4 }` を使う。
...
⚠️★**API を直叩きして「保存できた」で済ませないこと。** 2026-09-14 に4ステップへ
   変えたとき、step 1 の保存は API で確かめたが**画面を1→2→3→4 と通していなかった。**
   詰まりはその間にあった。
...
| 2 | 直近のお勤め先 | 会社・部署名・職種・社内での呼び方・入社年月 | 出す |
| 3 | 転職について | `career_stance` ＋ 関心のある職種 | 元から出さない |
| 4 | あとは任意 | 勤務地・勤務形態・これまでの職歴・学歴 | 出す |
