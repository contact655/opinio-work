...
- ⚠️★**コード変更が1行も無い migration は、デプロイのきっかけ自体が発生しない**（2026-09-07）。
  ...
  デプロイを走らせること。**
- ⚠️★**そのデプロイ直後も、1回目のリクエストは古い値を返す**（stale-while-revalidate。2026-09-07）。
  **2回目以降が正。** 実測: デプロイ直後の1回目 `?industry=finance` 6件 / `?industry=erp` 3件 →
  **2回目以降 5件 / 4件**。⚠️ **新しいクエリキー**（このとき新設した `?industry=legal`）は
  キャッシュ実体が無いので**1回目から正しい** —— つまり**新しい値だけ見て「反映された」と
  判断すると、既存の値が古いまま残っていることに気づけない。**
  → **本番の確認は必ず2回リクエストしてから判定する。**
