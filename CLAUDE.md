...
### ⚠️★★`getCompanyEmployees` だけは `is_test` を除外していない（2026-09-12 / 意図した例外）

**これは漏れではない。柴さんが残すと判断した例外。**
実装は `26e85303`（除外条件から `is_test` を外した）、判断の確認は 2026-09-12。
**他セッションが「揃っていないから」と勝手に戻さないこと。戻すなら柴さんに確認する。**

| | |
|---|---|
| 目的 | **職種ごとの分割表示と構成バーを実データで動かして確認するため**。実ユーザーだけでは現役社員が3名しかおらず、分割が一度も描かれなかった |
| 戻し方 | **`getCompanyEmployees` の `isSeedRow` に `is_test` の除外を1つ戻すだけ**（`src/lib/supabase/queries.ts`）。他に触る場所は無い |

⚠️★**影響は「見えている」こと自体。** ログインした**一般の利用者**に、実在企業のページで
   検証用アカウントが**現役社員・OB/OG として並ぶ**。
   実測（2026-09-12 / 本番 セールスフォース）: **現役10名のうち7名**、**OB・OG 2名のうち1名**が検証用。

⚠️★**未ログインにも件数だけは届く。** カードは0枚のまま（検証用は全員 `login_only`）だが、
   「**ログインすると N名のプロフィールが見られます**」の N は**検証用を含んだ数**。
   実測: 現役 **10名**（実ユーザーは3名）／ OB・OG **2名**（実ユーザーは1名）。
   ⚠️ この点は `26e85303` のコミットメッセージに書かれていない。**ここが唯一の記録。**

⚠️ **例外はこの1関数だけ。** 次の3つは**従来どおり除外している**（2026-09-12 実測）。
   - 「この会社の話を聞ける人」（`getPublicAmbassadors`）… 検証用0名
   - `/people` … 8名すべて非検証
   - 他46箇所（`/companies` のカードの人数・求人の経験者など）
