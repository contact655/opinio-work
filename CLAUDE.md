...
| `/companies/[id]/casual-meeting` | 60秒 | ✅ 効く | — |
| **`/companies/[id]`** | 60秒 | ❌ **毎回動的** | `auth.getUser()` |
| **`/jobs/[id]`** | 60秒 | ❌ **毎回動的** | `createClient()` |
| **`/articles/[slug]`** | 300秒 | ❌ **毎回動的** | `getArticlesBySlugs` の中の `createClient()` |
...
動的化を外すには、閲覧者ごとに変わる値をサーバーから追い出すしかない。
⚠️ ただし **`/companies/[id]` は社員一覧を `isAuthenticated` で出し分けている**。
安易にキャッシュすると**非ログインに公開範囲外の社員情報を配る**。
可視性が絡むページは、外す前に「未ログインと ログイン中で何が違うか」を
必ず実測してから設計すること（→ 下の「現在の鮮度設定」の注記）。
