...
⚠️★**`ops_reviewed_at` は本番で0件。掲載中5名は全員「未確認」のまま**（2026-08-30 実測）。
   最も古い行は **2026-07-19 からON**。**仕組みはあるが、誰も見ていない。**
   ⚠️ 「見る人と頻度が未決」は書式の問題ではなく、**実際に運用されていない**という意味。
      なりすまし対策がこの一覧しか無い以上、決めるまでは**対策が無いのと同じ。**
