...
⚠️★**「業種特化」（事業領域 `vertical`）は軸2と役割が重なっている。**
   該当2社はどちらも `is_primary = false` で、カードには別の事業領域が出ている。
   **整理するなら軸2に寄せて事業領域から外す**が、`?industry=vertical` の被リンクが
   切れるので**未着手**。触るときはリダイレクトを用意すること。
