...
| **`/companies/[id]`** | 60秒 | ❌ **毎回動的** | `auth.getUser()`（下記の理由で据え置き） |
...
効かない `/companies/[id]` は **0.41〜3.11秒**だった。

#### `/companies/[id]` を据え置いている理由（2026-08-09）

**社員一覧を `isAuthenticated` で出し分けている。**

```js
isAuthenticated ? emps : emps.filter((e) => e.visibility === "public")
```

⚠️ 安易にキャッシュすると**非ログインに公開範囲外の社員情報を配る**。

実測（`/companies/salesforce`）:
...
つまり**SEO流入には社員が1人も表示されていない**。これは性能ではなく
プロダクトの論点なので、直す前に「誰に何を見せたいか」を決めること。

外すには社員セクション（約1,164行）＋依存部品をクライアントに移し、
可視性を担保した専用APIを作る必要がある。⚠️ ただし上の
「認証は安くない」の数字より、**ログイン中の人には社員が出るのが
今より遅くなる**（0.367秒 → 0.07秒 + 認証API 約0.4秒）。
未ログインには純粋な改善だが、一長一短であることを踏まえて判断すること。
