...
⚠️ 並びは「よく選ばれる」（`COMMON_PREFECTURES`）を先頭に出す。
   **語彙は [lib/utils/location.ts](src/lib/utils/location.ts) が唯一の出どころ。**
   職歴エディタとオンボーディングの `<optgroup>` が同じ定数を見ている。
   ⚠️ 2つ目の見出しは**「その他」**。一覧に出るのは**企業がある都道府県だけ**なので、
      47件から選ぶ入力欄（職歴・オンボーディング）の「すべての都道府県」とは意味が違う。

⚠️ **`/jobs` の都道府県は別実装だが、こちらは壊れていない**（選択肢も絞り込みも
   `extractPrefecture(job.location)` の1つから作っている）。
   ✅ **グループ分けは 2026-09-06 に `/jobs` にも入れた**（ピルとサイドバー「勤務地」の2箇所）。

⚠️★**分け方は [lib/utils/location.ts](src/lib/utils/location.ts) の `groupPrefectures()` だけ。**
   **3箇所が同じ関数を使う**（企業の一覧 / 求人のピル / 求人のサイドバー）。
   呼び出し側で `COMMON_PREFECTURES` を展開し直さないこと。並びが割れる。

⚠️ **1グループにしかならないときは見出しを付けない**（`group` が null で返る）。
   見出しは2つを見分けるためにあるので、1つしかないなら邪魔なだけ。
   **公開求人はいま東京都だけなので、`/jobs` では実際に見出しが出ない。**
   「実装したのに出ない」ではなく、それが正しい状態。
