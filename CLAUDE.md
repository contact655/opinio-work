...
## ⚠️ スカウトは送れるが、受け取る手段が無い（2026-08-09 確認）

**企業は送信でき、行も作られるが、求職者がそれを知る方法が1つも無い。**
...
**送信をできなくするほうが安全。** 送れるのに届かない状態は、
企業にも求職者にも嘘をつくことになる。`POST /api/biz/scouts` の
`isPublished` ガードの隣に、明示的な停止フラグを置くのが最小の対処。
