...
⚠️★**「非公開企業」という語は、このファイルの中で2つの意味で使われている。**
   ①本人が社名を伏せた（`MASKED_COMPANY_LABEL`）と、②`is_published = false` の企業。
   **①は描画されなくなったので、以後この語は②の意味だけで使うこと。**
   ⚠️ ④非上場（`phase = non_listed`）とも紛らわしい。**新しく書くときは
      「非掲載」「ページ非公開」のように、どの軸かが分かる語にする。**
