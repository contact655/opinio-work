...
**ページ非公開の企業**（`is_published = false`）の挙動を確認する場合、dev の結果は本番と一致しない。

同様に `login_only` など visibility 系のフィルタも
dev と本番で挙動が異なる場合がある。
**「dev で見えた = 本番で同じ挙動」と判断しないこと。**

### ⚠️★ローカルの `npm run dev` / `next start` は**本番 Supabase に繋がる**（2026-09-07 追記）

**「ローカルだから安全」ではない。** 環境が分かれているのはコードだけで、DB と Storage は本番と同一。
書き込み系（保存/♡・応募・面談リクエスト・投稿・フォローなど）を画面から試すと、
**本番のデータがその場で変わる。**

→ **試す前に「何をどう戻すか」を決めてから触ること。** 戻し方まで決まっていないなら触らない。

⚠️ **2026-09-07 に実際に踏んだ。** ♡（保存）の動作確認で `ow_bookmarks` に1行入り、
   アプリの DELETE 経路（`DELETE /api/bookmarks`）で戻した。
   元が2行（どちらも 2026-08-03 作成）だったことを SQL で確認してから戻している。
   ⚠️ **`created_at` で「自分が入れた行」を特定できたから戻せた。**
      既存行と見分けが付かない書き込み（既存行の UPDATE など）は**戻せない**ので、
      そちらは事前ダンプ（`./scripts/dump-tables.sh`）が要る。

⚠️ **読み取りだけの確認は自由にしてよい。** この注意は書き込みの話。

### ⚠️★検証でメールを含む経路を通すと、**本番から実際に送信される**（2026-09-18 に踏んだ）

**`.env.local` に `RESEND_API_KEY` が入っているので、ローカルの dev でも本物のメールが飛ぶ。**
`lib/notify/email.ts` の mock 分岐は「**キーが無いとき**」だけで、`NODE_ENV` は見ていない。

⚠️ 2026-09-18 に実際に3通飛ばした（企業からの在籍報告の検証で `contact@opinio.co.jp` 宛。
   本文に「（検証）」が入っていたので実害は無かった）。

→ **メール送信を含む経路を検証で通すときは、`RESEND_API_KEY` を外して mocked で通すこと。**

```bash
RESEND_API_KEY= npm run dev     # ← このプロセスだけキーを空にする
```
...
- ⚠️★**コード変更が1行も無い migration は、デプロイのきっかけ自体が発生しない**（2026-09-07）。
  `?industry=` などの古い値が居座り続ける（実測: DBは erp 3 / finance 6 / ai 12 なのに
  本番は 1 / 7 / 13 のまま）。**データだけの migration でも、ファイルを commit / push して
  デプロイを走らせること。**
- ⚠️★**そのデプロイ直後も、しばらく古い値を返す**（stale-while-revalidate。2026-09-07）。
  ⚠️★**「2回目以降が正」ではない。新しくなるまで数回かかることがある。**
  実測（同じデプロイの中で）: `?industry=infra` は**2回目**で新しくなったのに、
  `?industry=devtools` は**1回目も2回目も古い値（2社）**を返し、**3回目**でようやく 4社になった。
  ⚠️★**キャッシュはクエリキーごとに独立している。** 片方が新しくなっても、
  もう片方は古いままでありうる —— **1本だけ確かめて「反映された」と判断しない。**
  ⚠️ **新しいクエリキー**（新設した `?industry=legal`）はキャッシュ実体が無いので**1回目から正しい**。
  つまり**新しい値だけ見て確認を済ませると、既存の値が古いまま残っていることに気づけない。**
  → **本番の確認は、値が変わるはずの URL を1本ずつ、新しくなるまで数回リクエストして判定する。**
- ⚠️★**さらにフッターの「事業領域から探す」だけは約5分遅れる**（2026-09-07）。
  `getBusinessDomainFacets` が `unstable_cache`（`revalidate: 300` / tag `business-domains`）で、
  migration は `revalidateTag` を呼ばないため。**TTL 切れで自然に復帰する**ので待てばよい。
  → まとめると **「デプロイして、新しくなるまで数回叩いて、5分待てば全部直る」**。
...
### ✅★★`getCompanyEmployees` の `is_test` 除外は**戻した**（2026-09-14）

**2026-09-12 〜 09-14 のあいだだけ、ここは `is_test` を除外していなかった**
（`26e85303`。職種ごとの分割表示と構成バーを実データで動かすため、柴さんが承知のうえで
本番に出した例外）。**2026-09-14 に柴さんの指示で除外を戻し、他の46箇所と同じ扱いに揃えた。**

⚠️★**この1行を外さないこと**（`isSeedRow` の `u?.is_test === true`）。
   外すと「実在企業のページに検証用の人が現役社員として出る」状態に戻る。

#### なぜ戻したか

同日に自社ドメインの検証用アカウント5件（`contact+28` 〜 `contact+32`）を
`is_test = true` にしたところ、**セールスフォースの現役社員11名が全員検証用**になった。
実在企業のページに**検証用アカウントだけ**が並ぶ状態で、例外を残す前提が崩れた。

#### 戻した結果（2026-09-14 実測 / `/api/jobseeker/companies/[id]/employees`）
...
| Box Japan | **1 → 0** | 0 |
| 株式会社Opinio | 0 | **1 → 0** |
| 伊藤忠テクノソリューションズ / 日本HP / 海光電業 / Archi Village | **変わらない**（各1） | 変わらない |

⚠️★**職種ごとの分割表示と構成バーは、実データでは描かれなくなった。**
   例外を入れたときの目的がこれだった。⚠️ **本番のデータを検証用で膨らませて確かめないこと。**
   見た目を確かめるなら **`/dev/preview`** を使う。

⚠️ 未ログインの見え方は前から変わらない（検証用は全員 `login_only` で、呼び出し側が
   未ログインには `visibility === "public"` だけを渡す）。**変わったのは件数の表示。**
   「ログインすると N名のプロフィールが見られます」の N から検証用が消えた。

⚠️ `auth_id` と `is_test` を select から落とさないこと。落とすと `undefined` になり、
   **除外が静かに効かなくなる**（型では気づけない）。

⚠️ **実ユーザーの数え方が 2026-09-14 に変わった。** それまで `@opinio.co.jp` の
   `contact+28`〜`+32` の5件が `is_test = false` で**実ユーザーとして数えられていた**。
   倒した結果、**登録ユーザー 12人 → 7人 ／ 職歴がある人 8人 → 4人**。
   ⚠️ **これより前の日付で「実ユーザー N人」と書いてある記述は、この5件を含んだ数。**

⚠️★**`hiroki.ikuto.placeholder@opinio.co.jp`（生藤 弘樹）は `is_test` にしない。**
   `auth_id` が NULL で、**運営が履歴書から書き起こした実在の人物**
   （`archive/154_add_users_narifuji_komatsu.sql`）。`is_test` は検証用データの印で意味が違う。
   すでに `auth_id IS NULL` の除外（7箇所）で公開面からは外れている。

⚠️ **ISR の静的HTMLには焼かれていない**（2026-09-12 実測）。83社ぶんを検証用26名の氏名で
   総当りして0件。社員セクションはクライアントから取りに行くため。
   ⚠️ 2026-09-14 に掲載は22社になった（⑨）。**事前生成の対象社数が変わっただけで、
      結論（焼かれていない）は変わらない。**

⚠️★**検証のために `is_test` を一時的に false にしないこと。** その瞬間、実在企業のページに
   検証用アカウントが出る（柴さんの指示）。確認は SQL と API の応答で行う。

---

## ⚠️★★Next 14 は GET のルートハンドラを**既定でキャッシュする**（2026-09-20 に踏んだ）

**`export const dynamic` も `revalidate` も書いていない `route.ts` の GET は、
本番でもローカルでも応答が固定される。** DB を変えても API が古い値を返し続ける。

⚠️ **2026-09-20 に `/api/industries` で実際に踏んだ。** 業種の小分類を29件足して
   **DB は53行になったのに、API は古い22行を返し続けた。**
   気づいたのは `Cache-Control: no-cache` を付けたときだけ51行が返ったから。

```bash
# ★確かめ方。**クエリ文字列を変えるだけでは見抜けない**（キャッシュキーが同じ）
curl -s "$URL/api/industries" | jq '.industries | length'                    # 古い値
curl -s -H "Cache-Control: no-cache" "$URL/api/industries" | jq '.industries | length'  # 本当の値
```

⚠️★**これは「ルートキャッシュと Supabase クライアントの判別軸」の4層目。**
   上の節は静的レンダリング / supabase-js の fetch / `unstable_cache` の3層を挙げているが、
   **ルートハンドラのキャッシュはそのどれでもない**（`unstable_cache` を使っていなくても効く）。

⚠️★**マスタを足す migration とセットで疑うこと。** 運営が業種・職種などの
   選択肢を足す日は**コード変更が無い**ので、
   「コード変更が1行も無い migration はデプロイのきっかけ自体が発生しない」と重なり、
   **足したのに画面へ永久に出ない**形になる。

→ 変わったら即出したい選択肢の API には **`export const dynamic = "force-dynamic"`** を書く。
  ⚠️ 秒数（`revalidate`）にしないのは、マスタが変わるのが運営の操作の瞬間だけで、
     キャッシュで守るほどのアクセスが無いため。**外さないこと。**
...
| ~~`ow_threads`~~ | ✅★**2026-09-27 に DROP した**（`20260927080000`）。`ow_messages.thread_id` の相手で、同じく0行・参照0件だった。⚠️★**`ow_casual_meetings` と取り違えないこと。** カジュアル面談の実体はそちらで**現役** |

✅★**この表は空になった**（2026-09-27）。次に「未使用テーブル」を見つけたらここに足す。
⚠️★**落とす前に必ず数えること。** この3つはどれも
   「行数0 ／ `src` からの参照0件 ／ **この表を指す FK 0件** ／ 関数の本文・ビュー0件」
   を実測してから落としている。**FK だけ見て判断しない**（PL/pgSQL の本体は
   Postgres が依存として追跡しない）。
⚠️★**CASCADE を付けないこと。** 指す FK が0件と確かめてあれば素の DROP で通る。
   通らなければ想定外の依存があるということで、黙って壊すより失敗させたい。
⚠️★**トリガー関数は表と一緒に消えない。** その表専用の関数なら同じ migration で落とす
   （`ow_mentor_reservations` で実際に1つ残りかけた）。共有の `set_updated_at` 系を
   巻き込まないよう、**使っているトリガーの数を先に数える。**
...
3. 到達できるなら、**受け取る先があるか**を確かめる
   ⚠️★**「`getCompanyNotificationRecipients` が0件なら届かない」は 2026-08-23 以降ありえない**
      （2026-09-15 に訂正）。③の運営フォールバックが入ったので、**必ず `ADMIN_EMAIL` に落ちる。**
      この手順で0件を探しても**永久に見つからない。**
   → **企業側に受け取る人が居るか**を見るなら `filterCompaniesWithOwnRecipients`
     （①notification_emails ／②有効な管理者だけを見る）。
     一覧は `/admin/companies` の「通知の宛先なし N社」。
...
### ⑤★都道府県フィルタ —— 選択肢を作る表と絞り込む表を別々に持たない（2026-09-06）

**選択肢の8つのうち5つが、選ぶと必ず0件だった**（福岡県・埼玉県・神奈川県・京都府・広島県）。

原因は [lib/search/companies.ts](src/lib/search/companies.ts) に**逆向きの対応表が2つ**あり、
同期していなかったこと。

| 表 | 役割 | 中身 |
|---|---|---|
| `BRANCH_TO_PREF` | **選択肢を作る**（`fetchDistinctLocations`） | 12件（大阪→大阪府 … 名古屋→愛知県） |
| `PREF_TO_BRANCH_KEYS` | **絞り込む**（`searchCompanies`） | **手書きで愛知県の1件だけ** |

`branch_locations` に「福岡」があるので選択肢には出るが、絞り込みは
`location ilike '%福岡県%'` に落ちて0件になっていた（本社が福岡の企業は0社）。

→ **`PREF_TO_BRANCH_KEYS` は `BRANCH_TO_PREF` から導出する。** 手で書かない。
   実測（修正後）: 東京都78 / 大阪府30（1→30）/ 愛知県16 / 福岡県16（0→16）/
   埼玉県・神奈川県・京都府・広島県 各1（すべて0→1）。**0件の選択肢は無くなった。**

⚠️ **都道府県フィルタは本社だけでなく支社も拾う。** だから 大阪府 が 1社 → 30社 に増えた。
   これは意図した意味（選択肢を支社から作っている以上、絞り込みも支社を見ないと辻褄が合わない）。

### ⑥★★都道府県・フェーズ・事業領域は「0件の選択肢を出さない」の**例外**（2026-09-06 / 2026-09-14）

**どちらもマスタ全件を出す。** 該当が0社・0件のものも出す。

| | 出すもの | 出どころ |
|---|---|---|
| 都道府県 | **47件すべて** | `PREFECTURE_FILTER_GROUPS`（lib/utils/location.ts） |
| フェーズ | **13件すべて**（親3 + 子10） | `PHASE_OPTIONS`（lib/constants/phase.ts） |
| **事業領域**（`/companies` のチップだけ） | **マスタ全件**（有効14件） | **`getBusinessDomainOptions()`**（2026-09-14 追加） |

⚠️★**リポジトリの原則（0件の選択肢を出さない）とは逆向き。** それでもこうしたのは、
   実データから作ると**歯抜けになって不自然**だったため。

     都道府県 … `/jobs` の選択肢が**「東京都」1つ**になり絞り込みとして成立しない
     フェーズ … **シリーズB と シリーズD以降 はあるのに シード・A・C が無い**という
                段階の梯子になっていた（柴さんの指摘）

#### ⚠️★事業領域を3つ目の例外にした（2026-09-14 / 柴さんの判断。A案）

**この節は 2026-09-14 まで「例外はこの2つだけ。事業領域・職種には広げないこと」だった。**
61社をディレクトリから外したとき（⑨）、`getBusinessDomainFacets()` のままだと
チップが **14件 → 11件**（ハードウェア・半導体／マーケットプレイス／基幹業務システムが消える）に
痩せたため、**チップだけ**マスタ全件に切り替えた。

⚠️★**広げたのは `/companies` のチップ1箇所だけ。** `getBusinessDomainFacets()`（0社を返さない）は
   **そのまま**で、次の4つは今も0件を出さない:
     sitemap の `?industry=` URL ／ LP のファセット ／ フッター「事業領域から探す」 ／
     `companies/(list)/page.tsx` の `facetForMetadata`（`?industry=` 単独時の title/canonical）
   **「中身の無いページを自分から知らせない」は変えていない。**

⚠️★**職種には広げないこと。** 例外は 都道府県 / フェーズ / 事業領域チップ の3つだけ。

⚠️ 2026-08-25 に「ITサービス・受託」が該当0社のまま出続け、押すと必ず0件だったのを
   直した経緯がある。**その修正を戻したわけではない**（あちらは sitemap と LP を含む話で、
   今回はチップだけ）。

実測（2026-09-14 / 本番）: チップ **14件** ／ フッター **11件** ／
sitemap の `?industry=` **11本**。

⚠️ **`fetchAvailablePhases` と `availablePhaseOptions` は削除した**（2026-09-06）。
   DB を引いて選択肢を絞る必要が無くなったため（`/companies` のクエリが1本減った）。

⚠️★**選択肢は [lib/utils/location.ts](src/lib/utils/location.ts) の
   `PREFECTURE_FILTER_GROUPS` だけ。3箇所が同じものを使う**
   （企業の一覧 / 求人のピル / 求人のサイドバー「勤務地」）。
   呼び出し側で `COMMON_PREFECTURES` を展開し直さないこと。並びが割れる。
   見出しは入力欄の `<optgroup>` と同じ文言（「よく選ばれる」「すべての都道府県」）。

⚠️ **`fetchDistinctLocations` は削除した。** 実データを引く必要が無くなったため。
   ⚠️ ただし **`BRANCH_TO_PREF` は残す。絞り込み側（`PREF_TO_BRANCH_KEYS`）が使う。**
      支社しかない県（福岡・広島など）を選んだときに拾うのはこの表。

⚠️ **`/jobs` のサイドバー「勤務地」にあった「2県以上あるときだけ出す」ゲートも外した。**
   ピル側には元からそのゲートが無く、**同じ画面で食い違っていた。**

実測（2026-09-06 / 修正後）: 選択肢は両画面とも47件＋見出し2つ。
`/companies` は 東京都78 / 大阪府30 / 愛知県16 / 福岡県16 / 広島県1 …、
沖縄県・青森県などは0社（**それが正しい状態**）。
47件あるので `/companies` のドロップダウンには絞り込み入力（`searchable`）を付けた。

⚠️★**`/jobs` の他のピルは 0件の選択肢を出したままにしてある**（2026-09-06 実測）。
   事業領域は**15件出るのに求人は2件**（AI・データ → 0件）、勤務形態は
   フルリモート → 0件、雇用形態は業務委託 → 0件。
   **柴さんの判断で今回は触っていない。** 直すなら `/companies` の
   `getBusinessDomainFacets()` と同じ形にする。
...
### ⑦★カードは**同タブ遷移**。`target="_blank"` に戻さないこと（2026-09-07）

遷移先とタブの扱いは **`CompanyCardList` の `openInNewTab`（既定 `false` ＝ 同タブ）**。
`target` / `rel` を直書きしない。**新しい呼び出し元を足すときは明示的に渡すこと**
（現在の4系統はすべて `openInNewTab={false}` を明示している）。

⚠️★**以前は `target="_blank"` が2箇所に直書きされていた**（compact / list）。外した理由:
   ① **`<Link>` の prefetch は既定（AUTO）で発火しているのに、取得した RSC が
      1バイトも使われていなかった。** 別タブは必ず新規ドキュメント読み込みになるため。
      実測（2026-09-07 / 本番）: ビューポート内 **15枚ぶん**を取得して捨てていた。
   ② **同じ画面で挙動が割れていた。** `?view=list` のカード面は別タブ、同じ行の
      「詳細 →」ボタンは同タブ（`router.push`）、一覧下部の「最近見た企業」も同タブ。
      **行き先はすべて同じ URL。**

実測（2026-09-07 / 本番 opinio.jp / `MutationObserver` でクリック→描画）:

| 条件 | クリック→描画 | 新規ドキュメント取得 |
|---|---|---|
| prefetch 済み | **99 / 121ms** | **0件**（RSC の追加取得も0） |
| prefetch 未達 ＋ `x-vercel-cache: STALE` | **202ms** | 0件（RSC 1本・93ms） |
| prefetch 未達 ＋ `x-vercel-cache: MISS` | **890 / 1,152ms** | 0件（RSC 1本・781 / 1,003ms） |

⚠️★**`<div onClick>` にしないこと。`<Link>` のままにしてある。** ⌘クリック・中クリック・
   右クリック→新しいタブが使える（別タブで見比べたい人の逃げ道が残る）。Next の `Link` は
   `isModifiedEvent` で `preventDefault` せずブラウザに任せる。加えて ♡ の
   `preventDefault()` が意味を持ち続ける（`<div>` だと `stopPropagation` だけが頼りになる）。

⚠️ **戻るは Router Cache から復元され、一覧を再取得しない**（実測: すぐ戻る 79ms /
   65秒放置後 115ms、どちらも一覧の再取得 **0件**）。`?page=2` / `?view=list` /
...
⚠️★**`/companies` と `/jobs` の両方**が同じ部品を使う（2026-09-09 に求人へ広げた）。

| | 実体 |
|---|---|
| クリックの横取り | [CompanySplitLinks](src/components/companies/CompanySplitLinks.tsx)（上から拾って `preventDefault` するだけ）。`basePath` で `/companies` と `/jobs` を切り替える |
| 骨組みと CSS | **[CompanySplitLayout](src/components/companies/CompanySplitLayout.tsx)** |
| 右ペインの中身 | **[CompanyPane](src/components/companies/CompanyPane.tsx)** / **[JobPane](src/components/jobs/JobPane.tsx)**（どちらも要約。**詳細ページを使い回さない**） |
| しきい値 | [lib/constants/splitView.ts](src/lib/constants/splitView.ts) の `SPLIT_MIN_WIDTH` |
| レール幅 | 一覧グリッド **420** ／ 企業の詳細表示・求人 **700**（横長の行は固定部分だけで 499px 要る） |

⚠️★**部品名が `Company…` なのは歴史的な理由。** `/jobs` から使うのは 2026-09-09 から。
   改名すると CSS の class 名（`companies-split` / `companies-pane`）や本ファイルの
   記述まで芋づるで、**取りこぼすと片方の画面でだけレールが潰れる**。
   同じ形の前例がある（`?industry=` は事業領域、`--font-noto` はシステムフォント）。
   **名前より「1箇所であること」を優先している。**

⚠️★**ペインの CTA はヘッダーの中に置く。** 末尾に置くと、内容の厚い企業・求人では
   `max-height` ＋ `overflow-y: auto` の**内部スクロールの奥**に入り、
   ページをスクロールしても出てこない（実測 2026-09-09 / Salesforce:
   ペイン 730px に対し中身 907px、CTA は y=1154 のまま動かない）。
   ⚠️ **薄い企業では末尾でも見えるので、1社だけ見て判断すると同じ形に戻る。**

⚠️ **レールの1列化 CSS はグリッドの class を名指ししている**
   （`companies-grid4` / `search-results-grid`。求人は元から縦1列なので行間だけ）。
   **4つ目のグリッドを載せるときはそこに足すこと。**

⚠️★**カードの `target="_blank"` は分割ビューと両立しない。** `CompanySplitLinks` は
   `target !== "_self"` のアンカーを**素通しする**（別のリンクを横取りしないため）ので、
   `_blank` のままだとペインが開かない。`JobListItem` は 2026-09-09 にこの理由で外した
   （企業カードは 2026-09-07 に prefetch の無駄を理由に外していて、結果的に同じ形）。
...
⚠️★**選択の判定は `?selected=` の値を id と slug の両方と突き合わせる**（2026-09-18 に変更）。
   ⚠️ **この記述は「解決済みの id で比べる」だった。** それだとページ先頭で
      `getCompanyBySlugOrId` を await する必要があり、**スケルトンが出せない**
      （await するとページ全体が待ち、新しい RSC が届くまで前の企業の内容が残る）。
      両方と比べれば解決が要らない。**uuid で直リンクしても印が付くことを実測で確認済み。**

⚠️★**この節の「`?view=list` は対象外」は誤りだった**（2026-09-18 に実測して訂正）。
   実装は `companies/(list)/page.tsx` が `?view=list` にも `CompanySplitLayout`
   （`railWidth={LIST_RAIL_WIDTH}`）を付けている。**記述が実装に追いついていなかった。**

### ★スクロールの持ち主（`scrollMode`）── 3経路セットで変える（2026-09-18）

| 値 | 形 |
|---|---|
| `"page"` | ペインが sticky。**ページ全体がスクロールする** |
| `"panes"` | レールとペインが**それぞれ独立してスクロールする** |

**`/companies` は3経路とも `scrollMode="panes"`**（一覧グリッド / `?view=list` / 絞り込み結果）。

⚠️★**1つだけ外さないこと。** 3経路は**同じ画面**で、ビュートグルや絞り込みで行き来する。
   片方だけ `"page"` にすると**同じ画面でスクロールの挙動が割れる**
   （⑦の `target="_blank"` と同じ形）。
⚠️ `CompanySplitLayout` の JSDoc は「`/companies` の3経路は既定のまま」と書いてあったが、
   2026-09-18 に3つとも明示的に `"panes"` を渡す形にした。**既定値（`"page"`）は変えていない。**
⚠️ `--split-top` は `/companies` からは渡していない（ツールバーの高さがほぼ一定で、
   部品側の既定 162px と実測がほぼ一致する）。`/jobs` は詳細検索の開閉で高さが変わるので、
   **あちらだけ `ResizeObserver` で測っている。**
...
### ⑨★★掲載企業を 83社 → 22社 にした（2026-09-14 / 柴さんの指示）

**61社を `listing_status = 'draft'` にして、ディレクトリ（一覧・検索・サジェスト・
sitemap・LP）から外した。** migration は `20260914160000_unlist_61_companies.sql`。

⚠️★**`is_published` は true のまま。企業ページは残してある。**
   ＝ **職歴の企業ピッカーからは今も選べる**（それが目的）。実測して決めた:

| 倒すと壊れるもの | 根拠 |
|---|---|
| 職歴のリンク | `lib/utils/timeline.ts` は **`is_published` だけ**を見る（2件が該当） |
| ♡ ブックマーク | `/mypage/bookmarks` は可視性を**一切見ていない** → 404 へのリンクが残る |
| 記事の会社CTA | `resolvePublishedCompanyHref` は `filterVisibleCompaniesStrict`（6社が該当） |

⚠️ この状態は新設ではない。**既に5社**が同じ形で動いていた
   （アサヒビール／スマートキャンプ／みずほ証券／株式会社エージェント／海光電業）。

⚠️ `trg_guard_company_approval` が止めるのは**掲載を立てる方向だけ**。`'draft'` へ倒すのは通る。
   61社とも `is_approved = true` なので**戻せる**（`listing_status = 'listed'` を当てるだけ）。

#### ✅★★軸2（顧客の業界）は**選択肢だけ残って結果が0件**。**このままにすると決めた**

`vertical` の企業10社のうち **9社が対象に入った**（建設4社・Ubie・nCino・クアルコム・
ミラクル・ゲインサイト）。残るのは自社（Opinio）だけで、それは常に除外される。

| どこ | いまの状態 |
|---|---|
| `/companies` の「顧客の業界」チップ | **6件出るが、押すと5件が0社**。IT・ソフトウェアだけ1社（自社） |
| **`/mypage` の「◯◯の経験が活きる会社」** | **母集団0 → どのユーザーにもブロックが出ない** |

✅★**2026-09-14 に柴さんが「このままでよい」と確認。決着済み。**
   **不具合として直しにいかないこと。** 直すとしたら **vertical 9社を `listed` に戻す**
   （22社 → 31社）だが、**それは今回の指示（61社を一覧から外す）を部分的に取り消すこと**になる。
   ⚠️ チップを隠す案も採らない。「選択肢は残す」という判断を半分だけ戻す形になる。

⚠️★**放っておけば直る。条件は「掲載中の vertical 企業が2社以上になること」だけ。**
   選択肢（`fetchAvailableTargetIndustries`）は**ページのある企業**から作るが、
   絞り込みの結果（`searchCompanies`）と `/mypage` のブロック（`industryMatch`）は
   **掲載中だけ**を見る。だから **`listing_status` を `'listed'` に戻した社が2社出た日に、
   コードを1行も触らずに復活する**（`MIN_COMPANIES_PER_BLOCK = 2`）。
   ⚠️ **そのとき「なぜか急に出はじめた」と驚かないこと。** 仕様どおり。

⚠️ 実害が今すぐ出ているわけではない。**この変更の前から**、実ユーザー7人の業種
   （IT・ソフトウェア / 金融・保険 / 小売・流通 / 商社・卸売）では該当 vertical が
   各1社しかなく、`MIN_COMPANIES_PER_BLOCK = 2` を満たさず**0ブロックだった**。
   失われたのは「建設出身者が登録した日に出るはずだったもの」。

#### そのほか、この変更で静かに変わったもの

- ✅★**フィードの `company_joined` 80件のうち55件**は**そのまま流れ続ける。**
  **2026-09-14 に柴さんが「このままでよい」と確認。決着済み。**
  `isCompanyPostAlive` は `is_published` だけを見て `listing_status` を見ない
  （`lib/feed/visibility.ts` に理由がある: ディレクトリから外しただけの企業は
  ページが生きているので、告知を消す理由が無い）。
  ⚠️★**一覧には無いのにフィードでは「参加しました」が流れる**——
     食い違って見えるが**リンクは 200 で、行き止まりにはならない。**
     **`isCompanyPostAlive` に `listing_status` を足さないこと。**
- 事前生成が 83社 → 22社

#### ✅★企業名を引く経路は3つ。**軸を取り違えない**（2026-09-14 に整理）

**61社を非掲載にしたことで、「ディレクトリの軸」を「マスタの軸」の用途に使っていた
2箇所が表に出た。** どちらも同日に直した。

| エンドポイント | 軸 | 認証 | 誰が使うか |
|---|---|---|---|
| **`/api/search/suggest`** | **ディレクトリ**（`filterListedCompanies`） | 不要 | ヘッダーの検索サジェスト |
| **`/api/companies/lookup`** | **マスタ**（`is_test=false` のみ） | **必須** | 職歴の企業ピッカー ／ **スカウトのブロック**（2026-09-14〜） |
| **`/api/companies/search`** | **マスタ**（2026-09-14 に変更） | **必須**（同日に追加） | **`/biz/companies/add/new` だけ** |

⚠️★**`/api/companies/search` は「ディレクトリの軸」ではない**（2026-09-14 に変えた）。
   答えるのは「**この会社はマスタに既にあるか**」。掲載で絞ると
   **非掲載企業の重複を検出できず、重複行ができる。**

⚠️★**CLAUDE.md の旧記述は誤りだった。** 「ヘッダーのサジェスト・`/biz` の重複チェック・
   スカウトのブロック設定が乗っている」と書かれていたが、**ヘッダーは
   `/api/search/suggest` という別ルート**（2026-09-14 実測）。
   **「何が乗っているか」は grep で数え直してから書くこと。**

⚠️ `/api/companies/search` は 2026-09-14 まで**未認証で叩ける公開エンドポイント**だった。
   非掲載の企業まで返すようにしたので認証を足した（「掲載していない」という状態そのものが
   運営の情報。`lookup` と同じ理由）。
   ⚠️ **`/biz` ロールでは絞れない。** `company` ロールは存在せず、初めて企業を作る人は
      `ow_company_admins` の行をまだ持たない。**ログイン必須が上限。**

⚠️★**求職者に見せる一覧・検索・サジェストを新しく作るときは `filterListedCompanies`。**
   「この会社は存在するか」を聞く画面（ピッカー・重複チェック・ブロック）は**マスタの軸**。

⚠️ ブロック一覧の社名は **`companyDisplayName` に畳んでから返す**
   （`GET /api/jobseeker/scout-settings`）。`lookup` が表示名（「Adobe」）を返すので、
   生の `ow_companies.name`（「アドビ株式会社」）のままだと**同じ画面で食い違う。**

実測（2026-09-14 / 本番 opinio.jp）: 掲載 **22社** ／ 事業領域チップ **14件** ／
顧客の業界チップ **6件** ／ フッター **11件** ／ sitemap 企業URL **22本**・`?industry=` **11本** ／
`/companies/akamai` は **200 かつ noindex**（ページは生きている）／ LP「掲載企業22社」。

---

## ⚠️★`/people` の絞り込み（2026-09-18）

上部の並びは `/companies` と同じ（検索窓 / 詳細検索 / 並び替え / 一覧・詳細 / 人数）。
条件は**職種・年代・外資・面談OK**の4つで、**すべて詳細検索の中**にある。

### ★`?view=` を関係の絞り込みに使わないこと。関係は `?rel=`

| パラメータ | 意味 |
|---|---|
| `?rel=` | **関係**（`following` / `followers` / `coworkers`） |
| `?role=` / `?age=` | 複数選択（カンマ区切り）。**同じ項目の中は OR** |
| `?foreign=1` / `?meeting=1` | ON/OFF |
| 表示形式（grid / list） | ★**URL に入れていない。localStorage のまま** |

⚠️★**`?view=following` の形にしないこと。** `/companies?view=list` は**表示形式**の意味で、
   同じ語が隣り合うページで2つの意味を持つ。`?industry=` がラベルと食い違っているのと
   同じ形を、**新しく増やさない**ための判断。
⚠️ 表示形式を URL に入れていないのは、あれが**人ごとの好み**で共有したい状態ではないため。
   ⚠️ 入れると `?view=` が復活して上の衝突が戻る。

⚠️ **項目どうしは AND、項目の中は OR**（`/companies` と同じ約束）。
⚠️★**値を持たない人は、その項目で絞ったときだけ落ちる。** 現職なし → 職種のときだけ／
   生年月日なし → 年代のときだけ／マスタ紐付きの職歴なし → 外資のときだけ。
   **他の条件では落とさない。**（2026-08-20 に撤去した年齢フィルタが
   「生年月日が未入力の人を無条件に落としていた」のと同じ形を作らない。）

⚠️ **「同じ会社にいた人」は `company_id` の集合で突き合わせる。** 社名の文字列で比べないこと
   —— 表記ゆれで別会社を同じと見なすし、本人が伏せた社名を復元することにもなる。
   ⚠️ `DirectoryPerson.companyIds` は**画面に出さない**（突き合わせ専用）。

### ★`FilterChip` は [components/common/FilterChip.tsx](src/components/common/FilterChip.tsx) の1箇所

⚠️★**同じ名前の別実装を作らないこと。** 2026-09-18 まで `CompanySearchBar.tsx` と
   `PeopleListClient.tsx` に**別々の `FilterChip` が2つ**あり、押したときの見た目が
   少しずつ違っていた。`/companies` 側を**そのまま**切り出したので、あちらの見た目は変えていない。

⚠️ 複数選択（`values` / `onToggleValue`）は**追加の口**。単一選択（`value` / `onSelect`）は
   そのまま動く。**片方に寄せないこと** —— `/companies` は全チップが単一選択で、
   複数選択にすると URL の形まで変わる。

⚠️ `/people` の `.ppl-chip`（詳細検索・外資・面談OK）は `FilterChip` と**同じ行に並ぶ**ので、
   active の見た目を揃えてある（`font-weight: 600` / box-shadow なし）。**片方だけ戻さないこと。**

⚠️★**選択中の条件チップ（`ActiveChip`）を消さないこと。** 詳細検索を畳んだときに、
   いま効いている条件を外に出すためのもの。`/jobs` で 8条件を畳んだときと同じ理由で、
   無いと**絞り込んだ結果を見ている最中に理由が画面から消える**。

### ★左サイドバー

⚠️ **面談OKのトグル本体をサイドバーに作らないこと。** ONは「在籍中の全社ぶんの行を
   作る／有効化する」多段処理で、実体は `components/profile/editor/IntentCard.tsx` の
   `runTalk`。サイドバーは**状態を出して `/mypage` へ送るだけ**。
   ⚠️ `ow_company_members` の行が無い人には**この行ごと出さない**（柴さんの判断）。

⚠️★**%（完成度）を出さないこと。** 完成度バーは 2026-08-16 に廃止、2026-09-10 に
   「%も『あと N 項目』も出さない」と決めてある（docs/profile-progress-20260910.md）。
   サイドバーが出すのは**次の1件だけ**で、割合も件数も出さない。
   ⚠️ その1件に**生年月日を入れないこと。** 文言が「入力すると他のユーザーに経歴が伝わります」
      の方向なので、**他のユーザーに出ない項目**を混ぜると嘘になる。

⚠️ フォロー数は **0 でも「0人」と出す**（柴さんの指示）。
   ⚠️ `lib/people/followCounts.ts` の「0 のときは項目ごと出さない」は
      **`/u/[id]` と `/mypage` のメタ行の話**。ここは別。
   ⚠️ 実測（2026-09-18 / 本番）: `ow_user_follows` も `ow_company_follows` も **0行**。
      今日はどの利用者が押しても 0件 になる。**仕様であって不具合ではない。**

⚠️★**狭い画面で中身ごと消さないこと**（`hidden lg:flex` の形にしない）。899px 未満では
   一覧の上に畳み、**フォロー数と「面談OKだけ見る」は畳みの外**に置いてある。

⚠️ ミニプロフィールは [components/common/ProfileCard.tsx](src/components/common/ProfileCard.tsx)。
   **フィードの左カラムと同じ部品・同じデータ元**（`lib/people/sidebarData.ts`）。
   ⚠️★肩書きは **会社 ／ `ow_roles` 名**。`ow_experiences.role_title`（自由入力）に
      戻さないこと —— それが 2026-09-18 まで「フィードでは AE / `/people` では
      Salesforce／新規事業開発」と食い違っていた原因（**同じ行の別々の列**を読んでいた）。

⚠️ **所属が無い人も一覧に出す**（2026-09-18 に `affiliation.kind !== "none"` の除外を外した）。
   実測: 5人 → **7人**。空欄を「未登録」等で埋めないこと。高さは CSS の行内 stretch で揃う。

---

## ⚠️★`/jobs` の絞り込みは「詳細検索」1つに集約した（2026-09-09）

**条件の置き場を増やさないこと。** それまで**3箇所**に散っていた。

| どこ | 何が入っていたか |
|---|---|
| 上部のピル行 | フェーズ / 事業領域 / 都道府県 / 勤務形態 / 雇用形態 / 年収 / 外資系 |
| 左サイドバー（260px） | **職種**（件数付き）/ **勤務地** / 「検索する」 |
| モバイルの行3 | **職種**クイックピル（上位10件） |

**職種は3箇所、勤務地は都道府県ピルと同じものが2箇所**にあり、しかも勤務地には
「2県以上あるときだけ出す」ゲートがピル側に無く、**同じ画面で食い違っていた。**

→ いまは **「詳細検索」ボタン1つ**。押すと8条件のピルが出る。

⚠️★**閉じていても「選択中の条件」をチップで外に出す**（`activeChips`）。**消さないこと。**
   8条件を畳んだので、これが無いと**絞り込んだ結果を見ている最中に理由が画面から消える**
   （`/companies` で「顧客の業界」を結果側に出したのと同じ理由）。
   ⚠️ チップの ✕ は**近道であって唯一の入口ではない**。パネルを開けば元のピルからも外せる。

⚠️ **職種は親18件のみ。** 子まで出すと148件になり、この幅では選べない
   （2026-08-06 に職歴エディタで「105件を目視で探させる UI は機能していない」と判明済み）。

⚠️ **サイドバーを復活させないこと。** `SidebarFilters` / `SectionHeader` / `CheckItem` /
   `ACTIVE_FILTER` / `isDesktop` は削除した。
   ⚠️ `.jobs-layout` の **220px 2カラム指定を消し忘れると、存在しない列にカードが
      押し込まれて幅 120px まで潰れる**（2026-09-09 に実際に踏んだ）。

⚠️ **職種の色分けを廃止した判断（2026-08-30）はそのまま生きている。** 色を持っていた
   `ACTIVE_FILTER` は使い手が消えたので削除しただけで、方針は変えていない。

⚠️★**「0件の選択肢を出す/出さない」の方針は変えていない。** `/jobs` のピルには
   該当0件の選択肢が今も出る（2026-09-06 実測: 事業領域15件中13件が0件）。
   ⚠️ 直すなら `/companies` の `getBusinessDomainFacets()` と同じ形にする。
      ただし**公開求人は2件しかない**ので、直しても選択肢が2つになるだけ。
...
- **SELECT を列単位で配っているテーブルの一覧**（**2026-08-23 実測に更新**）。
  ...
  |---|---|---|
  | `ow_users` | **29 / 38** | **22 / 38**（2026-08-19〜） |
  | `ow_experiences` | **28 / 37** | **21 / 37** |
  | `ow_career_profiles` | 7 / 9 | **5 / 9**（2026-08-20〜） |
  | **`ow_company_members`** | **12 / 15** | **11 / 15** |

  ⚠️ **2026-09-15 に測り直した**（前回は 2026-08-23）。分母が動いているのは列が増えたため
     —— `ow_users` は 2026-09-14 に本人の属性6列（姓・名・ふりがな・性別・電話）を足した。
     ⚠️★**足した6列は SELECT を配っていない**（UPDATE のみ）ので、分子は増えていない。
     ⚠️ **分母だけ見て「配り忘れ」と読まないこと。** 意図して配っていない列がある。
...
## ⚠️ 企業ページの3つのスイッチ（2026-08-13 確立 / **2026-09-09 に見出しを訂正**）

⚠️★**この節の見出しは「企業ページは作られた時点で見える」だった。実装と食い違っていた**
   ので直した（2026-09-09 / 柴さんの判断 = 実装が正）。実際には
   **承認と分類が済むまでページは見えない。**
...
| `is_published` | **詳細ページが見えるか**（404 ゲート） | **`true`**※※ |
| `is_approved` | 運営が内容を確認した。**公開と一覧掲載の前提条件** | `false` |

※ DB既定は `'listed'` だが、`POST /api/biz/companies` は明示的に `'draft'` を入れる。
※※ DB既定は `true` だが、**`POST /api/biz/companies` は `false` を入れる**。
    つまり利用者・企業が作った企業のページは**最初は 404**。

### ⚠️★`is_published = true` にするには2つ必要（2026-09-09 に実測して判明）

| 何が要求するか | 条件 |
|---|---|
| **DB のトリガー** `trg_guard_company_approval`（BEFORE UPDATE） | **`is_approved = true`**。無いと `42501`「運営の承認が完了するまで公開できません」 |
| **アプリの公開ゲート** `checkPublishable` | **業種（`industry_id`）＋ 主の事業領域** |

⚠️★**「作られた時点で見える」を実装しようとして、この2つに止められた**（2026-09-09）。
   migration で6社を `is_published = true` にしようとしたところ、トリガーが 42501 で拒否。
   ⚠️ **トリガーは `service_role` と `auth_is_admin()` だけ素通しする。**
      `supabase db push` は postgres ロールなので**素通しされない。**
      migration で企業の公開状態を触るときは、この壁を先に確認すること。

⚠️ **結果として、経歴からのリンクは張られない。** `lib/utils/timeline.ts` は
   `isPublished === false` の会社について `company_id` を落として**会社名をテキスト表示**
   にする（404 の行き止まりを作らないため）。
   実測（2026-09-09 / 本番）: 利用者が作った6社は全て `is_published = false` で、
   うち3社は職歴から各1件参照されている（＝リンクにならない）。

⚠️★**「利用者が作った6社」は 2026-09-16 時点では 7社**（`source='user'` **3社** ＋
   `source='biz_self'` **4社**）。**全社 `is_published = false` なのは変わらない。**
   ⚠️ うち **`source='user'` の3社が実ユーザーの職歴から参照されている**
      （3件とも同じ1人。プルデンシャル生命保険・ニトリ・Archi Village）。
      ⚠️★**この3社を公開しない判断を 2026-09-16 に確認した**（柴さん。IT業界に絞る掲載方針と
         合わないため）。**「リンクが死んでいるから公開する」という理由で戻さないこと**
         ——職歴側は `timeline.ts` が**リンクにせずテキストで出す**ので、行き止まりは作っていない
         （2026-09-16 に全画面を実測。`docs/phase0-career-supply-20260916.md`）。

⚠️★**これを変えるなら、トリガーと公開ゲートの両方を変える判断が要る。**
   ゲートは 2026-08-25 に「分類の欠けた企業を公開しない」ために入れたもの。
   **片方だけ外さないこと。**
   ⚠️ 保留にした migration は `supabase/pending/20260909023500_publish_user_created_company_pages.sql`。
      当てるなら**先に6社を承認し、業種と事業領域を入れる**こと。
...
⚠️ **チップのラベルは「事業領域」**（2026-09-06 に「業種」から変更）。`/jobs` と揃えた。
   「業種」は `ow_industries` に使う語で、隣の「対象業界」チップがそちらのマスタ。
...
##### ✅ 「カードの粒度が2社だけ落ちた」は解消した（2026-09-06 に確認）

**この節は「Ubie と nCino のカードは『業種特化』になる」と書かれたまま古くなっていた。**
実際には両社とも**具体的な主の事業領域**が入っており（Ubie = AI・データ /
エヌシーノ = 基幹業務システム）、**カードに「業種特化」は出ていない**（実画面で確認）。

⚠️ **`industry`(text) を読み直して補わないこと。** 廃止する列に戻ることになる。

##### ★軸2（対象業界）を求職者側の絞り込みに出した（2026-09-06）

**「誰に売っているか」で絞れる。**「何を作っているか」＝事業領域とは**別の軸**。

| | 例（アンドパッド） |
|---|---|
| 事業領域（`?industry=`） | プロジェクト管理 |
| **対象業界（`?target=`）** | **建設** |

⚠️★**画面のラベルは「顧客の業界」。内部の呼び名は「対象業界」**（2026-09-06 / 柴さんの判断）。
   ずらしてあるのは意図的:
   - 「対象業界」だと**その会社自身の業界**とも読める。とくに選択肢に「IT・ソフトウェア」が
     あり、実際は「IT企業に売っている会社」の意味なので**まず誤読される。**
   - 「事業領域」と「対象業界」は日常語だとほぼ同義で、**チップを開くまで違いが分からない。**
   ⚠️ DB・URL（`?target=`）・このファイル・`/admin` は**「対象業界」のまま**。
      `?industry=` とラベルが一致していないのと同じ形で、承知のうえ。
   ⚠️ **画面に出る「対象業界」は、このチップ1箇所だけだった**（実測。他はすべてコメント）。
      新しく画面へ出すときは「顧客の業界」に揃えること。

実測（2026-09-06 / 掲載83社）: 建設4 / IT・ソフトウェア2 / 製造業1 / 小売・流通1 /
金融・保険1 / 医療・ヘルスケア1。`target_industry_scope` は **82/83社が設定済み**
（horizontal 70 / vertical 10 / consumer 2 / 未確認 1）。

⚠️★**2026-09-14 にこの数字は崩れた。** 61社をディレクトリから外したので、
   **掲載中の該当企業は 建設0 / IT・ソフトウェア1（自社のみ）/ 他すべて0**。
   選択肢は6件のまま出るが**押すと0件**になる。経緯と判断は ⑨ を読むこと。

⚠️ **選択肢は実データにあるものだけ**（`fetchAvailableTargetIndustries`）。
   「0件でも出す」例外は都道府県とフェーズだけ。
⚠️ **祖先展開しない。** 業種の祖先展開は本人側に掛ける規則で、企業の対象業界は展開しない。
⚠️★**`ow_company_target_industries` から `ow_companies` を埋め込まない**（複合FK）。
   `ow_industries` 側は単純FKなので埋め込んでよい。
⚠️ 事業領域と同時に指定されたら**積集合**。積が空のときは
   **ダミーの id を1件渡す**（空配列を `.in()` に渡すと全件返る）。実測: `ai` × `建設` = 0社。

##### ★2軸は「絞り込み」と「企業ページ」の両方に出す（2026-09-07）

**絞り込みにしか無い軸は、結果を見た人に伝わらない。**
2026-09-06 に「顧客の業界」チップを足した時点では、**企業ページのどこにも
その値が出ていなかった**。`?target=建設` で絞り込んでアンドパッドを開いても、
サイドバーは「事業領域：プロジェクト管理」だけで、**なぜヒットしたかが結果側で
説明されない。** しかも一覧では建設の4社（アンドパッド・ダンドリワーク・
フォトラクション・スパイダープラス）が**全部「プロジェクト管理」タグ**で並ぶ。

| どこ | 何を出すか |
|---|---|
| `/companies` のチップ | 事業領域 ／ 顧客の業界（軸2） |
| **`/companies/[id]` のサイドバー** | **事業領域の「すぐ下」に顧客の業界** |

⚠️★**離して置かないこと。** 2つ並んで初めて
   「**プロジェクト管理を、建設に売っている会社**」と1組で読める。

⚠️★**値が無ければ行ごと出さない。**「業界を問わない」等のフォールバックを書かない。
   0件は `horizontal` / `consumer` / **未確認**のどれでもありうる
   （明細を持てるのは `vertical` だけ ——複合FKで DB が保証している）。
   **区別できないものを断定しない。**

⚠️ 取得は `fetchTargetIndustriesByCompany`（`queries.ts`。`fetchBusinessDomainsByCompany`
   と同じ形）＋ `getCompanyTargetIndustriesCached`。**企業ごとに引かない（N+1）。**
   ⚠️ `is_active = false` の業種は落とす（`/companies` の選択肢と揃える）。

実測（2026-09-07 / dev）: アンドパッド＝プロジェクト管理／**建設**、
Ubie＝AI・データ／**医療・ヘルスケア**、セールスフォース（`horizontal`）＝**行ごと出ない**。

⚠️ **求人詳細（`/jobs/[id]`）には出していない。** あのサイドバーは既に
   「事業領域が2回出る」と注記のある混み具合で、**求人ページは会社ではなく職の説明**。
   足すなら重複の整理とセットで。

##### ★チップの一言は「事業領域」と「顧客の業界」の2つだけに付ける（2026-09-07）

名前が似ていて**開いて中身を見るまで違いが分からない**、という指摘への対処。
`FilterChip` の `hint` に入れる。

| チップ | 一言 |
|---|---|
| 事業領域 | この会社が**何を作っているか** |
| 顧客の業界 | この会社が**誰に売っているか** |

⚠️★**言い回しを対で揃えること**（「この会社が◯◯」）。揃っていないと
   2つが同じ軸の言い換えに見え、**比べられない。片方だけ変えない。**

⚠️★**全チップに付けないこと。** フェーズ・都道府県まで説明が付くと、
   この2つが持つ「**読まないと間違える**」という合図が薄まる。

⚠️ `/jobs` のチップには付けていない。あちらには「顧客の業界」チップが無く、
   **隣に紛らわしいものが無い**ため。

##### ✅★ラベル「事業領域」は変えないと決めた（2026-09-07 / 柴さんの判断）

「製品・サービス領域」「プロダクト領域」を検討したうえで**現状維持**。

| # | 理由 |
|---|---|
| ① | **衝突していたのは相手側で、そこは 2026-09-06 に直した。**「事業」と「顧客」は誰の話かが違うので、並べれば対比が立つ |
| ② | 「製品・サービス領域」は**チップで折り返す**。「プロダクト領域」は**カタカナで意味を運ぶ語が2つ目**になる（既に「フェーズ」がある） |
| ③ | `?industry=` / `ow_business_domains` / `/admin` の「対象業界」で**既に3つずれている**。ずらすたびに「どれの話か」を確かめる手間が全員に乗る |

⚠️★**この判断を覆す条件は1つだけ。** 事業領域の選択肢に**製品カテゴリでないもの**
   （業種や顧客属性に見えるもの）が入り始めたら、「製品・サービス領域」に寄せる。
   **「業種特化」を下げた件（下記）が、その最初の兆候だった。**
   いまの13件は全部が製品カテゴリなので、名前は中身と合っている。

⚠️ 画面に出る「事業領域」は求職者側で**4箇所**（`/companies` チップ・`/jobs` チップ・
   企業詳細サイドバー・フッター「事業領域から探す」）。変えるなら4つ同時。

##### ✅★「業種特化」は事業領域から下げた（2026-09-06 / migration 20260906140000）

軸2と役割が重なり、しかも**何に特化しているかを言えていなかった**ため。
`is_active = false` にしただけで、**行も明細も消していない**（UPDATE 1本で戻せる）。
実測: 事業領域の選択肢が **14 → 13件**。表示テキストの「業種特化」は
`/companies`・`/jobs`・LP とも **0件**。

⚠️★**RLS が `(is_active = true) OR auth_is_admin()` なので、無効化した行は
   anon から丸ごと見えなくなる。** その結果 `?industry=vertical` は **0社**になる
   （当初は「明細が残るので2社返る」と想定していたが、実測して外れていた）。
   許容したのは、**リンク元がもう無い**ため —— 唯一の導線だったフッターは
   `getBusinessDomainFacets()` から動的に組んでおり、同時に消えた。
   ⚠️ `grep "industry=vertical"` では**見つからない**リンクだった。動的生成に注意。

⚠️ 検証用企業（`is_test`）が「業種特化」を**主**にしている。migration のガードが
   これを検知して一度止まった（当初は掲載中だけを見ていて見落としていた）。
   ガードは `is_test` を除く形にして残してある —— **実企業が主にしていたら止める**ため。
...
⚠️ **企業ピッカーだけ業種にしている。** 事業領域は業種によっては任意
   （`requires_business_domain = false`）なので、見分けが目的の画面では欠けない側を選んだ。

   ⚠️★**2026-09-14 に前提が1つ崩れた。** それまでは「`/api/companies/search` は
      掲載中しか返さず、公開ゲートが `industry_id` を必須にしているので**必ず値がある**」
      と書いてあったが、同日このルートを**マスタの軸**に変えたので
      （⑨ の「企業名を引く経路は3つ」）、**非掲載・未公開の企業では `industry` が
      null になりうる。** 受け手は `?? null` で受けており業種の行が出ないだけ。
      ⚠️ **既定値で埋めないこと。**
...
`is_published` で絞る。dev で**ページ非公開の企業**のページを確認できるようにするための分岐で、
**この分岐は変えない**。
...
## ⚠️ 年齢は詳細だけ。一覧に出さない（2026-08-20 確立 / **2026-09-18 に絞り込みだけ例外**）

⚠️★**見出しは 2026-09-18 まで「一覧に出さず、年齢で絞り込ませない」だった。**
   同日に **`/people` にだけ「年代」の絞り込みを入れた**（柴さんの判断）ので直した。
   **「年齢で絞り込ませない」を全画面の規則として読まないこと。** 下の「★`/people` の年代」を読む。
...
⚠️★**「有料職業紹介事業の許可事業者だから一段リスクが高い」とは書かないこと**（2026-09-15 に削除）。
   OPINIO は**ダイレクトリクルーティング**＝職業安定法第4条第6項の**募集情報等提供**で、
   **職業紹介ではない**（`content/legal/terms-of-service-listing.md` 第2条3項）。
   許可番号 13-ユ-316441 は**会社の別サービス**のもの。
   ⚠️★**理由が弱くなったわけではない。むしろ直接効く。** 絞り込むのは**企業（事業主）自身**で、
      その企業に禁止行為の手段を渡すのがこの機能だから。同じ理由で**性別も絞らせない**
      （`src/lib/constants/gender.ts`。あちらは均等法5条で、**条文が違う**ので混ぜないこと）。
...
| 型 | 状態 |
|---|---|
| `lib/people/directory.ts` の `DirectoryPerson` | ★`age` **なし**／**`ageBand` だけ**ある（下記） |
...
### ★`/people` の「年代」絞り込み（2026-09-18 / 柴さんの判断）

**`/people` の詳細検索にだけ年代（20代〜70代）の絞り込みがある。**

| | |
|---|---|
| 型に載るもの | **`DirectoryPerson.ageBand`（`20s`〜`70s`）だけ** |
| 載せないもの | **実年齢・生年月日**。クライアントへ送らない |
| 表示 | ★**しない。** カードにも一覧にも年代を出さない。**使うのは絞り込みだけ** |
| 計算 | `lib/age.ts` の `getUserAge()` を通し、`directory.ts` で10年刻みに畳む |

⚠️★**`age`（実年齢）を型に戻さないこと。** 2026-08-20 の「型に無ければ書けない」という
   担保は、**`age` を置かないことで維持している**。絞り込みに要るのは帯までで、
   実年齢を送る必要がない。**帯を画面に出す実装も足さないこと。**

⚠️★**`/biz/candidates`（企業側）には入れない。** あちらを禁じている理由は
   **企業（事業主）に禁止行為の手段を渡すこと**で、`/people`（求職者どうし）とは別。
   ⚠️ ただし `/people` は**ログインすれば誰でも見られる**ので、採用担当者が個人アカウントで
      見ることは止められない。**この点を承知のうえで柴さんが入れた**（2026-09-18）。

⚠️ **20歳未満と80歳以上は `null` にする。** 選択肢に無い帯へ丸めると、実データと違う帯に入る。

⚠️ 生年月日が無い人は `ageBand` が null で、**年代で絞ったときだけ**落ちる（他の条件では落とさない）。
   実測（2026-09-18 / 本番）: 実ユーザー7人中 **3人**しか生年月日を持っていない
   （20代1 / 30代2）。**7人中4人は年代で絞った瞬間に消える。**
   ⚠️★**「年代で絞ると激減する」「0件になる」は仕様であって不具合ではない。**
      調べ直す前にここを読むこと。画面には理由を出していない
      —— 出すと**他人の未入力を説明する文**になり、読んだ人にできることが無いため
      （2026-09-18 / 柴さんの判断）。

⚠️★**0人の年代も選択肢に出す**（柴さんの指示）。実データから作ると歯抜けの梯子になる
   —— 都道府県・フェーズ・事業領域チップと同じ扱い。**4つ目の例外。**
...
⚠️★**候補者は匿名。** 画面側の担保は「`ow_users` を select しないこと」
   ...
   ⚠️★画面の「**双方が会いたいと答えたときに初めてお互いが分かります**」は、
      ...
      ⚠️★**2026-09-21 に文ごと書き直した。**「お互いが分かります」は**事実と違っていた**
         ——企業名は最初から出しているので、伏せているのは候補者の側だけ。
         いまは**何が渡るか**（公開プロフィール＝氏名・顔写真・見出し・職歴の勤務先と
         在籍期間）まで書いてある。⚠️ **②と⑨は同じ事実を向きだけ変えて書いている。
         片方だけ直さないこと。**
      ⚠️ mutual で新しいアクセス権が渡るわけではない（`/u/[id]` は `login_only` で
         ログインすれば誰でも読める）。渡るのは**「この提案の候補者はこの人だ」という
         結びつき**。
...
⚠️★★**提案は「答えられる企業」にだけ出す**（2026-09-21 / 柴さんの判断）。
   母数は **掲載中 → 在籍した会社を除く → ★`ow_company_admins`（`is_active`）がいる企業**。
   ⚠️★**`/biz` の入口は `ow_company_admins` だけ。** 実測（2026-09-21）: 掲載22社のうち
      ログインできる人がいるのは**2社**で、**残り20社は「会いたい」を押せる人が居ない**
      ＝ 候補者にとって**永久に無回答の行き止まり**になる。
   ⚠️★**メールを足しても解決しない**（押す画面に入れないため。
      `docs/ops-fallback-20260915.md`「コードは『受け取る』までしかしない」）。
   ⚠️★**`notification_emails` で代用しない。** 通知の宛先と「返答できるか」は別。
   ⚠️ **外した社数を運営画面に出す**（黙って消さない）。
...
⚠️★★**提案は本人の意思表示（`career_stance`）を尊重する**（2026-09-21）。
   ...
   （`can_send_scout()` の条件1を TS に写したもの）。**条件を書き写さないこと。**
   ⚠️★**2026-09-21 まで提案はこれを一切見ていなかった。** スカウトは `no_contact` と
      未設定を落とすのに**提案だけ素通り**で、実ユーザー10人中**4人**が
      「スカウトは届かないのに提案は出る」状態だった（＝「設定の意味を後から拡大」）。
   ⚠️ 受け取らない人を運営の候補者一覧から**消さない。ラベルで示す**（黙って消さない）。
   ⚠️★**規約には「提案」が1語も書かれていない。** プライバシーポリシー §5-1 は
      提供の場面を「次の場合に**限り**」で3つ挙げており、提案はどれにも当たらない。
      文案は [docs/pending/privacy-20260921-根拠つき提案.md](docs/pending/privacy-20260921-根拠つき提案.md)
      に置いてある（**当てていない**。改定日と掲示は柴さんの判断）。

⚠️★★**提案は「候補者 × 企業 × 求人」につき一度きり**（`ow_proposals_unique` ＋
   `on conflict do nothing`）。**一度出したら見送られても二度と提案に出てこない。**
   2026-09-21 に**意図した仕様として確定した**（柴さんの判断）。
   ⚠️★**期間や理由による自動の再提案を作らないこと。** 見送り理由には
      **時間で変わるもの**（`timing` / `salary` / `experience`）と**変わらないもの**
      （★`known`「すでに知っている」/ `location` ほか）が混ざっていて、
      **一律に N か月後へ流すと `known` の会社をまた出す**。見切りは実データが要る。
   ⚠️ 運営の逃げ道は **`/admin/proposals` の「消す」（1件だけ）**。
      `deleteProposalsForCandidate`（候補者の全部）は**④の材料まで cascade で消す**ので、
      1件で済むときは使わない。
   ⚠️ **根拠が2件そろわず作られなかった組み合わせは、行が無いので後から普通に作られる。**
      burn されるのは一度出したぶんだけ。混同しないこと。
...
##### 6. ★★★**保存の API でも同じことが起きる**（2026-09-11 / **実害あり**）

**上の1〜5は「読むとき」の話だったが、同じ形が「書くとき」にもあった。**
⚠️★**こちらのほうが見つけにくい。** マッパーではなく**呼び出し側と API の間**にあるため、
   片方のファイルを読んでも気づけない。
...
| 場面 | 正しい扱い |
|---|---|
| **作成**（POST） | **呼び出し側に決めさせない。** 型（`CreateExperienceBody`）から外し、サーバーは body を**読まない**（送られたら `console.warn`）。値は**既存行から引き継ぐ／無ければ既定** |
| **更新**（PUT） | 「**送られてこない＝変更しない**」。`?? 既定値` ではなく **`undefined` を返して列ごと触らない**（`JSON.stringify` がキーを落とす） |
...
⚠️★**ただし「引き継ぐ」も万能ではない**（2026-09-11 に一度入れて戻した）。
   ...
   **安全側でも、本人の代わりに決めたことに変わりはない。**
   ⇒ **引き継ぎを入れてよいのは「その設定が職歴全体に効く1設定」として
      本人に見えている場合だけ**。
      ⚠️ 例として挙げていた `visibility_company` は **2026-09-15 に入力欄ごと畳んだ**ので、
         **いまは「本人に見えている1設定」が1つも無い**。引き継ぎを入れてよい前提が
         消えた状態 ——**足すなら、まず本人が見える設定を作る話から。**
   ⚠️ `visibility_reason` は**入力欄が無い**ので、**まず入力欄の話が先**。

⚠️ 実害の無かったもの（**直していない。一覧に残す**）:
   `is_current`（本人の入力値なので送るのが正しい）／ `display_order`（意図どおり）／
   `visibility_salary`（誰も送らないので既定が効く）／ `prefecture`・`remote_work_status`
   （両側とも `null` で一致）。
→ 経緯と方式の比較は [docs/experience-create-defaults-20260911.md](docs/experience-create-defaults-20260911.md)
...
## ★オンボーディング周辺の見直し（2026-09-09〜10）── docs 5本の索引

**1つの見直しから一続きに繋がった作業。** 個別の理由は各 docs にある。**ここに書き写さない。**

| 何を | docs |
|---|---|
| 調査（0-1〜0-5。**黄色いバナーは不具合ではなかった**） | [phase0-onboarding-20260909.md](docs/phase0-onboarding-20260909.md) |
| 完了画面の削除 ／ `?next=` の実装（A案）／ 公開範囲の一文を戻した裏取り | [phase1-onboarding-20260909.md](docs/phase1-onboarding-20260909.md) |
| **`auth_id IS NULL` を7箇所で除外**（本人が登録していない行を出さない） | [auth-id-null-users-20260910.md](docs/auth-id-null-users-20260910.md) |
| `IntentCard` の進捗表示（**%も「あと N 項目」も出さない**） | [profile-progress-20260910.md](docs/profile-progress-20260910.md) |
| 完成度バーの代替が実在しない ／ `is_test` 除外が7箇所に重複 | [todo.md](docs/todo.md) |

⚠️★**この一連で繰り返し効いた原則は3つ。**
   ① **画面に書く強い言葉は実測で裏を取る**（「企業から見えていません」「声がかかります」は
      どちらも実測して**狭めた**）
   ② **個別の行を直すのではなく、条件を1箇所に定義して全経路から呼ぶ**
   ③ **効いていない仕組みを残さない**（`?next=` は読まれておらず、完了画面は到達しなかった）

## オンボーディングの現状（2026-09-16 更新・これが正）

⚠️★**この節は2回、実装から遅れた。**
   ・2026-09-14 まで「単一画面。ステップ分岐は無い」（実装は 09-11 に3 / 09-14 に4ステップ）
   ・**2026-09-16 まで「4ステップ」**（実装は **2026-09-15 の `27792de8` で3ステップ**に戻っていた）
   **画面を足す・減らすときは、同じコミットでここを直すこと。** 2回とも直せていない。

### 画面構成：**3ステップ**（`?step=` は URL に置く）

⚠️★**番号を直書きしないこと。** `OnboardingClient` の
   `STEP = { YOU: 1, COMPANY: 2, STANCE: 3 }` を使う。
   画面を1つ挟むだけで `step === 1` の意味が全部ずれる（2026-09-14 に実際にずらした）。

   ⚠️★★**書いた翌日に本番で踏んだ**（2026-09-15）。`goStep(3)` が1つ残っており、
      **3画面目の「次へ」が自分自身に戻って**オンボーディングが詰まった。
      それまで「転職について」は2画面目で、`goStep(3)` は「最後の画面へ」の意味だった。
      ⚠️ **関数名のズレが見落としを助けた** ——`goNextFromStep2` が実際は3画面目だった。
         **名前にも画面番号を入れないこと**（いまは `goNextFromStance` / `goNextFromCompany`）。

### ⚠️★★オンボーディングの画面を足す・入れ替えたら、**1画面目から最後まで通しで踏む**

**これを省くと、静的検査では絶対に出ない形で詰まる。**

| なぜ静的に出ないか |
|---|
| `goStep(3)` は**型としては正しい**。`tsc` も lint も通る |
| 症状が「**押しても何も起きない**」なので、HTTP status にもログにも出ない |
| 保存は効いているので、**DB を見ても異常が無い**（実際 `career_stance` は保存されていた） |

⚠️★**API を直叩きして「保存できた」で済ませないこと。** 2026-09-14 に4ステップへ
   変えたとき、step 1 の保存は API で確かめたが**画面を順に通していなかった。**
   詰まりはその間にあった。
   ⚠️ **画面数を本文に書かない**（当時は4、いまは3）。「1画面目から最後まで」と書く。

#### 手順

1. **未完了のアカウントが要る。** 完了済みだと `OnboardingGuard` が `/companies` へ飛ばす。
   ⚠️ **実ユーザーのフラグを倒さないこと。** `is_test` で `onboarding_completed = false`
   のものを使う（2026-09-15 時点で複数ある）。
2. `?step=1` から順に、**各画面で「次へ」を押して URL の `step` が1つ進むことを見る。**
   ⚠️ **最後の画面まで通す。** 途中まででやめると、その先の1本が残る。
3. 検証で入った値は**作業前に戻す**（`career_stance` / 希望職種 / `onboarding_completed`）。

⚠️ 押す前に `goStep(` を grep して**数字が無いこと**を確かめると早い。ただし
   **それだけで済ませない** ——条件分岐（`step === STEP.X`）のズレは grep では出ない。

| # | 画面 | 聞くもの | 「後で設定する」 |
|---|---|---|---|
| 1 | **あなたのこと** | **姓 / 名 / せい / めい / 生年月日 / お住まい —— すべて必須** | ★**出さない** |
| 2 | 直近のお勤め先 | 会社・部署名・職種・社内での呼び方・入社年月 | ★**ここにだけ出る** |
| 3 | 転職について | `career_stance` ＋ 関心のある職種 | 出さない |

⚠️★**4画面目「あとは任意」（勤務地・勤務形態・これまでの職歴・学歴）は
   2026-09-15 に削除した**（`27792de8`）。着地は `/mypage`（`DEFAULT_AFTER_ONBOARDING`）。
   ⚠️ 勤務地・勤務形態を `saveCurrentExperience` に足し戻さないこと ——**聞く画面が無い**ので
      常に空になる。入れてもらうのは `/mypage` の職歴の編集モーダル。

#### ⚠️★★お住まい（`ow_users.location`）を1画面目に足した（2026-09-23 / 柴さんの指示）

**生年月日のすぐ下。必須。** 保存は同じ `PUT /api/jobseeker/profile`。

足した理由は生年月日とまったく同じ形。実測（2026-09-23 / 本番 / 実ユーザー13人）:

| | |
|---|---|
| お住まいあり | **4人（31%）** |
| 生年月日あり | 8人（62%）※2026-09-14 に1画面目へ移す前は **3/12** |

⚠️★**器だけの列ではない。** 読み手が実在する ——`/biz/candidates` の**都道府県での絞り込み**と
   候補者カードのタグ ／ `/biz/conversations/[id]` ／ `/u/[id]` のメタ行（年齢の隣）。

⚠️★**必須にできるのは選択肢に「非公開」があるから。** 逃げ道のない生年月日より弱い要求。
   `RESIDENCE_EXTRA_OPTIONS` から「非公開」を外すなら、**先に必須をやめるかどうかを決めること。**

⚠️★**勤務地（`ow_experiences.prefecture`）とは別の列。** 上の「足し戻さないこと」とは無関係。
   あちらは職歴の属性、こちらは本人の属性。

⚠️★**画面に出るラベルは「お住まい」。本人向け3箇所を同日に揃えた**
   （オンボーディング1画面目 ／ `/mypage` の編集モーダル ／ `/mypage/settings`）。
   ・「所在地」に戻さないこと —— `/biz/company` と `/admin` が**会社の住所**に
     「本社所在地」「オフィス所在地」を使っていて語がぶつかる（`/mypage` は 2026-09-23 まで
     これだった）
   ・「居住地」は **`/biz/candidates` の企業向けの語**。「『話せる人』は企業・運営向けの語で、
     本人向けの画面では使わない」と同じ理由で本人には出さない
   ⚠️ **DB・API・`/biz` 側の語は変えていない**（`?industry=` と同じ形で、承知のうえ）。

⚠️★**選択肢は [`RESIDENCE_OPTION_GROUPS`](src/lib/utils/location.ts) の1箇所。**
   この列には **DB の CHECK が無く、語彙の担保が入力欄だけ**。
   ⚠️ `location.ts` の注記は 2026-09-23 まで「**3つ目の入力欄を作らないこと**」だった。
      3つ目を足したので**「入力欄は必ずこの定数から出すこと」に書き換えてある。**
      心配していたのは箇所数ではなく**語彙が割れること**で、3箇所とも import していれば起きない。

⚠️★**「後で設定する」が出るのは2画面目だけ。** 条件は
   `step !== STEP.STANCE && step !== STEP.YOU`。
   ＝ **経歴を聞く画面が、経歴を落とす唯一の抜け道になっている。**
   押すと `finish()` が走り、オンボーディングは完了して**経歴は保存されない**。
   実測（2026-09-16 / 実ユーザー9人）: **完了済みなのに経歴0が2人。**
   ⚠️ 下書きも部分保存も無いので、**この2人がこのボタン由来かは区別できない。**
   ⚠️ これは「任意項目に逃げ道を残す」という判断の結果で**バグではない**。
      見直すなら計測とセットで（docs/phase0-career-supply-20260916.md の C・E）。

#### ⚠️★1画面目を足した理由（2026-09-14 / 柴さんの指示）

実測（2026-09-14 / 当時「実ユーザー12人」と数えていた母集団）:
  ・**生年月日は 3/12** しか入っていなかった。最後の画面の末尾で、見出しが「あとは任意」だった
...
⚠️★**「氏名も 1/12 がメールのローカル部だった」と最初に書いたが、これは誤りだった**（同日訂正）。
   ...
   **漏れは経路としては実在したが、一度も起きていなかった。**
   ⚠️ 原因は判定式。`^[a-zA-Z0-9._+-]+$`（**ASCII だけの名前**）に当たったものを
      「メールのローカル部と同じ」と読み替えていた。当たっていたのは
      **本人が入力した英字の名前**（`Hina`）。
   ⚠️★このリポジトリの「**部分一致で結論を出さない。検出したら実テキストを目で見る**」
      （`.claude/rules/ui-debugging.md` ⑱）をそのまま踏んだ例。
      **`~` や部分一致で数えたら、必ず1件を実データで開いて確かめること。**

⚠️ **必須化の判断は変えていない。** 根拠が「既に漏れていた」から
   「**7人中2人（29%）がオンボーディングを完了せず、登録フォームが唯一の受け皿**」
   に変わっただけで、後者は実測どおり。
   ⚠️ コミットメッセージ（`0373e88a` / `9ac28ebe`）には誤ったほうの数字が残っている。
      push 済みなので直せない（amend / rebase は禁止）。**ここが訂正の記録。**

⚠️★**摩擦は増えていない。** 項目を足したのではなく**順番を変えた**のが主で、
   会社を検索して職種を選ぶ2画面目より軽い（打つだけ）。

#### ⚠️★★登録フォームの「お名前」は**姓・名の2欄**（2026-09-19 / 柴さんの判断）

**`/auth` の新規登録で姓と名を別々に聞き、`ow_users.name` は `buildDisplayName` で畳んで送る。**
1画面目は `family_name` / `given_name` が空のとき **`splitDisplayName`** で `name` を分け、
分けられなければ**登録名をヒントで見せる**（入力欄は空のまま）。

⚠️★**1欄に戻さないこと。** 戻すと空白で分かれない名前（「木村雅樹」。
   実測 2026-09-19 で実ユーザー9人中**5人**）が1画面目で空になり、
   **空の必須欄にブラウザが別人の名前を自動入力する**（実際に起きた）。

⚠️★**空白なしの名前を姓へ丸ごと入れないこと。** 姓＝木村雅樹 のまま名を足すと
   「木村雅樹 雅樹」になり、初期値が正しそうに見えるぶん気づかれにくい。

⚠️★**`splitDisplayName` とヒントを消さないこと。Google 経由はこのフォームを通らない。**
   しかも **Google は姓名を渡してこない**（2026-09-19 実測: `identity_data` のキーは
   `avatar_url / email / email_verified / full_name / iss / name / phone_verified /
   picture / provider_id / sub`。`given_name` も `family_name` も**無い**）。
   実測（2026-09-19 / 実ユーザー9人）: メール5 / Google 3 / 併用1。

⚠️ **姓名は列に保存していない。** 登録フォームが送るのは `name` だけで、
   `family_name` / `given_name` は**1画面目で本人が確認して「次へ」を押したとき**に入る。
   列へ保存する形にするなら **DBトリガー `handle_new_ow_user` と `linkOwUser.ts` の両方**を
   変えることになる（2026-09-19 に検討して**保留**にした。サインアップ全体を止めうるため）。

⚠️★**1画面目に「後で設定する」を出さないこと。** 出すと必須にならない。
⚠️★**1画面目だけ、保存に失敗したら進まない。** 必須項目なので、黙って次へ行くと
   「入力させたのに保存されていない」になる（他の画面は任意なので best-effort のまま）。

#### ⚠️ 2画面目：**会社を選ぶまで職種・入社年月は出さない**（入口の摩擦を増やさないため。維持）

| 項目 | 備考 |
|---|---|
| 会社 | マスタ検索 or 自由入力。どちらも可 |
| **部署名 / 社内での呼び方** | ★**2026-09-14 から常時表示。** それまで「＋ 社内での呼び方・部署名」で畳んでいたが、**押さずに素通りする人が多そう**という指摘で覆した。押さないと**存在に気づけない**。⚠️ 「これまでの職歴」の各行は元から常時表示で、これで2箇所が揃った。**片方だけ畳み直さないこと。** ⚠️ 並びは 部署名 → 職種 → 社内での呼び方 |
| 職種 | `RoleSearchSelect`（検索＋大分類18件）。⚠️ **子は任意。親のままでも保存できる。** |
...
| 何を | どこへ | どの API |
|---|---|---|
| 姓・名・ふりがな・生年月日・**お住まい** | **`ow_users`** | **`PUT /api/jobseeker/profile`**（1画面目の「次へ」） |
| 会社・部署・職種・入社年月 | `ow_experiences` | `POST/PUT /api/jobseeker/experiences` |

⚠️★`ow_users.name`（表示の正）は**姓＋名から派生**させて**同じ経路で一緒に**書く
   （`lib/constants/personName.ts` の `buildDisplayName`）。**別経路で書かないこと。**
   ⚠️★**2026-09-15 まで、その「別経路」が生きていた。** `/mypage` のプロフィール編集が
      **1欄の「名前」**を単独で PUT しており、オンボーディング1画面目（姓・名）と
      **同じ列を別々に書いていた**。先に保存したほうが後勝ちで、`name` と
      `family_name`/`given_name` が食い違う形。**実害0件だったのは、
      まだ誰も姓・名を持っていなかったから**（起きなかった0ではなく**起こせなかった0**）。
      → `/mypage` の編集モーダルも**4欄（姓/名/せい/めい）**にし、
        `PUT /api/jobseeker/profile` の**単独 `name` を廃止**した。
      ⚠️★**1欄に戻さないこと。** 戻すと同じ形に戻る。
      ⚠️★**型では止まらない**（`body` は `Record<string, unknown>`）。歯止めは
         route.ts の `console.warn` と注記だけ。**`name` を送る呼び出し側を足さない。**

#### ⚠️★氏名を後から直せる場所は `/mypage` の編集モーダルだけ（2026-09-15）

**オンボーディング1画面目は一度きり**なので、そこだけだと
**完了済みの人は姓・名・ふりがなを永久に持てない**（実測 2026-09-15: 実ユーザー7人中**5人**が完了済み）。

| | |
|---|---|
| 必須か | **オンボーディングでは必須／このモーダルでは任意** |
| 空のまま保存 | **できる。** その場合キーごと送らないので `name` は動かない |
| 片方だけ | **保存前に止める**（`handleSaveHeader`）。API も 400 で弾く（二重） |
| 姓・名が空の人 | ヒントに**いまの表示名**を出す。出さないと「名前が消えた」と読まれる |

⚠️★**既存の `name` から姓・名を機械的に分割しないこと**（「山田太郎」は分割できず、
   空白で切るのは推測値の投入）。**本人に入れてもらう。**

⚠️ 初期値の4列は `/mypage/page.tsx` が **admin クライアント**で引く。
   ⚠️★`birth_date` と同じで **authenticated に SELECT を配っていない**ので、
      session クライアントの select に混ぜると**クエリごと 403**になり、
      ダッシュボードが丸ごと空になる（画面は 200 のまま）。

#### ⚠️★居住地の選択肢は [`RESIDENCE_OPTION_GROUPS`](src/lib/utils/location.ts) の1箇所（2026-09-15）

**47都道府県＋「海外」「非公開」。** `/mypage` の「所在地」と `/mypage/settings` の
「居住地」が**同じ `ow_users.location`** に書くので、割れると片方で選んだ値が
もう片方で**空欄に見え、保存した瞬間に消える。**

⚠️ 2026-09-14 に `/mypage/settings` を作ったとき、既存の `LOCATIONS`
   （`mockProfileData.ts`・**11件**）を数えずに `PREFECTURE_FILTER_GROUPS`（47件）で
   2つ目を作ったのが原因。**38県は片方でしか選べなかった。**
   実害0件だったのは、実ユーザー3人の値が**たまたま両方に入っていた**ため。
⚠️★**絞り込み（`/companies` `/jobs`）は `PREFECTURE_FILTER_GROUPS` のまま。**
   「非公開」は本人の意思表示で、検索条件としては意味を持たない。
⚠️ `LOCATIONS` は削除した（使い手0件）。**書き戻さないこと。**
⚠️ `PUT /api/jobseeker/profile` に **`.select()` を足さないこと**。`birth_date` や
   `family_name` は `authenticated` に SELECT を配っていないので **403（42501）** になる。
⚠️★既存値のプリフィルは **`d2f3132d` で実装した**（同日。この行は「していない」のまま
   古くなっていたので直した）。**session クライアントでは引けない** ——
   クエリが丸ごと 403 になり `ow_users.id` まで取れなくなる。だから
   **`onboarding/page.tsx` が admin クライアントで別途引いている。**
   ⚠️ 月日は **`<option value="07">` に合わせてゼロ埋めする**。`slice` で切り出した
      `"7"` を渡すと**一致する option が無く、`value` が空のまま**になる
      （`.claude/rules/ui-debugging.md` ②と同じ形）。

#### ⚠️★★電話番号・ふりがな・性別は「**器だけ先に持つ**」と決めた（2026-09-14 / 柴さんの判断）

**読み手が本人と運営しかいないが、これは漏れではない。消さないこと。**

| 列 | いま誰が読むか |
|---|---|
| `phone` / `gender` / `location` | 本人（`/mypage/settings`）と運営（`/admin/candidates`） |
| `family_name_kana` / `given_name_kana` | 本人（オンボーディング1画面目・**必須**）と運営（同上） |

⚠️★**「読み手が無い＝死列」と判断して DROP しないこと。** このリポジトリが禁じているのは
   **「保存経路の無い入力UI」**（2026-07-28 の6件）で、これは**保存も表示もある。**
   足りないのは**その先の業務**だけ。柴さんが「LinkedIn なども電話番号を登録している」
   と判断し、**標準的なプロフィール項目として先に器を持つ**ことにした。

⚠️★**当初「人材紹介の書類（推薦状・求職管理簿）で使う」と書いたが、これは誤り**（同日訂正）。
   **OPINIO は企業と求職者が直接やり取りする「ダイレクトリクルーティング」**で、
   職業安定法第4条第6項の**募集情報等提供**に該当し、**職業紹介ではない**
   （`content/legal/terms-of-service-listing.md` 第2条3項 / `legal-agency.md`）。
   求職管理簿は**別サービス**の話。**この取り違えを戻さないこと。**

⚠️★**いまの出し先は「本人＋運営」まで。企業には出していない。**
   使い道が決まったら、**出し先を改めて決めること**（本人だけ／運営だけ／企業にも）。
   ⚠️★**性別だけは別格。** 企業に**見せる／絞らせる**のは避ける
   ——**企業が直接絞り込むダイレクトリクルーティングだからこそ、均等法5条違反を
   直接手助けする形になる**（`src/lib/constants/gender.ts` に理由の全文）。
   **性別の絞り込みは入れない。**
   ⚠️ 年齢で絞らせない判断（上の「年齢は詳細だけ」）と同じ向きだが、
      **根拠の条文が違う**（あちらは労働施策総合推進法9条）。混ぜないこと。
...
既定は `real`（当時の実測で14件中13件が real で、企業ページに出るのが本人の目的に沿うため）。
⚠️ この「14件」は 2026-08-10 時点の数。**2026-09-14 に検証用5件を `is_test` に倒したので
   母数が変わっている**（登録ユーザー 12人 → 7人 ／ 職歴がある人 8人 → 4人）。
   **これより前の日付で「実ユーザー N人」と書いてある記述は、その5件を含んだ数。**
   ⚠️★**最新は 2026-09-16 実測で 登録ユーザー 9人 ／ 職歴がある人 6人。**
      内訳とファネルは [docs/phase0-career-supply-20260916.md](docs/phase0-career-supply-20260916.md)。
      ⚠️ **数字は増えるので、書くときは必ず実測日を添えること。**
⚠️★**「会社名は伏せる」チェックはもう無い**（2026-09-15 に設定ごと畳んだ。
   「✅★★会社名を伏せる設定は畳んだ」を読むこと）。
   ⚠️ 列・データ・読み手は残っているので、**既定が `real` であること自体は変わらない。**
...
⚠️★**`company_anonymized` を送るクライアントはもう無い**（2026-09-12 / 柴さんの指示）。
   職歴エディタの匿名企業の入力欄と `isAnon` の分岐を**経路ごと削除した**。
   OPINIO に「非公開企業」という概念が無いため。
   ⚠️ ここでの「非公開企業」は**社名を明かさない会社**（`company_anonymized`）の意味。
      **ページ非公開（`is_published = false`）とも、非上場（`phase = non_listed`）とも別。**
      語の使い分けは下の「★『非公開企業』という語を使わない」を読むこと。
   ⚠️★**2026-09-02 の「経路ごと消さずに読み書きを保つ」はこの指示で置き換えた。**
      当時の心配は「既存の匿名行を編集すると `company_text` へ黙って化ける」だったが、
      実測（2026-09-12 / 本番34件）で **`company_anonymized` を持つ行は0件**
      （実ユーザーも0件）。**化ける行が1つも無い**ことを確かめてから外している。
   ⚠️ **列・データ・API の XOR 検証・表示側（`toStint` / `timeline.ts` /
      `companyName.ts`）は残してある。** 消したのは入力欄とクライアントの分岐だけ。
...
## ⚠️★職歴・学歴は `/mypage` の行の鉛筆で編集する（2026-09-12 に一覧ページを畳んだ）

**`/mypage/details/experience` と `…/education` は `src/middleware.ts` が `/mypage` へ転送する。**
2026-08-17 のフェーズ3で「1件ずつ触るのは一覧ページの仕事」と決めた構造を、職歴・学歴だけ戻した
（柴さんの指示）。**他の section（実績・受賞・資格・言語・スキル・メディア・発信）は従来どおり。**

⚠️★**ページ側の `redirect()` では 307 にならない**（`/mypage/loading.tsx` の Suspense 境界）。
   だから middleware で返している。**戻すときは転送も外すこと。**

| どこにあるか | 何 |
|---|---|
| 行の右端の鉛筆（18px / 当たり判定32px・767px以下は44px） | 編集モーダル |
| 編集モーダルのフッター**左端** | 削除（二段階確認） |
| 編集モーダルの中 | 「＋ この会社に役割を追加」 |
| セクション下の「＋ 職歴/学歴を追加」 | 追加（**0件でも出す**） |
| 「すべて表示（他n件）」 | **その場で展開**（遷移しない） |

⚠️ **見出しの ＋ と ✎ は両方外してある。戻さないこと。**
⚠️ 「＋ 雇用形態を追加」は削除した。雇用形態は編集モーダルの入力欄で入れる。

⚠️★**入学年月が無い学歴は「年月未設定」として学歴セクションの下に全件出す。**
   年表（`toTimelineEducationEntries`）が**入学年月の無い行を落とす**ので、ここが唯一の入口。
   **落とすとどの画面からも編集できなくなる**（実ユーザー2件が該当。2026-09-12 実測）。
   ⚠️ 「すべて表示」の件数には含めない（常に全件出すため）。

⚠️★**経歴ストーリー（`StoryAccordion`）は 2026-09-12 に入口ごと畳んだ**（柴さんの指示）。
   置き場が `/mypage/details/experience` の1箇所だけで、そのページを転送にしたため。
   **テーブル（`ow_experience_stories` / `ow_story_sections`）・列・データと API ルート4本は
   残してある**（実データ0件。作り直す可能性があるため）。
   ⚠️ 部品と `MergedTimeline` の差し込み口（`renderCareerExtra`）は削除した。
      復活させるなら「渡した画面にだけ出す」形を守ること（`/u/[id]` に渡さない）。

---

### ⚠️★「選んだ理由」の設問は職歴の編集から切り離してある（2026-09-12）

**実体は [ExperienceReasonModal](src/components/profile/editor/ExperienceReasonModal.tsx)。**
入口は3つ ——①新規追加の保存直後 ②職歴カードの吹き出しアイコン
③**職歴セクション上部の「まとめて答える」**（未回答が1件以上のときだけ出る）。

⚠️★**職歴の編集モーダルに設問を戻さないこと。** 役職を直すたびに設問が出る形になる。
⚠️★**理由を送らない PUT が既に存在する。** `parseReasonFields` の
   「**キーが無い＝触らない**」を外すと、**職歴を1項目直しただけで回答が全部消える。**

| 決め事 | ⚠️ |
|---|---|
| 題は現職かどうかで変える | 判定は **`hasLeftCompany`（`CareerHistoryEditor.tsx`）の1つだけ**。画面と保存 body が同じ関数を見る |
| **ステップ表記「1 / 2」は現職でも出す** | 現職でも画面は2枚（理由 → ギャップ）。**分母を消さない** |
| **選択0件でも「次へ」は通す** | ギャップだけ答えたい人の経路。**「保存」だけ**を全ステップ0件のとき出さない |
| 保存しても閉じない | 保存後は在籍者数の「見返り」に切り替わる。`justSaved` を立てない（ボタンが押せなくなる） |
| 在籍者数は `GET /api/jobseeker/companies/[id]/employees` | **数のために別ルートを作らない。** ⚠️ `is_test` を含む数（上の例外）／自由入力の会社では**行ごと出さない** |
| スラッグ | **削除も改名もしない。⚠️★追加はできる**（`careerReasons.ts` の注記が正）。⚠️ 2026-09-15 まで「**追加も**しない」と書いていたが誤り —— 次に必要になった人が「できることをできない」と判断してしまう |

#### ⚠️ 選択肢を足すときの3条件（2026-09-15 に確認）

**入社 12個 / 退職 13個**（うち11個は軸を突き合わせるため**共通のスラッグ**）。
柴さんが 2026-09-15 に「少なくないか」を検討し、**このままにすると判断した。**

足す日が来たら、この3つを守ること。

1. ⚠️★**末尾に足す。** 「上に置いた選択肢ほど選ばれやすい」ので、途中に入れると
   **既存の選択率が動いて前後比較ができなくなる**（`careerReasons.ts` の並び順の注記）。
2. ⚠️★**退職側と対にするか、片側だけにする理由を書く。** 対応が崩れると
   「◯◯で入った人の何%が◯◯で辞めたか」が出せない。
3. ⚠️★**DB の CHECK も同時に広げる**（`ow_experiences_join_reasons_check` /
   `..._leave_reasons_check` に**配列で直書き**されている）。UI だけ足すと
   **選べるのに保存できない**（3層の規約）。

⚠️ 実データ（2026-09-15 実測 / 全37行）: 入社理由あり **5行** ／ 退職理由 **0** ／
   決め手 **0** ／ `ow_experience_gaps` **1**。
   ⚠️★**足すなら早いほうが安全。** 集計が積み上がってからだと、
      選択肢が増えた前後で数字が比べられなくなる。

→ 実測と前後比較は [docs/reason-flow-20260912.md](docs/reason-flow-20260912.md)
...
### ⚠️★職種の候補を絞る条件は用途ごとに違う（2026-09-12 に希望職種を揃えた）

| 用途 | 絞り込み | 大分類 |
|---|---|---|
| **希望職種 / 関心のある職種**（オンボーディング step2 ＋ `/mypage` の「転職・面談の状況」） | `is_active` かつ **`is_it_saas`** | **10** |
| **職歴の職種**（オンボーディング step1・3 ＋ 職歴の編集モーダル） | `is_active` のみ | **18** |
| 求人（`/biz/jobs/*`） | `is_active` かつ `is_it_saas` | 10 |

⚠️★**希望職種の条件は [lib/roles/desiredRoleOptions.ts](src/lib/roles/desiredRoleOptions.ts) の1箇所。**
   **オンボーディングと `/mypage` の両方がこの関数を通る。** 画面側に `is_it_saas` を書かないこと。
   ⚠️ 2026-09-12 まで **`/mypage` だけが絞っており、オンボーディング18分類 / `/mypage` 10分類**と
      食い違っていた（柴さんの指示で `/mypage` に揃えた ＝ 案B）。
   ⚠️ 絞る理由は「**おすすめの求人に使う項目で、掲載される求人は IT/SaaS の企業だけ**」。
      対象外の職種を選べても当たる求人が無い。

⚠️★**職歴には掛けない。** あちらは「これまでの経歴」。非IT の8分類
   （医療・建設・製造・教育・販売・金融・物流・公務）を落とすと、
   過去に非IT職だった人が自分の職種に辿り着けない。

⚠️★**絞り込みを足すときは必ず「選択済みの足し戻し」とセットにする。**
   足し戻しが無いと、**対象外の職種を既に持っている人の選択が画面から消え、
   別の職種を足して保存した瞬間に失われる**（保存は「送られた配列で置き換える」形）。
   ⚠️ **親も足す。** 子だけ足しても大分類の行が無ければ開けず選べない。
   → 2026-08-06 に求人フォームで入れた仕組みと同じ。実測は
      [docs/roles-selected-value-check.md](docs/roles-selected-value-check.md)。
...
### ✅★★会社名を伏せる設定は畳んだ（2026-09-15 / 柴さんの判断）

**`/mypage/settings` の入力欄と説明文を外した。⚠️ 列・データ・API・読み手は残してある。**

⚠️★**入力欄を戻さないこと。** 戻すなら下の3つを踏まえて改めて決める。

| # | 畳んだ理由 |
|---|---|
| ① | ★**目的がもう別の仕組みで満たされている。** 根拠は「**転職を考えていると今の会社に知られること**」を防ぐことだったが、**`can_send_scout()` の条件2・2b が在籍企業には候補者一覧へそもそも出さない**（`company_id` 一致＋**自由入力の社名も `normalize_company_name` で一致**）。**社名だけ消すより強い。** 画面の「現在お勤めの会社は自動的に含まれます」（ブロック中の企業）がこれ |
| ② | **守り切れていなかった。** 企業向けの画面が `visibility_company` を見落とす漏れを**1か月で3件**直している（`/biz/employees` 08-13 ／ `/biz/candidates` 09-10 ／ `/api/biz/company/members` 09-10）。**既定で漏れる約束**で、企業向けの面が増えるたびに同じ形を踏む |
| ③ | **効き目が弱い。** 氏名と顔写真は出るので、相手が本人を知っていれば意味がない。⚠️ **LinkedIn にも「勤務先だけ伏せる」設定は無い** |

⚠️★**「使われていないから外した」ではない。** 実測は35件すべて `real`（0人）だが、
   **入口ができたのは 2026-09-11 で、畳んだ時点で4日前**。**0件は根拠に使っていない**
   （「起きなかった0か、起こせなかった0か」を分ける規則どおり）。

⚠️★**企業ページの「現役社員」から自分を外す手段も、これで無くなった。**
   必要になったら**別の設定として**作ること。**会社名を伏せる話とは別の問題。**

⚠️ 参照0件になったもの（**残してある。消さないこと**）:
   `COMPANY_VISIBILITY_OPTIONS` ／ `PUT /api/jobseeker/company-visibility` ／
   `MASKED_COMPANY_LABEL`（＝「非公開企業」。`/biz/candidates` は今も参照しているが、
   **設定できないので描画されない**）。

### ★「非公開企業」という語を使わない（2026-09-15 に書き分けた）

**4つの別々の状態を指して使われていた。** 新しく書くときは**右の語**を使うこと。

| 何の話か | 実体 | ★使う語 |
|---|---|---|
| ① 本人が勤務先の社名を伏せた | `visibility_company = masked` | （**2026-09-15 に設定ごと畳んだ**。描画されない） |
| ② 詳細ページが 404 | `is_published = false` | **ページ非公開** |
| ③ 一覧に出さない | `listing_status = 'draft'` | **非掲載** |
| ④ 上場していない | `phase = non_listed` | **非上場** |
| ⑤ 社名を明かさない会社 | `company_anonymized` | **匿名企業**（⚠️ 入力欄は 2026-09-12 に削除済み） |

⚠️ ②③は**別の軸**（「企業ページの3つのスイッチ」）。**「非公開」1語でまとめない。**
...
⚠️ **ラベルを作る側も定数に寄せた**（`toStint` / `CareerHistoryEditor` /
   ...
   ✅ `api/jobseeker/experiences/route.ts` の2箇所も**同日に寄せた**（残り0）。

⚠️ 確かめ方: **定数を実際に改名して `grep '"非公開企業"' src` を打つ。**
   動作に使っている箇所が残っていれば出る。**実測（2026-09-15）: 0件。**
   ⚠️★**「0件だった」で終わらせないこと。** この検査は**改名してから打って初めて効く**
   （改名せずに打つと、定数の定義そのものが当たって判断を誤る）。
...
| 画面 | 判定に使うもの |
|---|---|
| ~~`/career-trajectories`~~ | ⚠️★**このページは存在しない**（2026-09-15 に確認）。`src/app` にルートが無く、残っているのは `robots.ts` の Disallow 1行だけ。**判定の話として読まないこと** |
...
⚠️ **「話せる人」は企業・運営向けの語。本人向けの画面とメールでは使わない**（2026-08-23 / B-1）。
   ...
   ⚠️★**当時ここに書いてあった「本人向けは『話を聞かれてもよいか』、訪問者向けは
      『話を聞けます』」は、2026-09-18 に「面談OK」へ統一したので無効。** 下の節を読むこと。

### ⚠️★★状態のラベルは「面談OK」1語に統一した（2026-09-18 / 柴さんの指示）

**画面とメールに出る状態のラベルは、どの面でも「面談OK」。** 語の変遷はこう:

| いつ | 語 | 範囲 |
|---|---|---|
| 〜2026-08-23 | 「話を聞かれてもよい」／「話を聞けます」／「面談可」 | 面ごとにバラバラ |
| 2026-09-12 | `/mypage` のトグルだけ「**面談対応可能**」 | **本人の2箇所だけ** |
| **2026-09-18** | **「面談OK」** | ★**全部**（下の表） |

| 面 | 直したもの |
|---|---|
| `/people` | カードのバッジ・1列表示のバッジ・注意書き |
| 企業ページ | 社員カードのバッジ・`AmbassadorWidget` の見出し（「この会社で面談OKの人」） |
| `/u/[id]` ・ `/mypage` | `TalkableBadge`・`IntentCard` のトグルと行（`aria-label` も） |
| `/mypage/ambassador-invite/[token]` | 見出し・説明・箇条書き |
| **メール** | `ambassadorInviteTemplate` / `joinRequest` 系の件名と本文 |
| `/biz/members` ・ `/admin/ambassador-requests` | 説明文 |
| `/dev/preview` | ラベルとバッジのサンプル |

⚠️★**一部だけ別の語に戻さないこと。** 2026-08-28 と 2026-09-12 の2回、
   **片方だけ変えて同じ設定が2つの名前で呼ばれた**。注記があっても防げていない。
   **語を変えるときは `面談OK` を grep して、画面に出る全箇所を数えること。**

⚠️★**「話を聞く（カジュアル面談）」は変えていない。** あれは**申し込む動作**で、
   状態のラベルではない。同じ理由で次の3つも据え置き:
   `/jobs/[id]` の「◯◯ の経験者に話を聞けます」（節の説明文）／
   `/people/role/[slug]` の meta description ／ 企業ページの「◯◯さんに話を聞く →」。
   ⚠️ **機械的に置換しないこと。** 動作と状態を同じ語にすると、押す先が分からなくなる。

⚠️ カードの見出しは「意思表示」→「**転職・面談の状況**」（2026-09-12）のまま。
⚠️ **企業・運営向けの「面談対応者」「面談受付中」は変えていない**（確定済み）。
   あれは**運営の語彙**で、本人・訪問者に出す状態ラベルとは別。
⚠️ コード内の呼び名（`IntentCard` / `career_stance` / `canTalk` / `ow_company_members` /
   `aria-label="意思表示を編集"`）は変えていない。
   **画面の語とコードの語がずれている**（`?industry=` と同じ形で、承知のうえ）。
   ⚠️ **DBの列名・値も変えていない。**
...
⚠️★**掲載中の面談対応者を持つ5社のうち、4社が通知の宛先0件**（2026-09-15 実測）。
   企業側に担当者がいるのは**セールスフォース（管理者2名）だけ**で、
   **残り4社は企業が気づける経路が無い。**
   ＝ **②（企業が外す）が効かないぶんを、この一覧が肩代わりしている。**
   ⚠️★**この記述は 2026-09-15 まで「5社とも宛先が0件」「残り3社」と書かれていて、
      それ自体が噛み合っていなかった**（全社0件なのに1社だけ担当者がいることになっていた）。
      実測して 4社 / セールスフォースのみ に直した。**数字は日付を添えて測り直すこと。**
   ⚠️ 宛先を増やせば運営の負担は減るが、**企業の連絡先を運営が推測して送ることになる**ので
   設計の判断が要る（未決）。
   → 宛先が無い掲載企業は **`/admin/companies` の「通知の宛先なし N社」**で一覧できる
     （2026-09-15 追加）。運営は**取り次がず、担当者登録を案内する**
...
⚠️ **2026-09-14 に掲載が22社になった**（⑨）ので内訳も変わった:
   `listed` 14 ／ `unicorn` 5 ／ `non_listed` 2 ／ `startup` 1。
   `listed_growth` / `series_b` / `series_d` は**掲載中0社**（フェーズのチップは13件のまま出る）。
...
## ⚠️★埋め込みが曖昧で「静かに0件」になっていないかも検査する（2026-09-18 確立）

```bash
node scripts/check-embeds.mjs --self-test   # ★先にこれ。検出器が効くか確かめる
node scripts/check-embeds.mjs               # src 全体
```

⚠️★**埋め込み（`ow_xxx(...)`）を足したら走らせること。** 2つの表のあいだに
**経路が複数ある**と、FK を名指ししない埋め込みは
`Could not embed because more than one relationship was found` で**クエリごと失敗**し、
`?? []` で受けている呼び出し側では**「0件」として静かに素通りする。**

⚠️★**経路は FK 1本とは限らない。中間表（多対多）経由も数えられる。**
   FK を2本以上持つ表は **59**（2026-09-18 実測）。**表が1つ増えるたびに、
   これまで通っていた埋め込みが黙って壊れうる。** 人が気をつける形では守れない。

⚠️★**`!inner` は結合の指定であって、FK の名指しではない。** 目で見ると
   「ヒントが付いている」ように見える。**実際に投げないと見分けられない。**
   2026-09-18 に `ow_company_admins` → `ow_users!inner(...)` がこれで、
   **`/biz/jobs/new` と `/biz/jobs/[id]/edit` の採用担当者が1年以上0人**だった
   （同じ企業のダッシュボードには出ていたので、画面の中で食い違っていた）。

⚠️ **理屈で判定する形に作り替えないこと。** スクリプトは PostgREST に実際に投げて
   応答で判定する（`limit(0)` の読み取りのみ）。判定規則は PostgREST の実装であって、
   こちらの推測ではない。

実測（2026-09-18 / 689ファイル）: 埋め込みを含む select **113箇所**（99通り）→ **0件**。
⚠️ **変数を含む select 11箇所は読めない。** スクリプトが場所を出すので目で確かめる
   （11箇所とも定数を展開して確認済み・すべて通る）。
...
#### ⚠️★2層でも「定数は1つ」。画面にローカル定数を書かない（2026-09-11 に踏んだ）

**希望職種の上限が、画面と API で違っていた。**

| どこ | 値 | 誰が見ていたか |
|---|---|---|
| `lib/constants/careerPreferences.ts` の `MAX_DESIRED_ROLES` | **10** | `PUT /api/jobseeker/career-preferences` |
| `IntentCard.tsx` の**ローカル定数**（同じ名前） | **5** | 画面 |

画面は5件で止めるのに、**API は10件まで受け付ける**状態だった。
名前が同じなので、**片方を読んだ人はもう片方に気づけない**（import していないので
`tsc` も何も言わない）。**5に揃え、ローカル定数を消して定数を参照する形にした**
（実データは6人が各1件で、下げても壊れる人は0人）。

⚠️★**濃度の制約は DB に置かないと決めた以上、残る2層が食い違うと歯止めが無くなる。**
   「DB の CHECK が最後に守る」が効かないので、**定数が1つであることが唯一の担保。**
...
⚠️★**実在しない求人の掲載は「的確表示義務」に関わる。** 値の精度以前の問題として扱うこと。
   ⚠️★**根拠は「有料職業紹介の許可事業者だから」ではない**（2026-09-15 に訂正）。
      OPINIO は**募集情報等提供**（職安法4条6項）で、**その立場のまま**
      職業安定法の「求人等に関する情報の的確な表示」（5条の4）が掛かる。
      **許可の有無と関係なく効く**ので、結論はまったく変わらない。
...
2. **「ページが薄く見える」「求人0件だと寂しい」は、この判断を覆す理由にならない。**
   実在しない求人の掲載は**的確表示義務**に関わる（職安法5条の4。⚠️ **許可の有無ではなく、
   募集情報等提供という立場そのものに掛かる**。2026-09-15 に根拠を訂正）。
   見栄えより先に来る
...
- **★`<select>` は1つずつセットする**（2026-09-09 追記）。3つ（生年/生月/生日）を
  **1回の実行でまとめて**セットしたら state に届かず、**保存されなかった。**
  1つずつ入れたら通った。**人の操作では起きない**ので実装の問題ではないが、
  自動検証では**送信前に `select.value` が実際に入っていることを確かめる**こと。
- **★★表示件数・人数・可視性の前後比較は、このコマンドで cold にしてから測る。**

  ```bash
  ./scripts/dev-cold.sh
  ```

  dev を止める → `.next` を消す → 起動 → 200 を待つ、を1本にしたもの。
  ⚠️★**注意書きを増やさない。** この罠は CLAUDE.md に2度書いたうえで**3回踏んだ**
     （2026-08-12 / 2026-09-10 / 2026-09-11）。**文章では防げないので手数を減らす方向で解決した。**
  なぜ要るか: `unstable_cache` が残っていると**「適用後の結果」を「適用前の測定値」として読む**
  （`/people` **1800秒** / 社員一覧 **300秒**）。**値を変えるたびに走らせる。**
  ⚠️ 本番は `.next` を消せない。タグを revalidate する経路が無ければ**TTL を待つ**
     （`directory-people` を `revalidateTag` する経路は現在0件）。
- **★カードの数値を DOM から拾うときは、拾った要素がその企業のものか確かめる**（2026-09-10 追記）。
  `.clc-stats` を親へ遡って企業リンクに結びつける方法は**隣のカードや
  フッターのリンクを拾って誤読する**。実際に「セールスフォースの募集中が0件」と
  読み違えた（正しくは2件）。**同時に出ている別の値（求人数など既知の値）で
  当たりを確かめてから**、目的の数値を読むこと。
- **★★状態の判定に文字列の包含（`includes` / `grep`）を使わない。`data` 属性と要素数で判定する**（2026-09-18 確立）。
  **2日で同じ形を2回踏んだ。** ①「未対応に戻す」を押した後に `innerText.includes("対応済み")` で
  判定し、**ボタン名「対応済みにする」に部分一致して「戻っていない」と誤読**（DB は正しく戻っていた）。
  ②「規約パネルが基本情報タブから消えたか」を `includes("掲載利用規約")` で見て、
  **サブナビの一言「掲載利用規約への同意が必要です」に部分一致して「まだ残っている」と誤読**。
  → **状態が変わる箇所には `data-state` を付けてある**（`/biz/employees` のカードとタブ、
  `/admin/member-reports` の行、`/biz/company` のサブナビ）。**装飾ではなく検証用。外さないこと。**
  ⚠️ 属性が無い箇所を測るなら、**完全一致か要素数**で見る（`querySelectorAll(...).length`、
  `(text.match(/^…$/gm) ?? []).length`）。⚠️ これは `.claude/rules/ui-debugging.md` ⑱
  「部分一致で結論を出さない」の**判定手段まで決めた版**。文章だけでは2回とも防げなかった。

### ⚠️★★`createAdminClient` で `ow_experiences` を引く企業側の画面は `visibility_company` を必ず見る

**RLS が効かない経路なので、条件を書き忘れると全件が実名で出る。**
**同じ問題を計3箇所で踏んでいる:**

| いつ | どこ | 症状 |
|---|---|---|
| 2026-08-13 | `/biz/employees` | 「会社名は伏せる」を選んだ人が**その勤務先の採用担当者には実名で見えていた** |
| 2026-09-10 | `/biz/candidates` | 同じ。**絞り込みとフリーワードでも当たっていた** |
| 2026-09-10 | `/api/biz/company/members` | 伏せた人の**氏名**がオフィス写真のタグ付け候補に並んでいた |

⚠️★**伏せた人が気にしているのは「社名が出ること」ではなく
   「転職を考えていると今の会社に知られること」**（2026-08-13 の判断）。
   だから**その企業の画面に出ること自体**が問題になる。
⚠️★**空欄にしないこと。** 企業側から「離職中」「未入力」に見え、別の不利益を与える。
   文言は **`MASKED_COMPANY_LABEL`（`lib/experiences/companyName.ts`）＝「非公開企業」**
   の1箇所だけ。`masked` と `hidden` で分けない。
   ⚠️★**2026-09-15 に、伏せる設定そのものを畳んだ**ので、この文言は**もう描画されない**
      （下の「✅★★会社名を伏せる設定は畳んだ」）。**ここの記述は経緯として残す。**
⚠️ **表示を伏せても絞り込みで当たれば意味が無い。** `/biz/candidates` は表示と
   フィルタが同じ値を読むのでサーバー側の置換1つで塞がる。
   **フィルタに社名の生値を渡す経路を新設しないこと。**

→ 36ファイルを数えた結果（該当2・非該当4）は
   [docs/visibility-company-biz-20260910.md](docs/visibility-company-biz-20260910.md)。
   **4箇所目を探すときに数え直さないこと。**
...
- **★`git push` はブランチ上の全コミットを送る（2026-08-22 に実際に起きた）。**
  **並行セッションがいる状態で main にコミットを置くと、
  ...
  ⚠️★★**push の直前に、送られる全コミットを1本ずつ読む（2026-09-16 に追加）。**

     ```bash
     git log --format="%h | %an | %s" origin/main..HEAD   # 何本あるか・誰のか
     git log --stat origin/main..HEAD                     # ★中身（変更ファイル）まで見る
     ```

     **1行目だけでは足りない。** 件名が `docs:` でも、その下にコードや migration の
     コミットがぶら下がっていることがある。**`--stat` でファイル名を必ず見ること。**

     ⚠️★**2026-09-16 に実際に踏んだ。** LP の改修を push したとき、自分の2本の下に
        別セッションの **`0871d356 feat(notify): メッセージが届いたらベルに出す`**
        （コード3ファイル ＋ migration 1本）が入っており、**確認を取らずに本番へ出した。**
        上の「コードまたは migration を含む → 確認を取る」は 2026-08-23 から
        書いてあったのに、**送られるコミットを数えていなかったので気づけなかった。**
        ⚠️ このときは結果として無害だった（migration は既に本番 DB へ適用済みで、
           コードと DB が整合していた）が、**それは運が良かっただけ。**

     ⚠️ 自分のものでないコミットがあったら、**push せずに報告する。**
        `git push` に「ここから上だけ」は無い。**送るか送らないかの二択。**
