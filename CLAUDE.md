...
| 職種 | **トップレベルのみ**（`parent_id is null` かつ `merged_into_id is null` かつ `is_active`）。**2026-08-28 実測で18件**（2026-08-10 は17件） |
...
⚠️ **運営の一覧が唯一の入口。** ここを見る人と頻度を決めておくこと（未決）。
   ⚠️ 「確認済み」を記録する列は無い。**新しい行かどうかは掲載日でしか分からない**
      （一覧は新しい順）。運用を決めるときは、この制約を前提にすること。
