...
### ⑥★★都道府県とフェーズは「0件の選択肢を出さない」の**例外**（2026-09-06 に柴さんが決定）
...
⚠️★**この例外はこの2つだけ。事業領域・職種には広げないこと。**
   あちらは「全体像が決まっている梯子・地図」ではなく、0件の選択肢を出す意味が薄い。
   `/companies` の事業領域は `getBusinessDomainFacets()` が0社のものを外している
   （「ITサービス・受託」が該当0社のまま出続け、押すと必ず0件だったのを直した経緯）。
...
⚠️★**`/mypage` のトグルのラベルは「面談対応可能」**（2026-09-12 / 柴さんの指示）。
   カードの見出しも「意思表示」→「**転職・面談の状況**」に変えた。
   ⚠️★**変えたのは本人の `/mypage` のこの2つだけ。** 訪問者向けの「話を聞けます」
      （`/people` のバッジ・企業ページの社員カード・`/u/[id]` のピル）と
      「この会社の話を聞ける人」は**変えていない**。
   ⚠️ 本人向けで「話を聞かれてもよいか」が残っているのは
      **招待メール（`templates.ts` の見出し）と `/mypage/ambassador-invite/[token]` の
      着地ページ**の2箇所。今回は触っていない。次に語を揃えるときはこの2つ。
   ⚠️ コード内の呼び名（`IntentCard` / `career_stance` / `aria-label="意思表示を編集"`）は
      変えていない。**画面の語とコードの語がずれている**（`?industry=` と同じ形で、承知のうえ）。
