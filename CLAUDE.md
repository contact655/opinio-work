...
### ⑥★★都道府県だけは「0件の選択肢を出さない」の**例外**（2026-09-06 に柴さんが決定）

**都道府県は 47件すべてを出す。** 該当が0社・0件の県も出す。

⚠️★**リポジトリの原則（0件の選択肢を出さない）とは逆向き。** それでもこうしたのは、
   実データから作ると `/jobs` の選択肢が**「東京都」1つ**になってしまい
   （公開求人2件がどちらも東京都）、絞り込みとして成立していなかったため。
   **入力欄（職歴・オンボーディング）と同じ見た目に揃える**ほうを優先した。

⚠️★**この例外は都道府県だけ。** フェーズ・事業領域・職種には広げないこと。
   それらは実データにあるものだけを出す（`fetchAvailablePhases` /
   `getBusinessDomainFacets` / `availablePhaseOptions`）。
