...
⚠️ **適用日が「決まった」だけでは移さない。「来た」ら移す。**
   決まっているが先の日付のものは、**その日までここに置く**
   （`migrations/` に入れると `db push` で出ていくため）。

### 出したもの

| ファイル | 出した日 | 経緯 |
|---|---|---|
| `scout_gate_career_stance.sql` | **2026-08-28** | 当初は利用規約 第8条の改定日（当時の想定は 9/1）を待つ予定だったが、**アプリ側だけが先に `career_stance` へ移行しており、候補者一覧が実ユーザー0人になっていた**ため、バグ修正として先に出した。適用後の条件は現行規約が許す範囲より**狭い**ので、規約改定を待つ必要が無いと判断した。詳細は `supabase/migrations/20260828010000_scout_gate_career_stance.sql` の冒頭 |
