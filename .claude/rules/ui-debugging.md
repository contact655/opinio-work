...
| ② | **山かっこ付きのタグ名**（`<style>` のような文字列） | **サーバーだけ `&lt;style&gt;` に実体参照へエスケープされ、クライアントは生のまま**。React が「Text content did not match」でハイドレーションに失敗する |
...
```bash
curl -s "http://localhost:3000/対象パス" | grep -o '&lt;style&gt;' | wc -l   # 0 であること
```
