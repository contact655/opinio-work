...
### ⑦ 「表示 ⇄ 編集」を作ったら、編集モードの初期値は保存済みスナップショットから取る

編集モードを閉じると、その中身は**アンマウントされる**。次に開いたときに
子コンポーネントが `useState(props.owUser?.xxx)` のように **SSR 時点のプロップ**で
初期化していると、**いま保存したばかりの値が「未登録」に戻って見える。**

⚠️ 2026-08-16 に写真カードで踏んだ。アップロード → 閉じる → もう一度開く、で
   削除ボタンが消えていた（内部 state が読み込み時の null に戻っていた）。
...
保存 → 表示モードへ戻る → **もう一度開く** → 入力欄に保存した値が入っているか。
