...
⚠️ **再発（2026-08-26）**: 「＋ 雇用形態を追加」で編集モーダルの該当項目まで
   スクロールさせる処理を rAF で書き、**90フレーム分ゼロ回**で一度も動かなかった。
   この節は 2026-08-16 に書かれていたのに、**同じ罠を踏んだ。**
   ⚠️ さらに `setTimeout` に替えても、**バックグラウンドタブでは 1秒に間引かれる**
      （60ms 間隔のつもりが約1秒になる）。**回数で期限を切ると想定より長く回る。**
...
### ⑪-2 ★ブラウザペインでは `getBoundingClientRect()` が実表示とずれる（2026-08-26 確立）

**「画面に見えているか」を rect で判定しないこと。スクリーンショットで判定する。**

⚠️ ⑨（`innerWidth` が 0 なら捨てる）**では捕まらない。**
   `innerWidth` / `visualViewport` は正しい値（1300×900）を返しているのに、
   rect だけが**約2.7倍**にスケールした値を返す状態を実測した。

実測（2026-08-26 / モーダルの中の `<select>`）:

| 見たもの | 値 |
|---|---|
| `innerWidth` / `innerHeight` | **1300 / 900**（正しい） |
| `getComputedStyle(モーダル)` | `position: fixed` / `inset: 0` / **`height: 900px`**（正しい） |
| `モーダル.getBoundingClientRect()` | **top 113 / height 2469**（合わない） |
| `select.getBoundingClientRect().top` | **1322** → 「画面外」と読める |
| **スクリーンショット** | **画面のほぼ中央に写っている**（＝実際は見えている） |

⚠️ **これを信じると「見えていないのに見えている」「見えているのに見えていない」の
   両方を誤判定する。** 実際に「スクロール処理が効いていない」と誤って読み、
   直す必要のないものを2回作り直した。

**判定の順番:**

1. **スクリーンショットを撮る**（これが正）
2. 補助として `document.elementFromPoint(x, y)` が目的の要素を返すか見る
   （rect 同士の比較と違い、**撮った絵の座標と突き合わせられる**）
3. rect は「**前後で動いたか**」の相対比較にだけ使う。絶対座標として使わない

⚠️ アニメーション中に測ると、そもそも値が確定していない。
   **`wait` を挟んでから撮る**（⑫と同じ理由）。
...
#### ★「欠けが無い」の確認は、要素の**集合**が一致するだけでは足りない（2026-08-26 追加）

**位置が正しいかまで見る。集合が同じで位置が違う不具合は、集計では原理的に検出できない。**

⚠️ **実例（2026-08-26）**: 年マーカーの欠けを調べるとき、ページから年を拾って
   「前 `[2025, 2021, 2017]` → 後 `[2025, 2021, 2017]`」と集計し、
   **木村さんは「変化なし・欠けなし」と報告した。**
   実際には**位置が誤っていた** —— 同社グループの一番上の役割は **2025-01** 始まりなのに、
   その真上のマーカーは **2021**（グループ内の最古の年）と出ていた。
   **年の集合は同じなので、何度集計しても出てこない。**

**集合を数えたら、次のどちらかで位置まで確かめる。**

| 何を | どう確かめる |
|---|---|
| 並び・対応関係 | **出現順に「何の直前／直後か」まで含めて並べる**（`[年] 会社 [年] 会社 …` の形） |
| 見た目 | **スクリーンショットを1枚撮る**（隣り合うものが目で見える） |

⚠️ 「件数が同じ」「集合が同じ」「サイズが同じ」はどれも**同じ弱さ**を持つ。
   **入れ替わり・付け替えは検出できない。**（同じ日に、開始日が同着の2社が
   実行ごとに入れ替わる件も、**サイズが同じだったので一度見逃している**）

#### ★★計測の対象に、**自分が書いた注意書き**を混ぜない（2026-08-31 追加）

**プレビューやデバッグ用の画面には「◯◯が出ていないこと」という注意文を自分で書く。**
その文言ごと本文を検索すると、**自分の注意書きに当たって「不具合あり」と読む。**

⚠️ 2026-08-31 に **同じ誤検知を1日で3回**やった。

| 探したもの | 当たった実体 |
|---|---|
| 「0名」が出ていないか | `51-200名` の**部分一致**と、自分が付けた社名「検証社員0名株式会社」 |
| `hybrid` の生値が出ていないか | 注記の「**『hybrid』がそのまま出ていないこと**」 |
| 年マーカーが降順か | ラベルの「**長期ブランク（2020〜2023 が空き）**」から拾った年 |

**3件とも実際の描画は正しかった。**

#### 守ること

1. **描画部分だけを対象にする。** `/dev/preview` は各バリエーションの
   白いカードに **`data-preview-body`** を付けてあるので、そこだけを読む。

   ```js
   [...document.querySelectorAll("[data-preview-body]")]   // ← 注記・見出しを含まない
   ```

2. **部分一致で結論を出さない。** `"0名"` は `51-200名` に当たる。
   行全体の完全一致（`/^\d+名$/`）か、**その項目の要素**を特定して読む。

3. **検出したら、実テキストを目で見る。** 件数だけで報告しない。
   上の3件はどれも、当たった箇所の前後を出した時点で誤検知と分かった。

#### ★「◯◯が無い」と報告する前に、grep か画面で確かめる（2026-08-26 追加）

**読んだ行範囲に無いことは、無いことの根拠にならない。**

⚠️ **実例（2026-08-26）**: `MergedTimeline.tsx` の並行職カードを調べ、
   「`career-group` の内側には鉛筆・ゴミ箱が無い」と報告した。
   **実際はあった。** `RowActionButtons` は**カードの右上に絶対配置**されており、
   `sed -n '980,1105p'` で読んだ範囲の**すぐ外**にあった。
   画面を1枚撮れば映っていたし、`grep -n RowActionButtons` を1回打てば分かった。

**不在を主張するのは、次のどちらかを実際にやってからにする。**

| 何を | どう確かめる |
|---|---|
| 部品・呼び出しが無い | **ファイル全体を grep する**（読んだ範囲ではなく） |
| 画面に出ていない | **その状態を作って1枚撮る**（コードを読むだけで言わない） |

⚠️ **削除する前にも同じことをする。** 「参照0件だから消してよい」と言う前に、
   リポジトリ全体（`src/` と `*.css` の両方）に grep を通すこと。

---

### ⑲ ★`<style>` のテンプレートリテラルに書いたコメントで、ハイドレーションが壊れる（2026-09-08 確立）

**CSS の中身ではなく、コメントに書いた文字だけで起きる。**
1日に2種類とも踏んだ。**どちらも `tsc` も lint も本番ビルドも通る。**

```tsx
<style>{`
  /* ⚠️ ここは <style> のテンプレートリテラルの中。 */   ← ★これが壊す
  .foo { color: red; }
`}</style>
```
...
| # | 何を書いたか | どうなるか |
|---|---|---|
| ① | **バッククォート**（`` ` ``） | テンプレートリテラルが**その場で閉じる**。以降が JS として解釈され、構文エラーか意味不明な差し込みになる |
| ② | **不等号**（`<` `>`）。⚠️★**タグ名でなくてよい** | **サーバーだけ実体参照へエスケープされ、クライアントは生のまま**。React が「Text content did not match」でハイドレーションに失敗する |

⚠️★**②の条件は 2026-09-09 に訂正した。** 当初「山かっこ付きのタグ名」と書いていたが、
   **裸の記号1文字で起きる。** 実際に踏んだのは CSS コメントの
   **「18px -&gt; 10px」という矢印のつもりの2文字**で、タグ名は1つも書いていなかった。
   → **矢印は「から」「へ」と日本語で書くか、`→` を使う。**

②の実際の出力（サーバー / クライアントの差分）:

```
Server: … ⚠️ ここは &lt;style&gt; のテンプレートリテラルの中。 …
Client: … ⚠️ ここは <style> のテンプレートリテラルの中。 …
```

⚠️ **失敗するのはその `<style>` だけではない。** 最も近い Suspense 境界ごと
   ハイドレーションが落ちるので、**その中のクライアント部品が丸ごと動かなくなる**
   （実際に「クリックの横取りが効かない」に化けた）。

**直し方は記号を書かないこと。** 「style タグの中」と日本語で書く。

#### 確かめ方

**配信された HTML を数える。** 画面を見ても分からない。

```bash
# ⚠️ タグ名だけを探さない。**エスケープされた不等号そのもの**を数える
curl -s "http://localhost:3000/対象パス" | grep -o '&gt;\|&lt;' | wc -l   # 0 であること
```

⚠️ この節は `<style>` に限らない。**JSX のテキストとして出る場所すべて**が対象
   （`<pre>` の中のコード例、注意書き、`/dev/preview` の説明文）。

---

### ⑳ ★`read_console_messages` は**古いバッファを返す**。直した直後の判定に使わない（2026-09-08 確立）

**同じタブで直して読み直しても、前の読み込みで出たエラーがそのまま出続ける。**
件数が増えないことに気づかないと、**直っていないと誤読する。**

⚠️ 2026-09-08 に実際に踏んだ。⑲の②を直して配信 HTML では
   `&lt;style&gt;` が **0件**になっているのに、コンソールは**古い文言のまま**の
   不一致エラーを返し続けた。総数が **6件のまま増えていない**ことが手がかりだった。

#### 手順

| やること | なぜ |
|---|---|
| **新しいタブで開き直す**（`tabs_create` → `navigate`） | バッファが空から始まる。**これが確実** |
| 出た件数を**前後で比べる** | 増えていなければ、それは今回のものではない |
| 併せて**配信 HTML を数える**（⑲） | コンソールに頼らず、肯定形で確かめられる |

⚠️ ⑨⑩と同じ形。**壊れている（ように見える）のは環境で、コードではない。**

---

### ㉑ ★`resize_window` は `resize` イベントを発火しない。JS で幅を見て出し分ける UI は切り替わらない（2026-09-17 確立）

**ビューポートの emulation を変えても、ページの `window.addEventListener("resize", …)` は呼ばれない。**
CSS のメディアクエリは効くので**見た目は変わる**が、
**JS で `window.innerWidth` を読んで出し分けている部分だけが前の幅のまま残る。**

⚠️ **そのまま測ると「リサイズしても切り替わらない」＝実装のバグ、と誤読する。**

実測（2026-09-17 / `/feed` の登録パネル。768px 以上は右レール・未満は本文の中）:

| 何をしたか | `innerWidth` | 測れたパネルの幅 | 実体 |
|---|---|---|---|
| 1024 → 375 に `resize_window`（**リロードなし**） | 375 | **340** | ❌ 右レール側のまま |
| 続けて `dispatchEvent(new Event("resize"))` | 375 | **343** | ✅ 本文側へ移った |
| 375 で `navigate`（**リロードあり**） | 375 | 343 | ✅ 正しい |

#### 手順

**幅で出し分ける JS がある画面は、次のどちらかで測る。**

```js
// ① リサイズ後に resize を手で発火させる（リロードせずに前後を比べたいとき）
window.dispatchEvent(new Event("resize"));
await new Promise((r) => setTimeout(r, 300));
```

**② その幅で `navigate` し直す**（＝マウント時の初期判定を通す）。**こちらが確実。**

⚠️ **①で「切り替わった」ことは、②でも確かめる。** ①はイベントだけを起こしているので、
   **マウント時の初期値が間違っていても気づけない**（SSR と初回描画の食い違いはこちら側に出る）。

⚠️ **実機では起きない。** 本物のブラウザのリサイズは `resize` を発火するし、
   スマホは境界をまたいでリサイズしない。**これは計測環境だけの話。**
