...
### ⑭ ★「外から開く nonce」は、受け側がアンマウントするかどうかで意味が変わる

`nonce` を +1 して受け側の `useEffect` で開く形（`openAddNonce` など）は、
**消費しても 0 に戻らない。** そのため受け側が**アンマウント→再マウント**すると、
**マウントした瞬間にもう一度発火する。**

```tsx
// ✗ 一度でも押したあとは、次にマウントされるたび開く
useEffect(() => { if (openAddNonce) setAdding(true); }, [openAddNonce]);
```

#### 実例（2026-08-16 / 2-5）

追加を1回使ったあと、**行の鉛筆やゴミ箱でカードを開くと追加フォームまで一緒に開いた。**
学歴・数値実績・受賞・メディア掲載の**4つとも**同じ形だった（受け側が
...
**「一度押す → 閉じる → 別の経路で同じカードを開く」を実際に踏む。**
同じ nonce でも、受け側が常に描かれていれば起きない。

| 受け側の置き場所 | アンマウント | 起きるか |
|---|---|---|
| `EditableSection` の `editContent` | **する** | **起きる** |
| カードの `children`（表示モードでも描かれる） | しない | 起きない |
| タブの中身（`display:none` で残す作り） | しない | 起きない |
