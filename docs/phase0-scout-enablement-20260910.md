...
- [x] ★★**本人が会社名の公開範囲を設定できること**（2026-09-11 に条件として追加 → **同日に実装**）
      統合（`visibility_company` 1本化）→ 入力欄（`/mypage/settings` の1設定）→ 6面で実測。
      ⚠️ 新しい職歴への引き継ぎも塞いだ（入口3つとも値を送っていて効いていなかった）
      → [visibility-company-two-columns-20260910.md](visibility-company-two-columns-20260910.md)
- [x] ★**「準備中」の文言をフラグに連動させた**（手で消す作業は無くなった）
- [x] ★**送信結果の記録**（`ow_scouts` の4列 ／ `/admin` の要対応 ／ `/biz/scouts` の表示 ／
      `RESEND_API_KEY` の送信時ガード ／ `/api/health` の `hasResendKey`）

⚠️★**ここが空になった＝あとは柴さんの判断と、当日に数え直すものだけ。**
