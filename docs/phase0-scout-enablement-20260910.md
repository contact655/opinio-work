...
- [ ] **最初に開ける企業を決める。** ⚠️★**サーバー側ゲートを入れたので、
      ...
      現在**有料は0社**
- [ ] **`ow_scout_quotas` を先に作るか決める。** ⚠️ 作らないと**実質30通**で開く
      （作り方・0にする方法は [scout-delivery-record-20260910.md](scout-delivery-record-20260910.md)）
- [ ] **止め方を決める** ── フラグを戻す／プランを `free` に戻す／枠を 0 にする、のどれで止めるか
- [ ] **1通目を `is_test` で通してから実ユーザーへ、という順序にするか**

#### 作業として残っているもの

- [ ] **④「企業に何が見えるか」を求職者本人に示す**（現在**無い**）
- [ ] ★**「準備中」の文言を消す箇所を洗い出して、フラグと同時に消す** ──
      LP の FAQ ／ `business/pricing` ／ `mypage/scouts/ScoutsClient` ／ `/biz/candidates` の案内。
      ⚠️ **開けたのに「準備中」と書いてある状態を作らない**
