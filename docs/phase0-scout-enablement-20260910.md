...
- [ ] **`visibility_company` が `masked` / `hidden` の経歴が0件か。**
      `/biz/candidates` はこの列を見ていないので、**0件でなくなった瞬間に実名が企業へ出る**
- [ ] **送れる相手の人数を数え直したか**（本調査時点で実ユーザー**5人**）
- [ ] **`ow_scout_quotas` を先に作ったか**（作らないと `monthly_limit` は既定の**30**）
- [ ] **`email_scout_enabled = true` の人数を確認したか**（現在38件中22件。**その人たちに実際にメールが飛ぶ**）
- [ ] **メールの本文と差出人を実物で確認したか**（本調査ではメールを**送っていない**。文面は未検証）
- [ ] **「準備中」の文言を消す箇所を洗い出したか** ── LP の FAQ ／ `business/pricing` ／
      `mypage/scouts/ScoutsClient` ／ `/biz/candidates` の案内。**フラグと同時に消す**
- [ ] **求職者に「企業に何が見えるか」を伝える画面を用意するか決めたか**（現在**無い**）
- [ ] **止め方を決めたか** ── フラグを戻す／プランを free に戻す／枠を0にする、のどれで止めるか
- [ ] **1通目を誰に送るか決めたか**（`is_test` で1通通してから実ユーザーへ、という順序にするか）
