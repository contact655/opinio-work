...
**作成日**: 2026-04-28
**更新日**: 2026-04-29（サービス化セット Commit B/F/C 完了後に同期）
**対象 commit**: A〜F + サービス化セット B/F/C（求職者側公開ページ群 + 認証・プロフィール）
**ステータス**: 🟡 8ページ実装完了、9ページ残存
...
| 状態 | 件数 |
|------|------|
| ✅ 実装済み（commits A〜F + サービス化セット） | 8 ページ |
| 🔲 未実装 | 9 ページ |
| **合計** | **17 ページ** |
...
| 5 | メンター | メンター一覧 | `/mentors` | `mentors` テーブル（`ow_` なし） | 中 |
| 6 | メンター | メンター予約 | `/mentors/[id]/reserve` | `mentors` + `ow_mentor_reservations` 新規 | 大 |
| 7 | 記事 | 記事一覧 | `/articles` | `ow_articles` 新規 | 中 |
| 8 | 記事 | 記事詳細（社員）| `/articles/[id]` | `ow_articles` type='employee' | 大 |
| 9 | 記事 | 記事詳細（メンター）| `/articles/[id]` | `ow_articles` type='mentor' | 大 |
| 10 | 記事 | 記事詳細（CEO）| `/articles/[id]` | `ow_articles` type='ceo' | 大 |
| 11 | 記事 | 記事詳細（取材レポート）| `/articles/[id]` | `ow_articles` type='report' | 大 |
