...
**作成日**: 2026-04-28
**更新日**: 2026-04-30（Commit DD: メール通知機能 T3/T4/T5 追加）
**対象 commit**: A〜F + サービス化セット B/F/C + D + V + AA/BB/CC/DD（求職者側公開ページ群 + 認証・プロフィール + 記事システム + DB 命名規則統一 + 企業詳細拡充 + メール通知）
...
| `ow_mentors` | メンター情報（migration 048 で `mentors` → `ow_mentors` にリネーム済み） | `id`, `name`, `avatar_initial`, `avatar_color`, `question_tags`, `is_available` |
...
### 6-5. `ow_mentors` テーブル（命名規約統一済み）

仕様書は `ow_mentors` を想定していたが、migration 008 時点のテーブル名は `mentors`（`ow_` なし）。  
Commit M-1 で `.from("mentors")` として実装後、**Commit V + migration 048 で `ow_mentors` にリネーム済み**。  
`getMentors()` / `getMentorById()` は `.from("ow_mentors")` を使用。全 8 TypeScript ファイル更新済み。

### 6-X+2. `ow_mentors` テーブルと `ow_users` の未連携（Commit M-2 で発見）

**発見**: Commit M-2 実装中（2026-04-30）

**問題**:
- `ow_mentors` テーブル（migration 008 → 048 でリネーム）は `ow_users` と独立しており、`user_id` カラムが存在しない
- 仕様書設計時の `ow_mentor_reservations.mentor_user_id NOT NULL REFERENCES ow_users(id)` が INSERT 不可能な状態だった（設計矛盾）

**対処（migration 045）**:
- `ow_mentor_reservations` に `mentor_id UUID REFERENCES ow_mentors(id)` カラムを追加（migration 048 リネーム後は自動追従）
- `mentor_user_id` を nullable 化
- INSERT 時は `mentor_id = ow_mentors.id`、`mentor_user_id = NULL` で記録

**現状の動作**:
- 求職者は予約 INSERT 可能（`user_id = ow_users.id`、`mentor_id = ow_mentors.id`）
- メンター本人による自分宛予約の閲覧は**未対応**（RLS の `mentor_read` ポリシーが `mentor_user_id` ベースのため、NULL では機能しない）
- `mentor_user_id` は NULL のまま、将来連携時に埋める設計

**将来の課題（M-5 候補）**:
- `ow_mentors` テーブルと `ow_users` の統合または明示的な連携（`ow_mentors.user_id UUID REFERENCES ow_users(id)` 追加）
- メンター本人による予約閲覧 + 承認フロー
- メンター登録フロー（現状は手動で `ow_mentors` テーブル直接操作、編集部が個別声がけ方針）

### 6-X+3. `ow_mentors` 命名規約（✅ Commit V で解決済み）

**発見**: Commit M-1 実装中（2026-04-30）  
**解決**: Commit V + migration 048（2026-04-29）

migration 008 で作成された `mentors` テーブルは `ow_` プレフィックスがなかった。  
**migration 048 で `ow_mentors` にリネームし、全アプリテーブルの命名規約を統一した。**
...
| Commit U | `17765ee` | UNIQUE 制約 (migration 047) + API 23505→409。S-T-1: 制約確認（23505 + constraint name 正確）。S-T-2: アプリ層 1回目→201、2回目→409。S-T-3: service_role 直接 INSERT → 409 + 23505 エラー（race condition 対応証明）。S-T-4: 既存ページ影響なし。S-T-5: cleanup 0件確認 | ✅ | S-T-1〜5 全 PASS |
...
| メール通知 | DD | — | `src/lib/notify/email.ts`（Resend wrapper + mock fallback）+ `src/lib/notify/templates.ts`（T3/T4/T5 テンプレート）+ API 3本修正。T3: カジュアル面談申込 → admin+user。T4: 面談 status 変更（company_contacted/scheduled/declined）→ user。T5: メンター予約申込 → admin+user。RESEND_API_KEY 未設定時は console.log mock。S-DD-1〜6 全 PASS |
...
| — | `ow_mentors` と `ow_users` の連携設計 + メンター本人の予約閲覧 + 承認フロー | `ow_mentors` テーブルは `ow_users` と未連携のため、メンター本人ログイン → 受信予約管理が未実装。§6-X+2 参照 | 大 | 設計議論必要 |
| ✅ 完了 | `mentors` テーブルを `ow_mentors` にリネームする migration | Commit V + migration 048 で完了。命名規約（`ow_` prefix）統一。全 8 TypeScript ファイル更新済み | — | — |
...
| — | メール通知 T1（求人応募）/ T2（応募 status 変更）実装 | T1: `POST /api/applications` に notify 追加（旧 `apply/notify` との統合要検討）。T2: `PATCH /api/biz/applications/[id]` に notify 追加（interview/offer/rejected 時に応募者へ通知） | 小 | Phase 5 Stage 3 後が自然 |
...
WHERE table_name = 'ow_mentors' ORDER BY ordinal_position;
```
