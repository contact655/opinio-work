...
### 6-3. `/biz` にログアウトボタンが存在しない

| 項目 | 内容 |
|------|------|
| 影響 | ブラウザから企業側ダッシュボードをログアウトできない（テスト担当者の切り替えに支障） |
| 回避策 | Supabase Dashboard の Auth 画面からセッション削除、または Incognito モード利用 |
| 状態 | 開発中は許容できるが、リリース前には必須 |
| 修正方針 | Phase ν-5 A-1 スコープ（BusinessLayout のヘッダーに追加、数行の変更） |
...
| ID | 内容 | 推定工数 |
|----|------|---------|
| **A-1** | `/biz` ログアウト機能の UI 実装（BusinessLayout ヘッダーにボタン追加） | 30 分 |
| **A-2** | 候補者側マイページの UI 統一（`/mypage/conversations` を `/biz/conversations` と同等品質に。系統 A/B の分断解消） | 2〜3 時間 |

> **A-1 は最初の 30 分で終わる**。次セッション冒頭に片付けておくと担当者アカウント切り替えが格段に楽になる。
...
これらは「自分が見えない / 相手が見えない」という共通テーマを持つ。A-1（ログアウトボタン）は独立した小タスクで、テーマと関係なく即着手できる。
...
#### ステップ 1: A-1 ログアウトボタン（30 分）

```
対象ファイル: src/components/business/BusinessLayout.tsx
変更内容: ヘッダー右上に「ログアウト」ボタンを追加
実装: supabase.auth.signOut() + router.push('/biz/auth')
テスト: 担当者_001 / _005 の切り替えが UI からできることを確認
```

> これ1つで担当者アカウント切り替えの煩雑さが解消し、以降の開発効率が上がる。
