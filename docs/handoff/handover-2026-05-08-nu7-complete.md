...
- `src/app/api/save-job/route.ts`（既存 API、参照元なし）の活用可能性
...
| 1.5 | 3 | サイドバー整理 + 型整理 |
| 2 | 2 | conversations MypageLayout 移行 |
| 3 | 2 | conversations/[id] MypageLayout 移行 |
| 4 | 3 | applications MypageLayout 移行 + 応募管理サイドバー追加 |
| 5 | 0 | ブックマーク概念統一（残骸 0件確認 + ν-8 候補2 記録） |
| 6 | 4 | 宙ぶらりん4件削除（1,712行）+ Header マイページリンク修正 |
| 7 | 2 | /profile/edit を MypageLayout でラップ（ν-7 最終段階） |
| **合計** | **21** | **ν-7 完全クロージング** |

### ν-7 完全クロージング宣言

Phase ν-7「/mypage 世界観統一 — レイアウトの分裂を癒す」、全7段階＋1.5 が完了。
...
- ブックマーク概念統一（残骸 0件確認）
- ν-8 候補（職歴の立体化 / 求人ブックマーク）を記録し次フェーズへ

**Hisato さんの動作確認 OK が出次第、正式クロージング確定。**
