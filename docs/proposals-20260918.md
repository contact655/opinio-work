...
## 5. 実データでどこまで出たか（★**2026-09-18 午前の実測**）

⚠️★**この章の数字はすべて 2026-09-18 午前のもの。同じ日の夕方に動いた。**
   最新は **§8 数字の更新** を見ること。**日付の無い数字は翌日には嘘になる。**
...
| **根拠の材料が1つでもある企業** | **3社** |
| **根拠が2件以上そろう企業** | ★**0社** |
| 入社の決め手（実ユーザー） | **0件**（非空5行はすべて `is_test`）★**午前の値。夕方に3件になった → §8** |
...
## 8. ★数字の更新（2026-09-18 夕方 実測）

**同じ日のうちに実データが動いた。** §5 の数字は**午前の値**で、すでに古い。

| | 午前 | **夕方** |
|---|---|---|
| 実ユーザー（`is_test=false` / `is_system=false` / `auth_id` あり） | 7 | **8** |
| `ow_experiences` | 37 | **41** |
| **決め手（`join_reasons` 非空）／実ユーザー** | **0** | **3** |
| `leave_reasons` | 0 | **1** |
| `join_reason_primary` | 0 | **2** |
| `ow_experience_gaps` | 1 | **19** |
| **決め手が1件以上ある掲載企業** | **0社** | **1社**（セールスフォース） |
| 決め手が3件以上ある掲載企業（比率が出せる） | 0社 | **0社** |

★**新しく登録した実ユーザー1名が、既存の入力UIから3件回答した。**

⚠️★**「入力する画面が無いから0件だった」ではない。** 入力UI
（[ExperienceReasonModal](../src/components/profile/editor/ExperienceReasonModal.tsx)）は
**2026-09-12 から `/mypage` にあり、動いている。**

⚠️★**次に数え直す人は、上書きせず行を足すこと。** いつ何が動いたかが読めなくなる。
...
⚠️ ただし **0件だった理由は「入力UIが無いから」ではない**（2026-09-18 夕方に判明）。
   入力UIは 2026-09-12 から `/mypage` にあり、**動いている。** 職歴を持つ既存の4人が全員、
   **設問が出た日より前が最終ログイン**で一度も見ていなかっただけ。
   → [evidence-gaps-20260918.md](evidence-gaps-20260918.md)
**器を増やすより、既に居る人に届ける導線のほうが効く。**
