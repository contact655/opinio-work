...
## ★ロゴが OGP画像のままの10社（2026-08-28 現在・12社は解消済み）

掲載79社のうち **22社のロゴが縦横比 1.91:1 ＝ OGP画像**だった。
`3f90b192`（7社）・`f199f4a9`（4社）・ミラクル1社で **12社を差し替え、残り10社**。

### ⚠️★まず前提: 残り10社は「使えないほど酷い」わけではない

**2026-08-28 に11社を68px枠に並べて目視した結果、当初の
「マーケティング文言が焼き込まれていて判読できない」は言い過ぎだった。**

| | |
|---|---|
| **68px でも読める** | Salesforce（雲）/ CISCO / SAP Concur / braze / okta / workday / MIRAKL |
| **読みづらい** | MongoDB / Translead / クアルコム |

実際には**中央にロゴを置いた単色背景**のものが多い。
→ **残り10社の差し替えは急ぐ作業ではない。**
   本当の問題は判読性より**不揃い**（白背景の正方形と色付きの横長が混在する）。
...
| オクタ | 57px | 同上 |
| ブレイズ | 48px | 同上 |
| **セールスフォース** | **32px** | 同上。⚠️ 唯一の公開求人がある企業 |
| ワークデイ | 32px | 同上 |
| Translead | 32px | 同上 |
| シスコ | 16px | 同上 |
| **クアルコム** | **無い** | 512/192px は**中身が React のロゴ**（下記）。他は 32px の favicon のみ |
| シンカ | — | **公式サイトが 503**（2026-08-28 に2回確認）。時間を置いて再試行 |

### ★しきい値は 150px（2026-08-28 に 180px から変更）

`CompanyLogo` の最大表示は **96px**（`size={96}` が1箇所）。padding が `px * 0.1`
なので画像領域は約77px、**2x DPI で必要なのは約154px**。

### ★★機械判定だけでは防げなかった2件（実際に踏んだ）

⚠️ **クアルコム**: `.../clientlib-react/resources/logo512.png` が 512x512 で取れたが、
   **中身は React のロゴ**（Create React App の既定画像）。寸法・形式・正方形判定を
   すべて通っていた。**そのまま入れると他社のロゴを掲載していた。**
⚠️ **ミラクル**: SVG を `qlmanage` でラスタライズしたら**ほぼ空白**になった。
   ✅ **原因が分かって解消済み** → `<style>` の中に `path { fill }` があり、
      **qlmanage は SVG 内の CSS を適用しない**（root は `fill="none"`）。
      **fill を path に直接書いてから**ラスタライズすると出る。
      さらに qlmanage は**白背景を焼き込む**ので、四隅から flood fill で外側だけ
      透明にした（円の内側の白い部分は残す）。実装は `upload-logos-20260828b.mjs`。
...
1. 画像を先に Storage へ入れる（`scripts/upload-logos-20260828b.mjs` を複製）
   ⚠️ **Web App Manifest も見る**（192/512px が載る）
   ⚠️ **node の fetch を使う**。Python の urllib は 403 / TLS で落ちるサイトがある。
      **ブラウザならさらに通る**（Databricks は Python 403 → node 200）
2. **110px 枠に並べて目視で自社ロゴであることを確認する**
3. migration で `logo_url` を書く
4. ⚠️ **確認は DB の `logo_url` を実際に取得して測る。** キーを組み立てて測ると、
   **DB が別の名前を指していても気づけない**（`.jpg` 2社と `.webp` 1社を実際に踏んだ）
