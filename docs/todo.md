...
## /mypage のモバイルで残っているもの（2026-08-16 記録・未着手）

375px の実測から。**フェーズ1で直したのはヘッダーCSS・タップ対象・右カラムの3つだけ。**

### ① 空カード7枚が縦に積まれる（設計判断）

| | 高さ | 375×812 の画面数 |
|---|---|---|
| データがある人 | 4,789px | 5.9画面 |
| **空の人** | 3,437px | **4.2画面** |

⚠️ **入力が1つも無い人がいちばん長いスクロールを強いられる**構造。
   「まだ登録していません」のカードが 159〜261px ずつ積まれるため。
   薄くするのが打ち手だが、**空セクションを出すこと自体が入力の入口**なので設計判断。

### ② 内側ナビ（`/u/[id]` にはある）

データがある人（5.9画面）には効くが、**①の空の人（4.2画面）には効かない**
（飛び先が全部空のため）。①と一緒に考える。

### ~~③ ヘッダーの編集フォームが 1,670px~~（2026-08-16 対応済み）

写真・カバー（約380px）と SNS 7欄（約430px）を**既定で閉じた**。
カードは **1,670 → 966px**、保存ボタンまで 1,557 → 854px。
閉じても分かるよう、行の右に状態を出す（「3件」「未設定」など）。開閉は保存しない。
⚠️ SNS の入力欄が 157px しかなく **URL の末尾しか見えない**のは**未対応**。

### ④ モバイルのタブバーが横スクロール

中身 502px / 枠 375px。`overflow-x: auto` なので破綻はしないが、
**初期表示で「ブックマーク」が完全に隠れる**（5項目中4.5個ぶんしか見えない）。

### ⑤ 文中のテキストリンク7件が高さ36px

「まだ〜していません。**〇〇を追加する**」の形。幅は 67〜162px あるので面積は足りる。
段落の行の中にあるので、高さを 44px にすると行間が崩れる。**今回は触っていない。**
...
## /u/[id] の「本人向け CTA」をどうするか（2026-08-16 記録・未着手）

`/u/[id]` を**本人が見ると**、空の自己紹介に点線の CTA（「自己紹介を書いて、
あなたのことを伝えましょう → プロフィールを編集する」）が出る
（`ProfileAboutSection` の `viewerIsOwner` 分岐）。

一方 `/mypage` は 2026-08-16 に**空のセクションを出さず「セクションを追加」に集約**した。
その結果、**本人向けの入口の出方が2ページで違う**。

⚠️ **編集の場所を `/mypage` に寄せた以上、一度決める価値がある。** 案は2つ。

| 案 | 内容 | 気になる点 |
|---|---|---|
| A | `/u/[id]` の本人向け CTA を消す（他人と同じ見え方にする） | 公開ページから編集へ戻る導線が「プロフィールを編集」ボタン1つになる |
| B | 残す | 同じ「空の自己紹介」に対して、`/mypage` は出さず `/u/[id]` は出す状態が続く |

⚠️ 自己紹介以外にも `viewerIsOwner` で本人にだけ出しているものがある
   （発信コンテンツの空状態など）。**決めるときは横断で数えること。**
