...
### 柴さん名義の4アカウント（2026-08-26 実測・件数のみ）

**本命を決めるときはこの表だけ見れば足りる。**

| アカウント | `is_test` | 職歴 | 所属 | 学歴 | 投稿 | profiles | auth | 最終ログイン |
|---|---|---|---|---|---|---|---|---|
| `hshiba@opinio.co.jp` | **true** | 0 | 0 | 0 | 0 | 1 | ✓ | **2026-08-26** |
| `s.hisato1020@gmail.com` | **false** | 0 | **1** | **2** | 0 | 1 | ✓ | 2026-08-23 |
| `hshiba+01@third-box.jp` | true | 0 | 0 | **1** | 0 | 0 | ✓ | 2026-08-07 |
| `hshiba+03@third-box.jp`（「山田 太郎」名義） | true | 0 | 0 | 0 | 0 | 0 | ✓ | 2026-08-07 |

列: 職歴=`ow_experiences` / 所属=`ow_company_members` / 学歴=`ow_user_educations` /
投稿=`ow_posts` / profiles=`ow_profiles`。
`auth` は `auth.users` に行があり `email_confirmed_at` あり・`banned_until` なし
＝ **4つとも今ログインできる**。

⚠️ **4つとも職歴（`ow_experiences`）が0件。** 「実体があるほう」と言っても、
   `s.hisato1020@gmail.com` が持っているのは**所属1件と学歴2件だけ**。
   移す中身は少ないので、寄せる方向の選択肢は狭くない。
⚠️ **最終ログインが最も新しいのは `hshiba@opinio.co.jp`**（`is_test` が立っているほう）。
   「運用で使っている」の裏付け。
⚠️ `/people` に出ているカードの所属が**学歴（獨協大学）**になっているのは、
   `ow_company_members` の1件（**株式会社Opinio**）が
   `display_consent = false` / `is_public = false` だから（実測）。
   **所属1件は公開されていない。**

| アカウント | 位置づけ |
|---|---|
| `hshiba@opinio.co.jp` | **運用でログインに使っている**ほう |
| `s.hisato1020@gmail.com` | **`/people` に出ている実体** |
...
⚠️ `hshiba+01` / `hshiba+03` も**ログインできる状態のまま**残っている。
   本命を決めるときは、この2つをどうするかも一緒に決めること
   （とくに `+03` は「山田 太郎」名義で中身ゼロ）。
...
## 語学が `/search` の語彙にならない（`ow_user_languages.name` が自由入力）

**対象**: `ow_user_languages`（2026-08-24 新設）。

`name` は **text の自由入力でマスタ参照ではない**（`proficiency` だけが5値の CHECK）。
そのため **「英語が話せる人」を `/search` で引けない。**
`/search` の解決先はマスタの ID に解決できる語だけで、自由入力は条件にならないため
（`lib/search/interpretQuery.ts` の約束①②）。

⚠️ スキルの標準マスタ（`ow_skills`）には**語学を入れていない**。
   習熟度を持つ `ow_user_languages` と重ねると入力欄が2つになるため
   （docs/phase0-skills-20260827.md §6）。**その判断は変えなくてよい。**

**やること**: 語学を検索の語彙にするなら、**言語側にマスタか正規化が要る**。
   選択肢は2つ。どちらも `ow_skills` に語学を足すことでは解決しない。
     (a) 言語マスタを作り `ow_user_languages.name` を FK にする
     (b) `name` に CHECK を張って値を閉じる（ISO 639 の主要言語など）

⚠️ 実データは **0行**（2026-08-27 実測）なので、いま移行のコストは無い。
   入力が溜まってからだと表記ゆれ（「英語」「English」「英語（ビジネス）」）の
   名寄せが要る。**やるなら早いほうが安い。**
...
**`user_id` をリクエスト本文から受け取る形にしないこと。**

RLS をバイパスしているので、**本文の `user_id` をそのまま使うと他人の行を
書き換えられる。** いま守っているのは
「`user_id` はセッションから解決した値だけを使い、全クエリに
`.eq("user_id", owUserId)` を付けている」という**アプリ側の実装だけ**で、
DB 側の防御は効いていない。

⚠️ 実測（2026-08-27）: 本文に他人の `user_id` を入れても自分の行にしかならず、
   他人の行は DELETE できない。**この性質はコードでしか担保されていない。**
...
⚠️ **この節はもともと `contact+26` の1件しか書いていなかったが、実際は12件あった。**
   `contact+01`〜`+15` は全て `is_test = true` なのに、**`+16` でぴったり途切れていた。**

### ⚠️ 実害が出ていた（2026-08-26 実測）

**セールスフォース・ジャパン（`listing_status = 'listed'`）の企業ページに、
ログイン済みの訪問者から3件が実在の人物として見えていた。**

| 区分 | 名前 | 実体 |
|---|---|---|
| 現役社員 | 山田 三郎 | `contact+17` |
| 現役社員 | **相川 隆二** | `contact+27`。**2026-08-25 に面談対応者として掲載され、「話を聞ける人」として出ていた** |
| OB/OG | 山田 25 | `contact+25` |

⚠️ **未ログインには漏れていない。** 全員 `visibility = 'login_only'` で、
   本番の `/companies/salesforce` には人数しか出ていなかった（実測: 氏名0件）。
...
|---|---|---|
| `is_test = true` | 20 | **32** |
| `is_test = false` | 18 | **6**（福永・大塚・木村・柴＝実ユーザー4／生藤／system） |
| `contact+NN` の未フラグ | 12 | **0** |
| SF の現役社員 | 山田 三郎 / 生藤 / 相川 | **生藤 のみ** |
| SF の OB/OG | 木村 / 山田 25 | **木村 のみ** |
| SF の面談対応者 | 2 | **1** |

**コード変更は不要だった。** `getCompanyEmployees` / `getPublicAmbassadorsCached` は
`is_test === true || visibility === 'private'` を除外している。

⚠️ **反映に最大2分かかる。** `getCompanyEmployeesCached` は `revalidate: 120`、
   面談対応者は `revalidate: 60`。**DB を直した直後に画面を見て「効いていない」と
   判断しないこと**（2026-08-26 に実際に一度そう読んだ）。

⚠️ **行は消していない。** `ow_users` を参照する FK 45列のうち **29列が ON DELETE CASCADE**。
   auth ユーザーを消すと投稿・職歴・会話・ブックマークまで連鎖して消え、**エラーは出ない**
   （CLAUDE.md「本番で検証用アカウントを作らない」）。

### ★残っている運用の宿題

⚠️ **今後テスト登録するたびに同じ宿題が増える。作ったら都度立てる。**
   1件だけ書いて放置すると、今回のように**12件に増えて企業ページに漏れる**。
   `contact+28` 以降を作ったら、その場で `is_test` を立てること。

⚠️ **`hiroki.ikuto.placeholder@opinio.co.jp`（生藤 弘樹）は対象外にした。**
   メールに `placeholder` とあるが実在の方で、セールスフォースの面談対応者として
   掲載されている。**テスト扱いにしないこと。**
...
## ~~「将来やりたいこと」が誰にも見えない~~（2026-08-16 対応 → **2026-08-23 に UI ごと廃止**）

⚠️ **この節は決着済みの記録。復活させないこと。** 2026-08-23 に
「目指す姿・ありたい未来」を **UI ごと撤去**した（柴さんの判断）。
`FutureSectionEditor.tsx` を削除し、`MergedTimeline` の `future` / `viewerIsOwner`、
`buildFutureData`、`PUT /api/jobseeker/profile` の `future_aspirations` 受け口も外した。

⚠️ **`ow_users.future_aspirations` の列とデータは残してある**（列は消さない方針）。
   本文が入っていたのは実ユーザー1人（38人中）。**読む経路も書く経路も無い死列**になった。
