...
## ★ロゴが OGP画像のままの15社（2026-08-28 記録・7社は解消済み）

掲載79社のうち **22社のロゴが縦横比 1.91:1 ＝ OGP画像**だった。
`3f90b192` で**7社を公式アイコンに差し替え、残り15社**。

| 社数 | 状況 | 次にやること |
|---|---|---|
| **6社** | 公式が 48〜156px の正方形アイコンを持つ<br>Okta(57) / Braze(48) / MongoDB(64) / Concur(96) / Zendesk(152) / Kyriba(156) | **ブランドキットか Wikimedia を当たる。** 180px 未満は採らない基準を維持するなら別ソースが要る |
| **5社** | 公式が 16〜32px の favicon しか出していない<br>Salesforce / Cisco / Qualcomm / Workday / Translead | 同上。⚠️ **Salesforce は唯一の公開求人がある企業**で優先度が高い |
| **4社** | サイトが取得できなかった<br>Databricks(403) / Mirakl / シンカ(503) / タイミー(TLS) | ブラウザで手動取得するか、時間を置いて再試行 |

⚠️ **推測で別の画像を当てないこと。** OGP画像のままのほうが「違うロゴ」よりまし。
⚠️ **180px 未満を採らない**基準は 2026-08-25（36px を見送り）から一貫している。

### 手順（`3f90b192` でやった形）

1. 画像を先に Storage へ入れる（`scripts/upload-logos-20260828.mjs` を複製）
2. **目視で自社ロゴであることを確認する**（110px 枠に並べて見る）
3. migration で `logo_url` を書く
4. ⚠️ **確認は DB の `logo_url` を実際に取得して測る。** Storage のキーを
   組み立てて測ると、**DB が別の名前を指していても気づけない**
   （掲載77社中 **9社が `logo.jpg`・1社が別名**。実際に一度誤認した）
