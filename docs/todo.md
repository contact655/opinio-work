...
⚠️ **①は放置すると増える。** 直すなら「新しいファイルを上げたあとに古いほうを消す」だが、
   **削除が失敗してもアップロード自体は成功させる**設計にすること
   （消せなかったせいでロゴが変えられない、のほうが害が大きい）。
   ⚠️ あるいは **`logo.png` 固定名にして upsert する**（一括投入スクリプトと同じ形）。
      こちらなら孤児が原理的に生まれない。**そちらのほうが筋が良い。**
