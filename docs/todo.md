...
## ⚠️★★2026-09-01 にやること ── 規約 第8条の改定とスカウト可否の付け替え

**改定日は 2026-09-01**（柴さんより。2026-08-28 に確定）。

### ① 規約 第8条が、いまの製品と食い違っている（**2026-08-27 から**）

⚠️ **これは 9/1 を待つ話ではない。すでにズレている。**
...
⚠️ 実害が出ていないのは**スカウト送信が止まっている**ため
   （`SCOUT_SENDING_ENABLED` 未設定）。**止めているものが外れた日に効く。**

→ `content/legal/terms-of-service-jobseeker.md` の第8条第1項・第3項を改定する。
   ⚠️ **法務文書なので、文面は柴さんが決める。** こちらで書き換えていない。

### ② スカウト可否の DB 関数を付け替える

`can_send_scout` の条件1が **`scout_enabled` のまま**。アプリ側は既に
`career_stance` で動いており、**DB だけが古い。**

**用意してある**: `supabase/pending/scout_gate_career_stance.sql`

⚠️ **`supabase/migrations/` に置いていない。** `db push` は保留分を全部当てるので、
   置いた瞬間に他の人の push で出ていく（**2026-08-26 に実際に踏んだ**）。

#### 9/1 の手順（この順で）

1. **規約の改定が公開されていることを確認する**（①）
2. `supabase/pending/scout_gate_career_stance.sql` を
   `supabase/migrations/YYYYMMDDHHMMSS_scout_gate_career_stance.sql` へ**移動**し、
   その日の日時で採番し直す
3. `npx supabase migration list` で**保留が自分のものだけ**か確認してから `db push`
4. 適用後、`can_send_scout` が `career_stance` を見ていることを実測する

#### 実測（2026-08-28・付け替えの影響）

送れる実ユーザーは **1人 → 2人**（減るのではなく入れ替わる）。
