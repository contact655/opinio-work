...
### ★確認が必要（ダッシュボードでしか見られない）

1. **Database → Backups** … 日次バックアップの有無・**保持期間**
2. 同画面の **Point-in-Time Recovery** … 有効か、有効なら**遡れる範囲**
3. 無効なら、いまのプランで有効化できるか（PITR は有料アドオン）
...
- **PITR を入れるか**（入れるなら費用と遡及範囲）
- 入れない場合の代替（例: `pg_dump` を定期取得して別ストレージへ）
- ⚠️ どちらも決めないと、**いまは「壊したら戻せない」状態**である
