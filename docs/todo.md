...
## ★リリース前に必須：テスト用アカウント `contact+26` に `is_test` を立てる（2026-08-21 記録）

**対象**: `ow_users` の「山田 25」（`auth.users.email = contact+26@opinio.co.jp`）。
2026-08-21 に**新規登録の経路をひととおり通すために作った**アカウント。

**なぜ残っているか**: 検証を続ける可能性があるため、その時点では立てずに残した。
⚠️ **`is_test = false` のままなので、いまも実ユーザーとして扱われている。**

    ユーザー一覧（/people）の表示条件  → 満たしている（一覧に出る17名のうち1名）
    週次メールの宛先                  → 入っている（email_weekly_enabled = true）
    職歴                              → 0件（オンボーディング未完了）

**やるなら**: 1行の UPDATE だけ。

```sql
update ow_users o set is_test = true
from auth.users u
where u.id = o.auth_id and u.email = 'contact+26@opinio.co.jp';
```

⚠️ **消さないこと。** `ow_users` を参照する FK 45列のうち **29列が ON DELETE CASCADE**。
   auth ユーザーを消すと投稿・職歴・会話・ブックマークまで連鎖して消え、**エラーは出ない**
   （CLAUDE.md「本番で検証用アカウントを作らない」）。

⚠️ 同じ理由で、**今後テスト登録するたびにこの宿題が増える**。作ったら都度立てる。
