...
## 📋 棚卸しの記録（2026-08-27 実施）

全42項目を実測で突き合わせた。**結果は報告済みで、削除は項目ごとに柴さんが判断する。**
ここに残すのは「**見出しを読むと事実と違うもの**」だけ。中身まで読めば分かるが、
見出ししか見ない人を誤らせるため。

| 項目 | 見出しの記述 | 実測（2026-08-27） |
|---|---|---|
| `★user_id の空間取り違え` | 「**1件は未修正**」 | **未修正は0件。** `get_blocked_companies` は `p_auth_user_id` ＋ `ow_users` join に修正済み（本文中では済みと書かれている） |
| `★StoryAccordion が事故で孤児` | 「孤児になっている・要判断」 | **孤児ではない。** `mypage/details/[section]/CareerDetails.tsx` が import して描いている（`e2208179` で復帰） |
| `候補者一覧に出ない実ユーザー4人` | 「職歴がある実ユーザー**9人**のうち」 | **いま4人**（`is_test` の付け直しで母数が変わった）。`ow_profiles` が無い2人は変わらず |

⚠️ **この表は棚卸しの結果であって、宿題ではない。** 上の3項目を整理したら、
   この節ごと消してよい。

⚠️ **数字を含む項目は古くなる。** 件数を根拠に判断する前に、必ず測り直すこと
   （今回も「9人」が「4人」になっていた）。
