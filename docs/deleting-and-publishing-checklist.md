...
**公開後に埋めても、生成済みのフィード投稿の本文は変わらない。**
投稿は部分UNIQUEインデックスで冪等になっており、一度作られると作り直されないため。
つまり `company_joined` の本文は**公開した瞬間の値で固定される**。

| カラム | 空だとどうなるか |
|---|---|
| `brand_name` | 正式名がそのまま投稿本文に入る（「Notion Labs Japan合同会社がOPINIOに掲載されました。」） |
| `tagline` | 「〜がOPINIOに掲載されました。」で終わる、中身のない投稿になる |
| `logo_letter` | 社名の頭文字でフォールバックするので実害は無いが、揃えるなら入れる |
