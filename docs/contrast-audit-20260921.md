...
### ⚠️ 残っているもの ── メール2本の CTA

`api/cron/weekly-match/route.ts:235` ／ `api/cron/weekly-jobs/route.ts:220` の
`background:#059669;color:#fff` は **3.77 のまま**。**白文字なので基準を割っている。**

⚠️★**CSS 変数が効かない**（メールHTML）ので `#047857` の直書きになる。
   globals.css の注記が挙げている「hex で書いてよい例外」がまさにここ。
⚠️ **配信は停止中**（`WEEKLY_EMAIL_ENABLED` 未設定 ＋ `vercel.json` の `crons` が空）なので、
   **今回は触っていない。** 再開するときに直すか、単独で直すかは柴さんの判断。
