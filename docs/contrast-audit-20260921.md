...
## ✅④ ドット ── **黄色の5つは濃い方へ揃えた。緑は対象外**（2026-09-21 追記）

⚠️★**ここは当初「文字が乗っていないので対象外」と書いた。その判断は緑にしか当てはまらない。**

| | 色 | どの背景でも | |
|---|---|---|---|
| 緑のドット | `--success` #059669 | 白 **3.77** | ✅ 3:1（WCAG 1.4.11）を満たす → **触らない** |
| **黄色のドット** | `--warm` #F59E0B | 白 **2.15** ／ `--warm-soft` **1.93** | ❌ **どこに置いても足りない** |

→ **黄色いドット5つを `--warm-strong`（#92400E）へ揃えた**（白の上 7.09 ／ 淡黄帯の上 6.37）。

| 場所 | 前 | 隣にテキスト |
|---|---|---|
| `CompanyEditSubNav.tsx:161`（未公開の変更あり） | `--warm` 1.93 | あり |
| `CompanyEditSubNav.tsx:227`（対応が必要です） | `--warm-ink` 7.09 | ★**なし**（`role="img"`＋`aria-label` だけ） |
| `admin/page.tsx:382`（要対応タスク） | `#F59E0B` 1.93 | あり |
| `AdminCompaniesClient.tsx:411`（順序を保存中） | `--warm` 2.15 | あり |
| `admin/biz-accounts/page.tsx:361`（未ログイン） | `#F59E0B` 1.93 | あり |

⚠️★**揃える向きは「濃い方」しかない。** 227 だけが**ドット単独で情報を担っている**ので
   3:1 が必須で、そこに合わせる以外の選択肢がない（明るい方に揃えると 7.09 → 2.15 の後退）。
⚠️ **リング（`boxShadow` の `rgba(245,158,11,…)`）は明るいまま。** 装飾なので基準の対象外で、
   **黄色という合図はリングと帯が担っている。**

### ⚠️★★揃えなかったドットが1つある。**揃えにいかないこと**

`CompanyEmployeeSections` の掲載バナーのドット（`c.tone`）は **`--warm` のまま**。

**緑＝掲載中 / 橙＝ログイン限定 / 灰＝非掲載** を `tone` 1つで切り替える作りなので、
**橙だけ濃くすると3色の濃さが揃わなくなる**（緑は `--success-soft` の上で 3.58 ＝ 基準内）。
⚠️ 2026-09-02 に「`tone`（明るい方）はドットと枠のまま。濃さだけ変える」と
   **明示的に決めてある**（同ファイルの `toneInk` のコメント）。**覆していない。**

---

## ⑤ 文字もアイコンも乗っていない ── 残り（対象外）

緑のドット・パルス・トグルの軌道で、**上に何も載っていない。**
...
#### ⚠️★もう1件あった ── `linear-gradient` を数え忘れていた

`weekly-jobs/route.ts:269` の副CTA「企業一覧を見る →」が
`background:linear-gradient(135deg,#F59E0B,#D97706);color:#fff` で、
**明るい側 2.15 ／ 暗い側でも 3.19**。どちらも 4.5 に届かない。

⚠️★**最初の掃き出しで漏れた理由は、grep が `background:#[0-9A-Fa-f]{6}` だったから。**
   **`linear-gradient` は hex で始まらないので当たらない。**
→ `MAIL_WARM_GRADIENT`（`#B45309 → #92400E` ／ 白で **5.02〜7.09**）に替えた。
⚠️ `lib/constants/meetingCta.ts`（カジュアル面談CTA）とは**別物**。
   あちらは 2026-09-06 に「橙を保ち、文字を濃くする」で決着済み。**混ぜないこと。**

#### ✅ 最後に、メールの白文字を**全部**数え直した（7箇所）

`style="…"` の中に `color:#fff` があるものだけを正確に抜き出した結果:

| 塗り | 比 | 件数 |
|---|---|---|
| `#002366` | 14.66 | 3 |
| ヘッダーのグラデーション（明るい側 `#3B5FD9`） | 5.49 | 1 |
| `MAIL_SUCCESS_FILL` #047857 | **5.48** | 2 |
| `MAIL_WARM_GRADIENT`（明るい側 #B45309） | **5.02** | 1 |
